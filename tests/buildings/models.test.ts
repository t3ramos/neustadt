...
} from '../../src/rendering/buildings/models.ts';
import {
  getFacilityAccess,
...
function assertInFootprint(
  ...
  assert.ok(
    bounds.min.x >= -0.5 - epsilon && bounds.max.x <= width - 0.5 + epsilon,
    `${label} crosses its X footprint: ${bounds.min.x} .. ${bounds.max.x}`,
  );
  assert.ok(
    bounds.min.z >= -0.5 - epsilon && bounds.max.z <= depth - 0.5 + epsilon,
  ...
  for (const kind of facilityKinds) {
    ...
    for (const rotation of [0, 1, 2, 3] as const) {
      const expected: [number, number] =
        rotation % 2 ? [nominal[1], nominal[0]] : [nominal[0], nominal[1]];
      const dimensions = getModelFootprint(kind, rotation);
      assert.deepEqual(
        dimensions,
        expected,
        `${kind} rotation ${rotation} must agree with construction`,
      );
      ...
      assert.ok(
        size.x > dimensions[0] * 0.8 && size.z > dimensions[1] * 0.8,
...
function meshParts(
  ...
  object.traverse((child) => {
    ...
    if (
      [size.x - width, size.y - height, size.z - depth].every((delta) => Math.abs(delta) < 0.0001)
    )
      matches.push(child);
  });
  return matches;
...
function cylindersAt(object: THREE.Object3D, height: number, x: number, z: number): THREE.Mesh[] {
  ...
  object.traverse((child) => {
    ...
    if (
      Math.abs(center.x - x) < 0.0001 &&
      Math.abs(center.z - z) < 0.0001 &&
      Math.abs(bounds.max.y - bounds.min.y - height) < 0.0001
    )
      matches.push(child);
  });
  return matches;
...
function geometrySignature(object: THREE.Object3D): string {
  ...
  object.traverse((child) => {
    ...
    const bounds = boundsOf(child);
    parts.push(
      [child.geometry.type, ...bounds.min.toArray(), ...bounds.max.toArray()]
...
test('legacy architectural family residential densities retain cottage, duplex and compact apartment proportions', () => {
  for (const variation of [0, 5, 10, 15, 20, 25, 30, 35]) {
    const cottage = createTileModel(tile('residential', { level: 1, variation }), state);
    assert.equal(
      meshParts(cottage, 0.49, 0.42, 0.5).length,
      1,
      'the cottage needs its original solid wall body',
    );
    assert.equal(
      meshParts(cottage, 0.59, 0.23, 0.61, 'BufferGeometry').length,
      1,
      'the cottage keeps its pitched roof',
    );
    const duplex = createTileModel(tile('residential', { level: 2, variation }), state);
    assert.equal(
      meshParts(duplex, 0.34, 0.75, 0.54).length,
      2,
      'the duplex needs two original attached bodies',
    );
    assert.equal(
      meshParts(duplex, 0.37, 0.18, 0.61, 'BufferGeometry').length,
      2,
      'both duplex homes keep pitched roofs',
    );
    ...
    assert.equal(
      meshParts(apartment, 0.66, 1.05 + (variation % 4) * 0.19, 0.61).length,
      1,
      'apartments keep the original moderate height and footprint',
    );
    assert.equal(
      geometrySignature(mature),
      geometrySignature(apartment),
      'level 4 keeps the original apartment design rather than introducing a prestige tower',
    );
  }
});

test('low commercial densities retain their original shops and stone offices', () => {
  for (const variation of [0, 5, 10, 15, 20, 25, 30, 35]) {
    const shop = createTileModel(tile('commercial', { level: 1, variation }), state);
    assert.equal(
      meshParts(shop, 0.74, 0.42, 0.61).length,
      1,
      'the original shop retains its solid low-rise body',
    );
    assert.equal(
      meshParts(shop, 0.78, 0.04, 0.24).length,
      1,
      'the shop keeps its projecting awning',
    );
    assert.equal(
      meshParts(shop, 0.1, 0.018, 0.22).length,
      4,
      'the awning retains four colored stripes',
    );
    const offices = createTileModel(tile('commercial', { level: 2, variation }), state);
    assert.equal(
      meshParts(offices, 0.69, 0.85 + (variation % 3) * 0.19, 0.66).length,
      1,
...
test('original factories retain a level-scaled chimney in the established location', () => {
  for (const level of [1, 2, 3, 4]) {
    ...
    assert.equal(
      meshParts(factory, 0.22, 0.15, 0.65, 'BufferGeometry').length,
      3,
      `factory level ${level} needs its three sawtooth roof sections`,
    );
    const chimneys = cylindersAt(factory, 0.65 + level * 0.18, 0.31, -0.23);
...
test('trees have rounded detailed canopies while keeping instanced forest geometry within budget', () => {
  for (let variation = 0; variation < 4; variation++) {
    ...
    assert.ok(
      triangles >= 240 && triangles <= 600,
      `tree tile ${variation} exceeds its 240–600 triangle budget: ${triangles}`,
    );
...
test('facility actors animate with a progressing clock, freeze with a paused clock, and remain finite', () => {
  ...
  for (const kind of actorKinds) {
    ...
    updateFacilityActors(actors, 12, localState);
    assert.deepEqual(
      actorTransforms(actors),
      beforePause,
      `${kind} moved with a frozen simulation clock`,
    );
    localState.speed = 1;
    ...
    for (const elapsed of [0, 23.4, 30.16, 45.24, 51.999, 52, 10000]) {
      ...
      assert.ok(
        actorTransforms(actors).every(Number.isFinite),
        `${kind} has invalid transforms at ${elapsed}s`,
      );
    }
  }
});
...
test('only a facility anchor creates actors; follower cells and regular lots do not duplicate them', () => {
  ...
  for (const kind of ['fire', 'police', 'hospital'] as const) {
    assert.equal(
      createFacilityActors(tile(kind), state),
      null,
...
test('connected facilities reserve an unscaled access ring and their visible ramp matches its shared height sampler', () => {
  for (const kind of facilityKinds)
    for (const rotation of [0, 1, 2, 3] as const) {
      ...
      for (const cell of localState.tiles)
        Object.assign(cell, { kind: 'empty', level: 0, elevation: 0, anchor: -1 });
      const [width, depth] = getModelFootprint(kind, rotation),
        x = 12,
        z = 12,
        anchor = z * localState.size + x;
      for (let dz = 0; dz < depth; dz++)
        for (let dx = 0; dx < width; dx++) {
          Object.assign(localState.tiles[(z + dz) * localState.size + x + dx], {
            kind,
            level: 1,
            rotation,
            anchor,
      ...
      for (let dx = -1; dx <= width; dx++)
        localState.tiles[(z + depth) * localState.size + x + dx].kind = 'road';
      const rootTile = localState.tiles[anchor],
        plan = getFacilityAccess(localState, rootTile)!;
      assert.equal(
        plan.connected,
        true,
        `${kind} rotation ${rotation} needs an adjacent street connection`,
      );
      ...
      model.traverse((part) => {
        ...
        for (let i = 0; i < part.geometry.userData.surfaceVertexCount; i++) {
          const wx = positions.getX(i) + x - localState.size / 2 + 0.5;
          const wz = positions.getZ(i) + z - localState.size / 2 + 0.5;
          const expected = sampleFacilityAccessHeight(plan, wx, wz) ?? plan.baseY + 0.052;
          assert.ok(
            Math.abs(positions.getY(i) + plan.baseY - expected) < 2e-5,
...
test('road-going emergency models have correct forward axes and complete collision dimensions', () => {
  for (const kind of ['firetruck', 'ambulance', 'police'] as const) {
    const car = createFacilityServiceVehicle(kind),
      bounds = boundsOf(car),
    ...
    assert.ok(
      car.userData.vehicleDimensions.wheelBase > 0 && car.userData.vehicleDimensions.mass > 0,
    );
    assert.ok(
...
test('a factory faces its actual street and the selected gate opens the curb without a fake intersection', () => {
  for (const [dx, dz, rotation] of [
    ...
    road.kind = 'road';
    const continuation =
      localState.tiles[(12 + dz + (dx ? 1 : 0)) * localState.size + 12 + dx + (dz ? 1 : 0)];
    ...
    const model = createTileModel(factory, localState),
      content = model.getObjectByName('facility-content')!;
    assert.ok(content);
    assert.equal(content.scale.z, 0.73);
    assert.equal(content.position.z, -0.2);
    assert.ok(Math.abs(content.parent!.rotation.y + (rotation * Math.PI) / 2) < 1e-8);
    assert.equal(
      model.children.filter((part) => part.name === 'facility-access-paving').length,
      1,
      'a small industrial lot needs a loading apron, not a perimeter highway',
    );
    const roadModel = createTileModel(road, localState),
      apron = roadModel.getObjectByName('road-facility-apron');
    assert.ok(apron, 'the driveway must visibly cross the street sidewalk');
    const end = plan.points[plan.points.length - 1];
    assert.ok(
      Math.abs(
        (dx ? apron.position.z : apron.position.x) -
          (dx
            ? end.z - (road.z - localState.size / 2 + 0.5)
            : end.x - (road.x - localState.size / 2 + 0.5)),
      ) < 1e-8,
      'the curb opening follows the actual lane offset',
    );
    assert.equal(
      meshParts(roadModel, 0.075, 0.004, 0.12).length,
      0,
      'a factory entrance is not a road intersection',
    );
    assert.equal(
      createTileModel(continuation, localState).getObjectByName('road-facility-apron'),
      undefined,
...
test('triangulated access surfaces support wheels between vertices without bridging curved ramps', () => {
  const cases = [
    { kind: 'police', rotation: 1, side: 'west', elevation: 0.5 },
    { kind: 'fire', rotation: 3, side: 'east', elevation: 0.5 },
    { kind: 'hospital', rotation: 2, side: 'north', elevation: 0.2 },
    { kind: 'airport', rotation: 0, side: 'south', elevation: 0 },
  ] as const;
  for (const { kind, rotation, side, elevation } of cases) {
    const localState = createCity(917, true, 40),
      x = 15,
      z = 15;
    for (const cell of localState.tiles)
      Object.assign(cell, { kind: 'empty', level: 0, elevation: 0, anchor: -1 });
    ...
    if (side === 'north' || side === 'south') {
      ...
      for (let roadX = x - 2; roadX < x + width + 2; roadX++)
        localState.tiles[roadZ * localState.size + roadX].kind = 'road';
    } else {
      const roadX = side === 'west' ? x - 1 : x + width;
      for (let roadZ = z - 2; roadZ < z + depth + 2; roadZ++)
        localState.tiles[roadZ * localState.size + roadX].kind = 'road';
    }
    const rootTile = localState.tiles[anchor],
      plan = getFacilityAccess(localState, rootTile)!;
    ...
    const model = createTileModel(rootTile, localState);
    model.position.set(x - localState.size / 2 + 0.5, elevation, z - localState.size / 2 + 0.5);
    model.updateMatrixWorld(true);
    const surfaces: THREE.Mesh[] = [];
    model.traverse((part) => {
      if (part instanceof THREE.Mesh && part.userData.drivingSurface) surfaces.push(part);
    });
    if (kind === 'airport') {
      const triangles = surfaces.reduce((total, mesh) => total + mesh.geometry.index!.count / 3, 0);
      assert.ok(
        triangles < 20_000,
        `a flat airport should retain coarse pavement, got ${triangles} triangles`,
      );
    }
    ...
    for (let i = 0; i + 1 < plan.points.length; i += 3) {
      ...
      for (const offset of [-0.11, 0, 0.11]) {
        const wx = a.x + dx * 0.37 - (dz / length) * offset,
          wz = a.z + dz * 0.37 + (dx / length) * offset;
        ...
        assert.ok(
          Math.abs(hit.point.y - expected) <= 0.004,
...
test('original opaque glazing remains non-emissive while discrete window lights stay subtle', () => {
  ...
  const glass = materials.filter(
    (material) => material.userData.glazing && !material.userData.facadeTexture,
  );
  ...
  buildings.traverse((child) => {
    ...
    const dimensions = boundsOf(child)
      .getSize(new THREE.Vector3())
      .toArray()
      .sort((a, b) => a - b);
    assert.ok(
      dimensions[0] <= 0.02 && dimensions[1] * dimensions[2] <= 0.025,
    ...
    for (const blend of [0, 0.5, 1]) {
      ...
      for (const wet of [false, true]) {
        ...
        for (const material of lights) {
          assert.ok(material instanceof THREE.MeshStandardMaterial);
          assert.ok(
            material.emissiveIntensity >= 0 && material.emissiveIntensity <= 0.2,
      ...
      const intensity = lights.reduce(
        (sum, material) => sum + (material as THREE.MeshStandardMaterial).emissiveIntensity,
        0,
      );
      assert.ok(
        intensity > previousNightIntensity,
        'window illumination should increase continuously toward night',
      );
...
test('raytracing snapshots preserve original opaque glazing and low window emission', async () => {
  ...
  source.add(createTileModel(tile('commercial', { level: 2 }), state));
  source.add(createTileModel(tile('residential', { level: 2 }), state));
  ...
  try {
    ...
    const originalLights = originalMaterials.filter(
      (material) => material.userData.buildingWindowLight,
    );
    ...
    const snapshotLights = snapshotMaterials.filter(
      (material) => material.userData.buildingWindowLight,
    );
    ...
    for (const material of [...snapshotGlass, ...snapshotLights]) {
      ...
      assert.ok(
        !originalMaterials.includes(material),
        'snapshot materials must remain independently owned',
      );
      assert.equal(material.transparent, false);
      assert.equal(material.opacity, 1);
      if (material.userData.glazing && !material.userData.facadeTexture)
        assert.equal(material.emissiveIntensity, 0);
      else assert.ok(material.emissiveIntensity > 0 && material.emissiveIntensity <= 0.2);
    }
    setModelNightBlend(0);
