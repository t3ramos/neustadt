...
import { createTrafficSignals, getTrafficSignalFixture } from '../src/traffic-signals.ts';
import type { CityState } from '../src/types.ts';

function city(size = 40): CityState {
  const state = createCity(571, true, size);
  for (const tile of state.tiles) { tile.kind = 'empty'; tile.elevation = 0; }
  return state;
}
...
test('all four inbound approaches receive a head on their own right curb facing the driver', t => {
  const state = city(), network = cross(state), junction = network.junctions[0];
  ...
  for (let index = 0; index < junction.approaches.length; index++) {
    const approach = junction.approaches[index], fixture = getTrafficSignalFixture(state, approach)!;
    const dx = fixture.mast.x - (approach.entry.x + .5 - state.size / 2);
    const dz = fixture.mast.z - (approach.entry.z + .5 - state.size / 2);
    assert.ok(Math.abs(dx * -approach.direction.z + dz * approach.direction.x - .465) < 1e-9);
    assert.ok(Math.abs(dx * approach.direction.x + dz * approach.direction.z + .465) < 1e-9);
    assert.ok(Math.abs(dx) - .055 / 2 > .335 && Math.abs(dz) - .055 / 2 > .335, 'The entire footing clears every asphalt arm');
    assert.ok(Math.abs(dx) + .055 / 2 < .5 && Math.abs(dz) + .055 / 2 < .5, 'No footing extends into an adjacent facility driveway');
    ...
    assert.ok((lensPosition.x - fixture.head.x) * fixture.facing.x + (lensPosition.z - fixture.head.z) * fixture.facing.z > .03);
    const lensMatrix = new THREE.Matrix4(); mesh(helper, 'red-lenses').getMatrixAt(index, lensMatrix);
    const normal = new THREE.Vector3(0, 0, 1).transformDirection(lensMatrix);
    assert.ok(normal.x * fixture.facing.x + normal.z * fixture.facing.z > .99999);
  }
});

test('rendered red, amber, and green always match the controller, including all-red clearance', t => {
  const state = city(), network = cross(state), controller = createTrafficController(network);
  const helper = createTrafficSignals(state, network.junctions); t.after(() => helper.dispose());
  const verify = () => {
    const signals = controller.signalStates(); helper.setSignals(signals);
    for (const fixture of helper.getDebug().positions) assert.equal(fixture.color, signals.find(signal => signal.approachId === fixture.approachId)?.color);
  ...
  verify(); assert.deepEqual(helper.getDebug().aspects, { red: 3, yellow: 0, green: 1 });
  controller.update(5, []); verify(); assert.deepEqual(helper.getDebug().aspects, { red: 3, yellow: 1, green: 0 });
  controller.update(1, []); verify(); assert.deepEqual(helper.getDebug().aspects, { red: 4, yellow: 0, green: 0 });
  controller.update(.6, []); verify(); assert.deepEqual(helper.getDebug().aspects, { red: 3, yellow: 0, green: 1 });
...
test('terrain and demolition updates refresh fixtures and drop removed approach lights', t => {
  const state = city(), network = cross(state), helper = createTrafficSignals(state, network.junctions); t.after(() => helper.dispose());
  ...
  helper.update(state, network.junctions);
  assert.equal(helper.getDebug().count, 0, 'Even a stale network cannot render on a demolished road');
...
test('unchanged phases reuse buffers, growth reuses materials, and disposal releases owned resources once', () => {
  const state = city(), network = cross(state), controller = createTrafficController(network);
  const helper = createTrafficSignals(state, network.junctions), parent = new THREE.Scene(); parent.add(helper.group);
  helper.setSignals(controller.signalStates());
  const red = mesh(helper, 'red-lenses'), version = red.instanceMatrix.version;
  helper.setSignals(controller.signalStates()); helper.update(state, network.junctions);
  assert.equal(red.instanceMatrix.version, version, 'An unchanged frame must not re-upload light geometry');
  const initialPole = mesh(helper, 'poles'), originalMaterial = initialPole.material;
  let initialDisposals = 0; initialPole.addEventListener('dispose', () => initialDisposals++);
  const resources = new Set<THREE.BufferGeometry | THREE.Material>();
  for (const child of helper.group.children as THREE.InstancedMesh[]) {
    resources.add(child.geometry);
    for (const material of Array.isArray(child.material) ? child.material : [child.material]) resources.add(material);
  }
  ...
  for (let z = 5; z < 30; z += 5) for (let x = 5; x < 30; x += 5) junctions = cross(state, x, z).junctions;
  ...
  helper.dispose(); helper.dispose(); helper.update(state, junctions); helper.setSignals(controller.signalStates());
  assert.equal(parent.children.length, 0); assert.equal(helper.group.children.length, 0);
  assert.equal(disposed.size, resources.size); assert.ok([...disposed.values()].every(count => count === 1));
  assert.equal(helper.getDebug().count, 0);
});
