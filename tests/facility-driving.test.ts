...
import {
  createDrivingController, drivingObstacle, drivingSurfaceHeight, getVehicleDimensions,
  type DrivableCar,
} from '../src/driving.ts';
import {
  FACILITY_ACCESS_FOOTPRINTS, facilityLocalToWorld, getFacilityAccess, sampleFacilityAccessAt,
...
function fixture(kind: Kind, rotation: Tile['rotation'], side: Side, elevation: number) {
  const state = createCity(917, true, 40), x = 15, z = 15;
  ...
  for (let zz = z; zz < z + depth; zz++) for (let xx = x; xx < x + width; xx++) {
    Object.assign(state.tiles[zz * state.size + xx], { kind, level: 1, elevation, anchor, rotation });
  }
  ...
  for (let i = 0; i < 4; i++) state.tiles[(roadZ + dz * i) * state.size + roadX + dx * i].kind = 'road';
  state.revision++;
  const plan = getFacilityAccess(state, state.tiles[anchor]);
  assert.ok(plan?.connected && plan.road && plan.to && plan.previous);
  const model = createFacilityServiceVehicle(kind === 'fire' ? 'firetruck' : kind === 'hospital' ? 'ambulance' : 'police');
  return { state, plan, model, dimensions: getVehicleDimensions(model), direction: { x: dx, z: dz } };
}
...
for (const elevation of [0, .2]) {
  test(`full emergency vehicles clear every driveway side and rotation at elevation ${elevation}, while walls remain solid`, () => {
    for (const kind of kinds) for (const rotation of rotations) for (const side of sides) {
      ...
      for (let i = 0; i < plan.points.length; i++) {
        ...
        const previous = plan.points[Math.max(0, i - 1)], next = plan.points[Math.min(plan.points.length - 1, i + 1)];
        const yaw = Math.atan2(next.x - previous.x, next.z - previous.z);
        assert.equal(drivingObstacle(state, point.x, point.z, yaw, dimensions), undefined,
          `${label}: complete vehicle must clear the route at point ${i}`);
      }
      const localWall = kind === 'fire' ? { x: -.20, z: -.25 }
        : kind === 'hospital' ? { x: -.10, z: -.50 } : { x: -.30, z: -.29 };
      const wall = facilityLocalToWorld(plan, {
        x: localWall.x * plan.contentScale.x, z: localWall.z * plan.contentScale.z,
      });
      assert.equal(drivingObstacle(state, wall.x, wall.z, 0, dimensions), 'Gebäude im Weg',
...
test('an elevated driveway never supplies a phantom hill past its endpoint on the actual road', () => {
  for (const kind of kinds) for (const side of sides) {
    const { state, plan, direction } = fixture(kind, 0, side, .5);
    const end = plan.points.at(-1)!;
    for (const distance of [.0001, .02, .10, .16, .18, .20, .22, .225, .30, .7]) {
      const x = end.x + direction.x * distance, z = end.z + direction.z * distance;
      assert.equal(sampleFacilityAccessAt(state, x, z), null,
        `${kind}/${side}: the road owns wheel support ${distance} tiles beyond the gate`);
      assert.ok(Math.abs(drivingSurfaceHeight(state, x, z) - sampleRoadHeight(state, x, z) - .057) < 1e-9,
...
test('the driving controller takes full service vehicles down the driveway and reverses back up without a ledge', () => {
  for (const kind of kinds) for (const elevation of [.2, .5]) for (const reversing of [false, true]) {
    const { state, plan, model } = fixture(kind, 0, 'south', elevation);
    const end = plan.points.at(-1)!, startZ = end.z + (reversing ? .7 : -.20);
    model.position.set(end.x, drivingSurfaceHeight(state, end.x, startZ), startZ);
    model.rotation.y = 0;
    const car: DrivableCar = {
      model, from: plan.road!, previous: plan.previous!, to: plan.to!, progress: 0, speed: 0,
    };
    ...
    try {
      ...
      for (let frame = 0; frame < 360; frame++) {
        ...
        assert.equal(controller.getStatus().blocked, undefined, `${label}: no physical slope or obstacle should stop the vehicle`);
        assert.ok(model.position.toArray().every(Number.isFinite), `${label}: finite suspension`);
        arrived = reversing ? model.position.z < end.z - .19 : model.position.z > end.z + .7;
        if (arrived) break;
      }
      ...
      if (!reversing) {
        assert.ok(Math.abs(model.position.y - drivingSurfaceHeight(state, model.position.x, model.position.z)) < .025,
...
test('cached facility collision geometry follows road addition and removal without relying on a revision bump', () => {
  ...
  const point=facilityLocalToWorld(plan,{x:-.70,z:-.30});
  assert.equal(drivingObstacle(state,point.x,point.z,0,dimensions),undefined,
    'The connected property leaves its outer side lane clear');
  const road=state.tiles[plan.road!.z*state.size+plan.road!.x],revision=state.revision;
  road.kind='empty';
  assert.equal(drivingObstacle(state,point.x,point.z,0,dimensions),'Gebäude im Weg',
  ...
  assert.equal(state.revision,revision,'This is a neighbor-driven cache regression, not a global reset');
});
