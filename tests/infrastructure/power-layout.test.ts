...
import {
  getPowerLayout,
  powerLayoutSignature,
  type PowerLayout,
} from '../../src/infrastructure/power-layout.ts';
import {
  createCity,
  getFootprint,
  recalculate,
...
import { sampleGroundHeight } from '../../src/rendering/world/terrain.ts';
import { sampleRoadHeight } from '../../src/rendering/infrastructure/roads.ts';
import { powerServiceCandidates } from '../../src/infrastructure/power-service.ts';
...
function building(
  ...
  const root = at(state, x, z),
    size = TOOL_DEFS[kind]?.footprint;
...
function validateSpans(state: CityState, layout: PowerLayout): void {
  ...
  for (const span of layout.spans) {
    ...
    assert.ok(
      span.path.length >= 2 && span.path.length <= 4,
      'Straight spans are bounded to three cells',
    );
    for (let i = 0; i < span.path.length; i++) {
      ...
      assert.ok(
        ...
        'No overhead corridor passes through a structure',
      );
      if (i === 0) continue;
      const previous = state.tiles[span.path[i - 1]];
      assert.equal(
        Math.abs(previous.x - tile.x) + Math.abs(previous.z - tile.z),
        1,
        'No gap or diagonal is skipped',
      );
      const a = span.path[i - 1],
        b = span.path[i],
        edge = a < b ? `${a}:${b}` : `${b}:${a}`;
...
test('line graph compresses straight runs and preserves every corner, branch, ring and endpoint', () => {
  ...
  assert.ok(
    layout.poles.some((pole) => pole.x === 10 && pole.z === 10 && pole.directions.length === 4),
  );
  assert.ok(
    layout.poles.some((pole) => pole.x === 30 && pole.z === 30 && pole.directions.length === 0),
  );
  const edges = layout.spans.reduce((sum, span) => sum + span.path.length - 1, 0);
  assert.equal(
    edges,
    12 + 10 + 24,
    'Every explicit orthogonal wire edge is represented exactly once',
  );
...
test('only large live facilities get service drops; ordinary lots and highrises share block supply without meters', () => {
  const state = city(),
    source = building(state, 2, 2, 'power');
  for (let x = 6; x <= 15; x++) wire(state, x, 4);
  const home = building(state, 10, 5);
  home.level = 4;
  const facility = building(state, 12, 5, 'police');
  const emptyZone = at(state, 11, 5);
  emptyZone.kind = 'commercial';
  const deadHome = building(state, 25, 20, 'police');
  wire(state, 25, 18);
  recalculate(state);
  assert.ok(home.powered);
  assert.equal(deadHome.powered, false);
  const layout = getPowerLayout(state),
    ids = layout.services.map((service) => service.building);
  assert.ok(
    ids.includes(idOf(state, source)),
    'The generating facility exports to the wired network',
  );
  assert.ok(ids.includes(idOf(state, facility)));
  assert.ok(
    !ids.includes(idOf(state, home)),
    'Even a fully developed highrise has no individual drop',
  );
  ...
  assert.equal(
    new Set(ids).size,
    ids.length,
    'Footprint followers must never create duplicate service cables',
  );
  assert.ok(
    layout.services.every((service) => layout.poles.some((pole) => pole.id === service.pole)),
  );
});
...
test('large facility uses a legal supplied nearby line instead of a closer dead wire', () => {
  ...
  recalculate(state);
  assert.ok(home.powered);
  const service = getPowerLayout(state).services.find(
    (item) => item.building === idOf(state, home),
  );
  assert.ok(service);
...
test('source destruction and conductor fires remove false service connections immediately', () => {
  const state = city(),
    source = building(state, 2, 2, 'wind');
  for (let x = 4; x <= 14; x++) wire(state, x, 3);
  const home = building(state, 14, 4, 'police');
  recalculate(state);
  assert.ok(
    getPowerLayout(state).services.some((service) => service.building === idOf(state, home)),
  );
  at(state, 8, 3).fire = 1;
  recalculate(state);
  ...
  recalculate(state);
  assert.equal(
    getPowerLayout(state).services.length,
    0,
    'A burning plant contributes no network supply',
  );
});
...
test('back-row buildings keep simulated electricity but do not get cables through a front building', () => {
  ...
  building(state, 2, 2, 'wind');
  for (let x = 4; x <= 14; x++) wire(state, x, 3);
  const front = building(state, 11, 4),
    back = building(state, 11, 5, 'police');
  building(state, 12, 4);
  recalculate(state);
  assert.ok(front.powered && back.powered, 'Developed buildings really conduct through each other');
  const layout = getPowerLayout(state);
  assert.ok(
    !layout.services.some((service) => service.building === idOf(state, front)),
    'The front ordinary lot has no service cable either',
  );
  assert.ok(
    !layout.services.some((service) => service.building === idOf(state, back)),
    'No visual cable should cut through the front roof',
  );
});
...
test('drop obstruction uses the physical curb pole position instead of the center of the road', () => {
  ...
  assert.ok(
    !blocked.services.some((service) => service.building === idOf(state, destination)),
    'The actual +0.86 curb ray crosses the southeast neighbor',
  );
  blockingLot.kind = 'empty';
  blockingLot.level = 0;
  recalculate(state);
  const unblocked = getPowerLayout(state).services.find(
    (service) => service.building === idOf(state, destination),
  );
  assert.ok(unblocked);
  assert.equal(
    unblocked.targetZ,
    4.95,
    'The curb pole sits south of the destination and uses its southern lot edge',
  );
});
...
test('all large facilities and rotations use a single nearest footprint-edge terminal', () => {
  ...
  for (const kind of kinds)
    for (const rotation of [0, 1, 2, 3] as const) {
      const state = city(),
        facility = building(state, 12, 12, kind, rotation);
      ...
      recalculate(state);
      const layout = getPowerLayout(state);
      const services = layout.services.filter(
        (service) => service.building === idOf(state, facility),
      );
      assert.equal(services.length, 1, `${kind}, rotation ${rotation}: one anchor service`);
      const service = services[0];
      assert.equal(service.targetX, maxX + 0.95);
      assert.equal(service.targetZ, maxZ + 0.5);
...
test('a dense zoned block needs no individual house cables, including empty plots and highrises', () => {
  ...
  assert.equal(
    ...
    2,
    'Only the power plant and clinic receive visible service connections',
  );
  assert.ok(layout.services.some((service) => service.building === idOf(state, clinic)));
  assert.ok(
    layout.services.every((service) => !!TOOL_DEFS[state.tiles[service.building].kind]?.footprint),
  );
});
...
test('roadside service drops stay on their own curb and never cross the starting road asphalt', () => {
  for (const [axis, destinationX, destinationZ, allowed] of [
    ['vertical', 8, 10, false],
    ['vertical', 11, 10, true],
    ['horizontal', 10, 8, false],
    ['horizontal', 10, 11, true],
  ] as const) {
    const state = city(),
      pole = wire(state, 10, 10, 'road');
    if (axis === 'vertical') {
      at(state, 10, 9).kind = 'road';
      at(state, 10, 11).kind = 'road';
    } else {
      at(state, 9, 10).kind = 'road';
      at(state, 11, 10).kind = 'road';
    }
    const destination = building(state, destinationX, destinationZ, 'police');
    const candidates = powerServiceCandidates(
      state,
      getFootprint(state, destination),
      (tile) => !!TOOL_DEFS[tile.kind]?.footprint,
    );
    assert.equal(
      candidates.some((candidate) => candidate.pole === idOf(state, pole)),
      allowed,
      `${axis} road, facility ${destinationX},${destinationZ}`,
    );
  }
});
...
test('unwired roads block automatic facility drops but explicit trunk crossings remain visible', () => {
  ...
  recalculate(state);
  assert.ok(
    !getPowerLayout(state).services.some(
      (service) => service.building === idOf(state, destination),
    ),
  );
  ...
  assert.ok(
    connected.spans.some(
      (span) => span.path.includes(10 * state.size + 8) && span.path.includes(10 * state.size + 9),
    ),
    'The player-placed trunk can cross a road',
  );
});
...
test('one legal facility drop feeds the touching residential block and preserves its real incoming cable', () => {
  ...
  recalculate(state);
  assert.ok(
    facility.powered && home.powered,
    'The legal service supplies the whole attached building block',
  );
  assert.ok(localPole.powered, 'An existing wire inside that block also receives its supply');
  const incoming = getPowerLayout(state).services.find(
    (service) => service.building === idOf(state, facility),
  );
  assert.ok(incoming);
  assert.equal(
    incoming.pole,
    8 * state.size + 10,
    'The real incoming cable must not switch to the closer, formerly dead local pole',
  );
  at(state, 11, 8).kind = 'road';
  recalculate(state);
  assert.equal(home.powered, false, 'A road across the sole service disconnects the entire block');
  assert.ok(
    !getPowerLayout(state).services.some((service) => service.building === idOf(state, facility)),
  );
});
...
test('facility feeds can supply another isolated facility block through a short legal second service', () => {
  ...
  const layout = getPowerLayout(state);
  assert.equal(
    layout.services.find((service) => service.building === idOf(state, first))?.pole,
    8 * state.size + 10,
  );
  assert.equal(
    layout.services.find((service) => service.building === idOf(state, second))?.pole,
    9 * state.size + 14,
  );
});
...
test('enclosed urban gardens share the supply graph and remote boundary edits invalidate the layout', () => {
  const state = city();
  for (let x = 8; x <= 18; x++) {
    at(state, x, 8).kind = 'road';
    at(state, x, 16).kind = 'road';
  }
  for (let z = 8; z <= 16; z++) {
    at(state, 8, z).kind = 'road';
    at(state, 18, z).kind = 'rail';
  }
  building(state, 4, 10, 'wind');
  wire(state, 6, 10);
  wire(state, 7, 10);
  wire(state, 8, 10, 'road');
  ...
  const homes = [
    ...
    [10, 14],
    [16, 10],
  ].map(([x, z]) => building(state, x, z));
  recalculate(state);
  assert.ok(
    homes.every((home) => home.powered) && facility.powered,
    'The enclosed city block includes four isolated lots and their interior gardens',
  );
  const layout = getPowerLayout(state);
  assert.ok(layout.services.some((service) => service.building === idOf(state, facility)));
  assert.ok(
    layout.services.every((service) => !!TOOL_DEFS[state.tiles[service.building].kind]?.footprint),
    'The isolated ordinary lots need no visible drop',
  );
  const signature = powerLayoutSignature(state);
  at(state, 18, 15).kind = 'empty';
  recalculate(state);
  assert.notEqual(
    powerLayoutSignature(state),
    signature,
    'A remote railway opening changes block reachability',
  );
  assert.ok(
    homes.every((home) => !home.powered),
    'The open countryside uses touching lots instead of an unbounded land block',
  );
});
...
test('sources export to their own component rather than a stronger disconnected nearby network', () => {
  const state = city(),
    wind = building(state, 8, 8, 'wind');
  const ownWire = wire(state, 10, 8);
  building(state, 2, 8, 'power');
  wire(state, 6, 8);
  recalculate(state);
  const service = getPowerLayout(state).services.find(
    (item) => item.building === idOf(state, wind),
  );
  assert.ok(service);
...
test('mast foundations use real hilly transport and terrain surfaces', () => {
  const state = city();
  for (const tile of state.tiles) tile.elevation = (tile.x % 4) * 0.5;
  for (let x = 5; x <= 12; x++) wire(state, x, 10, 'road');
  wire(state, 20, 20);
  const bridge = wire(state, 25, 25, 'road');
  bridge.elevation = -2;
  const layout = getPowerLayout(state);
  for (const pole of layout.poles) {
    const tile = state.tiles[pole.id],
      offset = tile.kind === 'road' ? 0.86 : 0.5;
    const x = pole.x + offset - state.size / 2,
      z = pole.z + offset - state.size / 2;
    assert.equal(
      pole.elevation,
      tile.kind === 'road' ? sampleRoadHeight(state, x, z) : sampleGroundHeight(state, x, z),
    );
  }
...
test('layout cache ignores time and money but tracks supply, development, fire, orientation and neighboring terrain', () => {
  const state = city();
  building(state, 2, 2, 'wind');
  const line = wire(state, 4, 3),
    home = building(state, 6, 3);
  recalculate(state);
  let signature = powerLayoutSignature(state);
  state.money += 100;
  state.month++;
  state.revision++;
  state.settings.timeOfDay += 1;
