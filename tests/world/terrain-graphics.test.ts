...
} from '../../src/rendering/world/terrain.ts';
...
function hillyCity(): CityState {
  ...
  for (const tile of city.tiles) {
    tile.kind = 'empty';
    tile.elevation =
      Math.round((3 + Math.sin(tile.x * 0.47) * 1.5 + Math.cos(tile.z * 0.31)) * 2) / 2;
  }
  return city;
}
...
test('indexed terrain has no holes and exact height sampling matches 100 raycasts through irregular elevations', (t) => {
  ...
  for (const tile of city.tiles) {
    ...
    tile.kind = tile.elevation < 0 ? 'water' : random() < 0.25 ? 'residential' : 'empty';
  }
  for (let z = 0; z < 16; z += 8)
    for (let x = 0; x < 16; x += 8) {
      ...
      assert.ok(
        geometry.getAttribute('position').count < geometry.index.count,
        'Vertices must be reused across terrain triangles',
      );
      group.add(chunk);
    }
  ...
  for (let i = 0; i < 100; i++) {
    const x = 0.001 + random() * 15.998 - city.size / 2,
      z = 0.001 + random() * 15.998 - city.size / 2;
    const actual = surfaceHeight(group, x, z),
      sampled = sampleGroundHeight(city, x, z);
    assert.ok(
      Math.abs(actual - sampled) < 0.00001,
      `Sampler differs from rendered surface: ${actual} versus ${sampled}`,
    );
  }
});
...
test('adjacent terrain chunks share identical edge heights and normals in both directions', (t) => {
  ...
  function boundary(chunk: THREE.Group, axis: 'x' | 'z'): Map<number, number[]> {
    ...
    const out = new Map<number, number[]>(),
      boundaryPosition = 8 - city.size / 2;
  ...
  for (const [neighbor, axis] of [
    [right, 'x'],
    [below, 'z'],
  ] as const) {
    const first = boundary(center, axis),
      second = boundary(neighbor, axis);
...
test('occupied tiles have a planar foundation at their exact persisted elevation', (t) => {
  ...
  for (const u of [0.05, 0.5, 0.95])
    for (const v of [0.05, 0.5, 0.95]) {
      const x = tile.x + u - city.size / 2,
        z = tile.z + v - city.size / 2;
      assert.equal(sampleGroundHeight(city, x, z), tile.elevation);
      assert.ok(Math.abs(surfaceHeight(chunk, x, z) - tile.elevation) < 0.000001);
    }
  // The bevel still joins the surrounding hills with the same sampler/mesh geometry.
  const x = tile.x + 0.01 - city.size / 2,
    z = tile.z + 0.5 - city.size / 2;
  assert.ok(Math.abs(surfaceHeight(chunk, x, z) - sampleGroundHeight(city, x, z)) < 0.00001);
});

test('terrain skirt closes all four perimeter edges using vertical triangles only', (t) => {
  const city = hillyCity(),
    mats = materials(),
    skirt = buildTerrainSkirt(city, mats.earth);
  ...
  assert.equal(indices.count / 3, city.size * 4 * 2 * 6);
  for (let i = 0; i < indices.count; i += 3) {
    const corners = [0, 1, 2].map((n) =>
      new THREE.Vector3().fromBufferAttribute(positions, indices.getX(i + n)),
    );
    ...
    assert.ok(
      corners.some((p) => p.y !== corners[0].y),
      'The skirt must not contain overlapping horizontal surfaces',
    );
  }
  const ray = new THREE.Raycaster();
  for (let i = 0; i < city.size; i++)
    for (let side = 0; side < 4; side++) {
      const along = i + 0.5 - half;
      const x = side === 0 ? -half : side === 1 ? half : along;
      const z = side === 2 ? -half : side === 3 ? half : along;
      const y = sampleGroundHeight(city, x, z) - 0.25;
      ...
      assert.ok(Math.abs(hit.distance - 1) < 0.00001, 'The near perimeter face must face outward');
    }
});

test('plate edges seal sea and land to a closed floor with opaque mineral strata', (t) => {
  const city = hillyCity(),
    mats = materials();
  // Include submerged seabed, sloping shoreline crossings and a road at the boundary.
  for (const tile of city.tiles) {
    tile.elevation = tile.x > 20 ? -2 : tile.x > 17 ? -0.5 : 2;
    tile.kind = tile.elevation < 0 ? 'water' : 'empty';
  }
  city.tiles[10 * city.size].kind = 'road';
  city.tiles[10 * city.size].elevation = 3;
  ...
  assert.equal(getTerrainPlateFloor(city), -4);
  const half = city.size / 2;
  const walls = skirt.getObjectByName('terrain-perimeter') as THREE.Mesh;
  const color = walls.geometry.getAttribute('color');
  assert.ok(new Set(Array.from(color.array).map((value) => value.toFixed(4))).size > 12);
  const water = skirt.getObjectByName('terrain-water-edge') as THREE.Mesh;
  assert.ok(water);
  const points = water.geometry.getAttribute('position');
  for (let i = 0; i < points.count; i++) {
    const x = points.getX(i),
      y = points.getY(i),
      z = points.getZ(i);
    assert.ok(Math.abs(x) === half || Math.abs(z) === half);
    assert.ok(y <= TERRAIN_WATER_LEVEL + 1e-6);
    assert.ok(y >= sampleGroundHeight(city, x, z) - 1e-5);
  }
  const ray = new THREE.Raycaster();
  for (let side = 0; side < 4; side++)
    for (let i = 0; i < city.size; i++) {
      ...
      const x = side === 0 ? -half : side === 1 ? half : along;
      const z = side === 2 ? -half : side === 3 ? half : along;
      const top = Math.max(TERRAIN_WATER_LEVEL, sampleGroundHeight(city, x, z));
      const direction = new THREE.Vector3(
        side === 0 ? 1 : side === 1 ? -1 : 0,
        0,
        side === 2 ? 1 : side === 3 ? -1 : 0,
      ...
      for (const y of [
        top - 0.025,
        (top + getTerrainPlateFloor(city)) / 2,
        getTerrainPlateFloor(city) + 0.025,
      ]) {
        ray.set(new THREE.Vector3(x, y, z).addScaledVector(direction, -1), direction);
        const hit = ray.intersectObject(skirt, true)[0];
        assert.ok(
          hit && Math.abs(hit.distance - 1) < 1e-5,
          `No holes at side ${side}, cell ${i}, height ${y}`,
        );
      }
    }
  ray.set(new THREE.Vector3(0, -8, 0), new THREE.Vector3(0, 1, 0));
  assert.equal(ray.intersectObject(skirt, true)[0]?.object.name, 'terrain-underside');
  for (const object of skirt.children) {
