...
const ground = (): CityState => {
  ...
  for (const tool of ['raise', 'lower', 'level', 'airport'] as Tool[])
...
test('lowering below sea level creates water and raising it back creates buildable land', () => {
  ...
  assert.equal(tile.elevation, -TERRAIN_STEP);
  assert.equal(tile.kind, 'water');
  const money = city.money;
  assert.equal(build(city, [point], 'residential').ok, false);
...
test('level uses an explicit target across a brush and charges only actual earth moved', () => {
  ...
  tileAt(city, 10, 10).elevation = 0;
  tileAt(city, 11, 10).elevation = 1;
  tileAt(city, 12, 10).elevation = 2;
...
test('terrain limits and unaffordable earthworks do not alter the map or treasury', () => {
  ...
  assert.equal(build(city, [point], 'raise').ok, false);
  assert.equal(JSON.stringify(city), before);
  tile.elevation = MIN_ELEVATION;
  tile.kind = 'water';
  before = JSON.stringify(city);
  assert.equal(build(city, [point], 'lower').ok, false);
  assert.equal(JSON.stringify(city), before);
  tile.elevation = 1;
  tile.kind = 'empty';
  city.money = 34;
...
test('terraforming survives saves and bridge demolition uses modified terrain rather than the old coast', () => {
  ...
  assert.equal(tile.kind, 'water');
  assert.ok(build(city, [point], 'road').ok);
