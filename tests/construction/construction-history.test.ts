...
import type { CityState, Point, Tool } from '../../src/domain/types.ts';
...
const line = (count: number, z = 10): Point[] =>
  Array.from({ length: count }, (_, i) => ({ x: 10 + i, z }));
function construct(
  city: CityState,
...
test('undo remains available through months and refunds only the original construction cost', () => {
  ...
  const month = city.month,
    money = city.money,
    points = structuredClone(city.history),
    events = structuredClone(city.events);
...
test('unrelated growth, loans, settings, and resident consequences survive construction undo', () => {
  ...
  city.name = 'A continuing city';
  city.speed = 3;
  city.tax = 12;
  city.funding.fire = 140;
  Object.assign(city.settings, {
    weather: 'rain',
    timeOfDay: 21,
  ...
  const other = structuredClone(tile(city, 25, 25)),
    effects = structuredClone(city.citizenEffects),
    money = city.money,
    settings = structuredClone(city.settings);
  assert.ok(undoConstruction(city, history).ok);
  assert.equal(city.name, 'A continuing city');
  assert.equal(city.speed, 3);
  assert.equal(city.tax, 12);
  assert.equal(city.funding.fire, 140);
  assert.equal(city.loan, 10000);
  assert.equal(city.progression.counters.loansTaken, 1);
  assert.equal(city.money, money + result.cost);
...
test('undoing a pipe beneath a zone preserves the grown building and its age', () => {
  ...
  city.month = 14;
  assert.ok(undoConstruction(city, history).ok);
  assert.equal(tile(city).kind, 'residential');
  assert.equal(tile(city).level, 3);
  assert.equal(tile(city).age, 8);
  assert.equal(tile(city).hasPipe, false);
  assert.equal(tile(city).hasPowerLine, true);
...
test('successive utility and zone edits undo independently after growth', () => {
  ...
  assert.ok(undoConstruction(city, history).ok);
  assert.equal(tile(city).hasPipe, true);
  assert.equal(tile(city).hasPowerLine, false);
  assert.equal(tile(city).level, 2);
  assert.ok(undoConstruction(city, history).ok);
  assert.equal(tile(city).hasPipe, false);
  assert.equal(tile(city).level, 2);
  assert.ok(undoConstruction(city, history).ok);
  assert.equal(tile(city).kind, 'empty');
  assert.equal(tile(city).level, 0);
...
test('facility placement and demolition undo whole footprints with original utilities', () => {
  const city = flatCity(),
    history: ConstructionHistory = [];
  tile(city, 11, 10).kind = 'tree';
  tile(city, 11, 10).hasPipe = true;
  ...
  assert.equal(tile(city, 11, 10).kind, 'tree');
  assert.equal(tile(city, 11, 10).hasPipe, true);
  assert.equal(tile(city).kind, 'empty');
  assert.equal(city.month, 7);
});

test('terrain undo restores elevation and water without touching current unrelated state', () => {
  const city = flatCity(),
    history: ConstructionHistory = [];
  tile(city).elevation = 0;
  tile(city).kind = 'tree';
  tile(city).hasPipe = true;
  ...
  assert.ok(result.ok);
  assert.equal(tile(city).elevation, 0);
  assert.equal(tile(city).kind, 'tree');
  assert.equal(tile(city).hasPipe, true);
  assert.equal(city.month, 4);
  assert.equal(city.money, money - result.cost);
  assert.equal(city.progression.counters.terrainTiles, 0);
});

for (const change of ['fire', 'rubble', 'utility', 'elevation', 'footprint'] as const)
  test(`conflicting ${change} change blocks the whole edit without refund or partial mutation`, () => {
    const city = flatCity(),
      history: ConstructionHistory = [];
...
test('construction XP and counters reverse without erasing newer monthly progress', () => {
  const city = flatCity(),
    history: ConstructionHistory = [];
  city.progression.xp = 300;
  city.progression.counters.built_road = 2;
  city.progression.counters.totalBuilt = 7;
  construct(city, history, 'road', line(3));
  city.progression.xp += 200;
  Object.assign(city.progression.counters, {
    profitableMonths: 4,
    lastProgressMonth: 8,
    loansTaken: 2,
  });
  assert.ok(undoConstruction(city, history).ok);
  assert.equal(city.progression.xp, 500);
  assert.equal(city.progression.counters.built_road, 2);
  assert.equal(city.progression.counters.totalBuilt, 7);
  assert.equal(city.progression.counters.profitableMonths, 4);
  assert.equal(city.progression.counters.lastProgressMonth, 8);
  assert.equal(city.progression.counters.loansTaken, 2);
});
...
test('history keeps ten edits and failed builds do not consume an undo entry', () => {
  ...
  const before = captureConstructionState(city),
    failure = build(city, line(1), 'road');
  ...
  assert.ok(
    line(10)
      .slice(1)
      .every((p) => tile(city, p.x, p.z).kind === 'empty'),
  );
  assert.equal(undoConstruction(city, history).ok, false);
});
...
test('demolishing a burning building does not record a fire-resurrecting undo', () => {
  ...
  tile(city).level = 2;
  const before = captureConstructionState(city),
    result = build(city, line(1), 'bulldoze');
  assert.ok(result.ok);
