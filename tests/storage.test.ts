...
import { createCityStorage, FALLBACK_CITY_KEY, LEGACY_CITY_KEY, type CityStorageDatabase } from '../src/storage.ts';
...
function memoryDatabase() {
  ...
  const database: CityStorageDatabase = {
    ...
    writeIfAbsent: async (key, value) => { if (!values.has(key)) values.set(key, structuredClone(value)); },
  };
...
test('a 4 MiB city is stored in IndexedDB without touching the quota-limited local store', async () => {
  ...
  let localWrites = 0;
  const storage = createCityStorage({ database, localStorage: () => ({ getItem: () => null, setItem: () => { localWrites++; throw new Error('QuotaExceededError'); } }) });
...
test('legacy saves are read unchanged and the first migration backup is preserved', async () => {
  const legacy = '{"version":1,"name":"Original"}', current = '{"version":2,"name":"Updated"}';
...
test('a newer fallback remains authoritative after IndexedDB recovers', async () => {
  ...
  await createCityStorage({ database, localStorage: () => local, now: () => 100 }).saveCityRaw('old database city');
  const brokenDatabase: CityStorageDatabase = { ...database, write: async () => { throw new Error('unavailable'); } };
  await createCityStorage({ database: brokenDatabase, localStorage: () => local, now: () => 200 }).saveCityRaw('new fallback city');
  ...
  assert.deepEqual(await recovered.loadSavedCity(), { raw: 'latest database city', source: 'indexeddb' });
});
...
test('concurrent saves commit in invocation order and a failed save does not poison the queue', async () => {
  ...
  let active = 0;
  let peakActive = 0;
  const delayed: CityStorageDatabase = { ...database, write: async (key, value) => {
    active++; peakActive = Math.max(peakActive, active); writes++;
    if (writes === 1) await new Promise<void>(resolve => { release = resolve; });
    await database.write(key, value);
    active--;
  } };
  const storage = createCityStorage({ database: delayed, localStorage: () => local, now: () => 100 });
  ...
  let fail = true;
  const recovering = createCityStorage({ database: { ...database, write: async (key, value) => {
...
test('corrupt current data is returned for validation instead of replaced by a legacy save', async () => {
  ...
  assert.deepEqual(await storage.loadSavedCity(), { raw: 'broken newest fallback', source: 'legacy' });
  await storage.saveCityRaw('explicitly imported replacement');
  assert.deepEqual(await storage.loadSavedCity(), { raw: 'explicitly imported replacement', source: 'indexeddb' });
...
test('saving in one tab preserves a different fallback written meanwhile in another tab', async () => {
  const local = memoryLocal();
  const { database } = memoryDatabase();
  await createCityStorage({ database: null, localStorage: () => local, now: () => 100 }).saveCityRaw('previous fallback');
  const concurrent: CityStorageDatabase = { ...database, write: async (key, value) => {
    await database.write(key, value);
    await createCityStorage({ database: null, localStorage: () => local, now: () => 300 }).saveCityRaw('other tab latest');
  } };
  await createCityStorage({ database: concurrent, localStorage: () => local, now: () => 200 }).saveCityRaw('this tab city');
  assert.deepEqual(await createCityStorage({ database, localStorage: () => local }).loadSavedCity(), { raw: 'other tab latest', source: 'legacy' });
});
...
test('an unreadable local fallback is reported even when the primary database is readable', async () => {
  ...
  const result = await createCityStorage({ database, localStorage: () => { throw new Error('Denied'); } }).loadSavedCity();
...
test('quota exhaustion rejects with an actionable export instruction and leaves existing data intact', async () => {
  ...
  const storage = createCityStorage({ database: null, localStorage: () => ({ getItem: local.getItem, setItem: () => { throw new DOMException('Full', 'QuotaExceededError'); } }) });
  await assert.rejects(storage.saveCityRaw('new save'), /Exportiere deine Stadt im Stadtmenü als Datei/);
...
test('storage denial never breaks preferences and failed reads are explicitly reported', async () => {
  const storage = createCityStorage({ database: null, localStorage: () => { throw new DOMException('Denied', 'SecurityError'); } });
...
test('a database read failure does not disguise an older local save as an unqualified successful load', async () => {
  ...
  const { database } = memoryDatabase();
  const storage = createCityStorage({ database: { ...database, read: async () => { throw new Error('read failed'); } }, localStorage: () => local });
  const result = await storage.loadSavedCity();
