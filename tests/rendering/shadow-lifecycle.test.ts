...
test('actual Three PCF array fallback requires a populated comparison shadow map', () => {
  ...
  const gl = {
    ...
    getActiveUniform: () => ({ name: 'directionalShadowMap[0]', size: 1, type: 0x8b62 }),
    getUniformLocation: () => ({}),
    uniform1iv: () => {},
  };
  const uniforms = new WebGLUniforms(
    gl as unknown as WebGLRenderingContext,
    {} as never,
  ) as unknown as {
    setValue: (gl: unknown, name: string, value: unknown, textures: unknown) => void;
  };
  const textures = {
    allocateTextureUnit: () => 0,
    setTexture2D: (texture: THREE.Texture) => sampled.push(texture),
  };
  ...
  assert.equal(
    fallback.compareFunction,
    null,
    'The installed array fallback cannot satisfy sampler2DShadow',
  );
});
...
test('shadow resize invalidates reflection readiness until a valid PCF comparison target is restored', () => {
  ...
  const first = new THREE.WebGLRenderTarget(2048, 2048);
  first.depthTexture = new THREE.DepthTexture(2048, 2048);
  first.depthTexture.compareFunction = THREE.LessEqualCompare;
  sun.shadow.map = first;
  assert.equal(reflectionShadowsReady(renderer, scene), true);
  first.dispose();
  sun.shadow.map = null;
  sun.shadow.mapSize.set(4096, 4096);
  assert.equal(reflectionShadowsReady(renderer, scene), false);
  const second = new THREE.WebGLRenderTarget(4096, 4096);
  second.depthTexture = new THREE.DepthTexture(4096, 4096);
  sun.shadow.map = second;
  assert.equal(
    reflectionShadowsReady(renderer, scene),
    false,
    'A plain depth target still cannot satisfy the PCF comparison sampler',
  );
  ...
  assert.equal(
    reflectionShadowsReady(renderer, scene),
    true,
    'Reversed depth comparison is valid as well',
  );
  second.dispose();
});
...
test('sky filtering waits without rendering when quality-switch shadow maps are absent', () => {
  ...
  scene.add(sunlight, ambient, fill);
  let renders = 0;
  const renderer = {
    toneMappingExposure: 1,
    shadowMap: { enabled: true, type: THREE.PCFShadowMap, autoUpdate: true, needsUpdate: true },
