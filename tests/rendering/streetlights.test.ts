...
import {
  createStreetlights,
  getRoadStreetlightFixture,
  removeLegacyStreetlight,
} from '../../src/rendering/lighting/streetlights.ts';
...
function pointLights(helper: ReturnType<typeof createStreetlights>): THREE.PointLight[] {
  return helper.group.children.filter(
    (object): object is THREE.PointLight => object instanceof THREE.PointLight,
  );
}
...
test('legacy replacement removes only the three exact road-lamp children without disposing shared primitives', () => {
  const state = city(),
    tile = road(state, 10, 8),
    model = createTileModel(tile, state);
  const before = [...model.children],
    originals = before.slice(-3) as THREE.Mesh[];
  let disposalCount = 0;
  ...
  assert.equal(
    disposalCount,
    0,
    'Removed road details share geometry and materials with other models',
  );
  removeLegacyStreetlight(model, tile);
...
test('fixtures follow the existing road lattice while only powered, non-burning roads produce illumination', (t) => {
  const state = city();
  road(state, 10, 8);
  road(state, 16, 8, 0, false);
  road(state, 22, 8).fire = 3;
  road(state, 11, 8);
  const rail = road(state, 28, 8);
  rail.kind = 'rail';
  const helper = createStreetlights(state);
  ...
  assert.deepEqual(
    { count, litCount, pointLights, activePointLights },
    { count: 3, litCount: 1, pointLights: 0, activePointLights: 0 },
  );
  assert.equal(diagnostics.groundPools, 1);
  assert.equal(diagnostics.enabled, true);
  assert.equal(diagnostics.nightBlend, 1);
  assert.equal(diagnostics.positions.length, 3);
  assert.equal(diagnostics.points.length, 0);
  const fixture = getRoadStreetlightFixture(state, state.tiles[8 * state.size + 10])!;
  assert.deepEqual(diagnostics.positions[0], {
    id: 8 * state.size + 10,
    x: fixture.bulb.x,
    y: fixture.bulb.y,
    z: fixture.bulb.z,
...
test('day, night and switch-off keep every lamp in the field with strictly zero off emission', (t) => {
  ...
  assert.ok(material(mesh(helper, 'ground-pools')).opacity > 0.3);
  for (const light of identities)
    assert.ok(light.visible && !light.castShadow && light.distance <= 2 && light.decay === 2);
  helper.setLighting(1, false);
  assert.equal(helper.getDebug().litCount, 0);
  assert.equal(helper.getDebug().groundPools, 0);
  assert.equal(helper.getDebug().enabled, false);
...
test('camera movement and quality changes never reassign, disable or cap powered lamps', (t) => {
  ...
  t.after(() => helper.dispose());
  helper.setLighting(1, true);
  const before = helper.getDebug();
  assert.ok(before.litCount > 40);
  assert.equal(before.field.sources, before.litCount);
  assert.equal(before.groundPools, before.litCount);
  for (const quality of ['performance', 'balanced', 'ultra'] as const) {
    ...
    for (const target of [
      ...
      helper.animate(1, target);
      assert.deepEqual(helper.getDebug(), before);
    }
  }
});
...
test('power loss, fire and road removal refresh active light assignments without stale illumination', (t) => {
  const state = city(),
    first = road(state, 10, 8),
    second = road(state, 16, 8);
  const helper = createStreetlights(state);
  t.after(() => helper.dispose());
  helper.setLighting(1, true);
...
test('fixture bases and every pool triangle conform to the actual nonplanar road surface', (t) => {
  ...
  assert.ok(Math.abs(footing.y - sampleRoadHeight(state, footing.x, footing.z) - 0.039) < 0.00001);
  const bulb = instancePosition(mesh(helper, 'lit-lenses'), 0);
  assert.ok(Math.abs(bulb.y - sampleRoadHeight(state, bulb.x, bulb.z) - 0.49) < 0.00001);
  const pools = mesh(helper, 'ground-pools'),
  ...
  for (let instance = 0; instance < pools.count; instance++) {
    ...
    const corners = Array.from({ length: 3 }, (_, i) =>
      new THREE.Vector3().fromBufferAttribute(positions, i).applyMatrix4(matrix),
    );
    for (let u = 0; u <= 8; u++)
      for (let v = 0; v <= 8 - u; v++) {
        const point = corners[0]
          .clone()
          .multiplyScalar(1 - u / 8 - v / 8)
          .addScaledVector(corners[1], u / 8)
          .addScaledVector(corners[2], v / 8);
        assert.ok(
          Math.abs(point.y - sampleRoadHeight(state, point.x, point.z) - 0.0457) < 0.00001,
  ...
  const x = 10.5 - state.size / 2,
    z = 8.5 - state.size / 2;
  const hits = new THREE.Raycaster(
    new THREE.Vector3(x + 0.17, 20, z - 0.09),
    new THREE.Vector3(0, -1, 0),
  ).intersectObject(pools);
  assert.ok(hits.length > 0);
  assert.ok(
    Math.abs(hits[0].point.y - sampleRoadHeight(state, x + 0.17, z - 0.09) - 0.0457) < 0.00001,
  );
});
...
test('bridge lamps stay above the deck and neighboring terrain edits invalidate fixture heights', (t) => {
  ...
  assert.ok(
    Math.abs(initial.y - 0.49) < 0.00001,
    'Bridge lighting follows the deck, never the submerged seabed',
  );
  neighbor.kind = 'rail';
  neighbor.elevation = 3;
  helper.update(state);
  const updated = instancePosition(mesh(helper, 'lit-lenses'), 0);
  assert.ok(updated.y > initial.y + 0.5);
  assert.ok(Math.abs(updated.y - sampleRoadHeight(state, updated.x, updated.z) - 0.49) < 0.00001);
});
...
test('ground pools use a finite soft alpha texture, preserve scene depth and opt out of AO geometry', (t) => {
  ...
  t.after(() => helper.dispose());
  const pools = mesh(helper, 'ground-pools'),
    shader = material(pools),
    texture = shader.alphaMap;
  ...
  assert.equal(shader.depthTest, true);
...
test('collision fixture API shares the real rendered mast position and includes unpowered poles', (t) => {
  const state = city(),
    tile = road(state, 10, 8, 2, false);
  const helper = createStreetlights(state);
  t.after(() => helper.dispose());
  const fixture = getRoadStreetlightFixture(state, tile);
  assert.ok(fixture);
  assert.equal(fixture.mast.x, tile.x - state.size / 2 + 0.5 - 0.47);
  assert.equal(fixture.mast.z, tile.z - state.size / 2 + 0.5 + 0.47);
  const base = instancePosition(mesh(helper, 'bases'), 0);
  assert.ok(Math.abs(base.x - fixture.mast.x) < 0.00001);
  assert.ok(Math.abs(base.z - fixture.mast.z) < 0.00001);
  assert.ok(Math.abs(base.y - 0.014 - fixture.base.y) < 0.00001);
  const pole = instancePosition(mesh(helper, 'poles'), 0);
  assert.ok(Math.abs(pole.y - 0.234 - fixture.mast.y) < 0.00001);
  assert.equal(fixture.mastRadius, 0.014);
  assert.equal(fixture.mastHeight, 0.468);
  assert.equal(fixture.baseRadius, 0.036);
  assert.equal(fixture.baseHeight, 0.028);
...
test('unchanged updates reuse buffers and resized/disposed helpers release owned resources once', () => {
  ...
  const initialBatch = mesh(helper, 'poles'),
    initialMaterial = initialBatch.material;
  ...
  for (const resource of [...geometrySet, ...materialSet, texture])
  ...
  assert.equal(
    disposals.size,
    0,
    'Shared helper geometry/materials remain alive through instance resize',
  );
  helper.dispose();
  helper.dispose();
  helper.update(state);
