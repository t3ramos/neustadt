...
import { CityPathTracer, createPathTracingSnapshot } from '../../src/rendering/raytracing.ts';
...
function harness(plans: BuildPlan[] = []) {
  ...
  const controller = new CityPathTracer(
    renderer,
    source.scene,
  ...
  function makeBackend(plan: BuildPlan) {
    ...
    const worker = {
      ...
      dispose() {
        assert.equal(pending, false, 'Do not terminate a worker while it owns pending geometry');
        counts.worker++;
      },
    };
      ...
      async setSceneAsync(
        ...
        scene.traverse((object) => {
          if (object instanceof THREE.Mesh)
            object.geometry.addEventListener('dispose', () => counts.snapshotGeometry++);
        });
...
test('Cancel restores realtime immediately; pending worker cleanup precedes a rapid re-entry', async () => {
  ...
  try {
    ...
    assert.equal(h.renderer.debug.onShaderError, h.originalShaderError);
    assert.equal(
      h.backends[0].counts.worker,
      0,
      'Pending transferred data must finish before worker disposal',
    );
...
test('scene revision changes discard an in-flight build and publish only the latest coherent city', async () => {
  ...
  try {
    ...
    assert.equal(
      await entry,
      true,
      'A single entry waits for the latest revision rather than returning stale failure',
    );
    assert.equal(h.backends.length, 2);
    released(h.backends[0]);
...
test('Cancel interrupts large snapshot expansion at its next yield and frees every partial geometry', async (t) => {
  ...
  try {
    ...
    assert.ok(
      cloneDisposals.size <= 130,
      `Cancellation must not expand all 4096 instances (${cloneDisposals.size} clones)`,
    );
...
test('teardown releases actual dependency-owned GPU buffers once and preserves borrowed textures', async () => {
  ...
  const PhysicalMaterial = Reflect.get(
    library,
    'PhysicalPathTracingMaterial',
  ) as new () => THREE.ShaderMaterial;
  ...
  const mainBlend = new THREE.MeshBasicMaterial(),
    lowBlend = new THREE.MeshBasicMaterial();
  const counts = new Map<string, number>();
  const watch = (
    name: string,
    resource: { addEventListener(type: 'dispose', callback: () => void): void },
  ) => {
    counts.set(name, 0);
  ...
  for (const name of [
    ...
    watch(name, uniform(name));
  for (const name of ['index', 'position', 'bvhBounds', 'bvhContents'])
    watch(`bvh ${name}`, uniform('bvh')[name]);
  for (const name of ['marginalWeights', 'conditionalWeights', 'map'])
    watch(`environment ${name}`, uniform('envMapInfo')[name]);
  watch('lights', uniform('lights').tex);
  const framebufferOwners = new Map<THREE.Texture, { __renderTarget: THREE.WebGLRenderTarget }>();
  Reflect.set(h.renderer, 'properties', {
    get: (texture: THREE.Texture) => framebufferOwners.get(texture),
  });
  const uploadRenderer = {
    getRenderTarget: () => null,
    getClearAlpha: () => 0,
    getClearColor: (color: THREE.Color) => color.set(0),
    setClearColor() {},
  ...
  for (const name of ['textures', 'iesProfiles']) {
    ...
    assert.equal(
      uniform(name).renderTarget,
      null,
      'The array-target replacement loses the public back-reference',
    );
    const target = framebufferOwners.get(uniform(name))
      ?.__renderTarget as THREE.WebGLRenderTarget & { fsQuad: { material: THREE.Material } };
  ...
  try {
    ...
    Object.assign(h.backends[0].tracer, {
      ...
      _lowResPathTracer: {
        material,
        _blendQuad: { material: lowBlend },
        _compileFunction: onRecompile,
        dispose() {
          lowResolutionDisposals++;
        },
      },
      _colorBackground: colorBackground,
      _internalBackground: internalBackground,
    });
    assert.equal(await h.controller.setEnabled(false), true);
    h.controller.dispose();
    for (const [name, count] of counts)
      assert.equal(count, 1, `${name} must be released exactly once`);
    assert.equal(lowResolutionDisposals, 1);
    assert.equal(
      borrowedDisposals,
      0,
      'Background and Sobol references must not be swept as owned material textures',
    );
