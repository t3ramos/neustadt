...
import {
  ...
  getPathTracingSettings,
} from '../../src/rendering/raytracing.ts';

const meshesIn = (scene: THREE.Scene) =>
  scene.children.filter((object): object is THREE.Mesh => object instanceof THREE.Mesh);
...
test('snapshot expands instances into world space, including parent transforms and nonuniform scales', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      false,
      'Photo geometry must not retain unsupported instances',
    );
    ...
    for (let instance = 0; instance < 2; instance++) {
      for (let vertex = 0; vertex < 3; vertex++) {
        expected
          ...
          .applyMatrix4(instanceTransforms[instance])
...
test('snapshot multiplies instance tint by existing vertex colors without changing live buffers or material', async () => {
  ...
  try {
    ...
    for (let instance = 0; instance < 2; instance++) {
      for (let vertex = 0; vertex < 3; vertex++) {
        near(
          colors.getX(instance * 3 + vertex),
          liveColors[vertex * 3] * tints[instance].r,
          'Red tint',
        );
        near(
          colors.getY(instance * 3 + vertex),
          liveColors[vertex * 3 + 1] * tints[instance].g,
          'Green tint',
        );
        near(
          colors.getZ(instance * 3 + vertex),
          liveColors[vertex * 3 + 2] * tints[instance].b,
          'Blue tint',
        );
      }
    }
...
test('snapshot excludes hidden ancestors, excluded HUD subtrees, and empty instance batches', async () => {
  ...
  try {
    assert.equal(meshesIn(snapshot.scene).length, 1);
    near(
      meshesIn(snapshot.scene)[0].geometry.getAttribute('position').getX(0),
      7,
      'Visible building position',
    );
...
test('snapshot owns cloned geometry and deduplicated materials while preserving live resources on disposal', async () => {
  ...
  assert.equal(
    first.material,
    second.material,
    'A shared source material should have one snapshot owner',
  );
...
test('instanced material groups retain assignments and cover every expanded indexed triangle', async () => {
  ...
  try {
    ...
    assert.equal(
      mesh.geometry.groups.reduce((count, group) => count + group.count, 0),
...
test('mixed RGB terrain and uncolored surfaces keep RGBA colors through the real pathtracer geometry merge', async () => {
  ...
  grassGeometry.setAttribute(
    'color',
    new THREE.Float32BufferAttribute([1, 0.5, 0.25, 1, 0.5, 0.25, 1, 0.5, 0.25], 3),
  );
  ...
  try {
    ...
    for (let meshIndex = 0; meshIndex < meshes.length; meshIndex++) {
      ...
      assert.ok(
        colors,
        'Every snapshot mesh needs a consistent color attribute before upstream merging',
      );
      assert.equal(colors.itemSize, 4);
      for (let vertex = 0; vertex < colors.count; vertex++) {
        for (let component = 0; component < 4; component++) {
          near(
            colors.getComponent(vertex, component),
            expectedColors[meshIndex][component],
            'Snapshot RGBA',
          );
        }
      }
    ...
    for (let meshIndex = 0; meshIndex < meshes.length; meshIndex++) {
      const materialIndex = generated.materials.indexOf(
        meshes[meshIndex].material as THREE.Material,
      );
      ...
      for (let vertex = 0; vertex < mergedColors.count; vertex++) {
        ...
        for (let component = 0; component < 4; component++) {
          near(
            mergedColors.getComponent(vertex, component),
            expectedColors[meshIndex][component],
            'Merged RGBA',
          );
        }
    ...
    assert.equal(
      grassGeometry.getAttribute('color').itemSize,
      3,
      'The live RGB buffer is unchanged',
    );
    assert.equal(
      plainGeometry.getAttribute('color'),
      undefined,
      'The live uncolored surface is unchanged',
    );
  } finally {
...
test('every graphics profile supplies changing random samples for all BSDF dimensions and shadow traversals', async () => {
  ...
  const Material = Reflect.get(
    library,
    'PhysicalPathTracingMaterial',
  ) as new () => THREE.ShaderMaterial & { stratifiedTexture: Sampler };
  const material = new Material();
  try {
    const randomDimensions = [
      ...new Set(
        [...material.fragmentShader.matchAll(/\brand[234]?\(\s*(\d+)\s*\)/g)].map((match) =>
          Number(match[1]),
        ),
      ),
    ];
    assert.ok(
      randomDimensions.length > 0,
      'Read actual random-coordinate use from the dependency shader',
    );
    assert.ok(
      Math.max(...randomDimensions) >= 16,
      'Include the volume and BSDF sampling dimensions',
    );
    ...
    for (const quality of ['performance', 'balanced', 'ultra'] as const) {
      ...
      for (const dimension of randomDimensions) {
        assert.ok(
          dimension < width,
          `${quality}: random dimension ${dimension} must fit texture width ${width}`,
        );
      }
      ...
      assert.ok(
        lastShadowRow < height,
        `${quality}: shadow row ${lastShadowRow} must fit texture height ${height}`,
      );
          ...
          for (const dimension of randomDimensions) {
            for (let component = 0; component < 4; component++) {
              ...
              assert.ok(
                ...
                `${quality}: valid sample at (${dimension}, ${row}, ${component})`,
              );
              samples.push(value);
            }
          }
      ...
      assert.notDeepEqual(
        readAddressedSamples(),
        before,
        `${quality}: advancing samples must not repeat a static noise pattern`,
      );
    }
  } finally {
