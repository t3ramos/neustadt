...
import { createPowerGridModel, powerServiceModelPlacement, powerWirePoints } from '../src/power-model.ts';
...
import { createFacilityServiceVehicle } from '../src/models.ts';
import { isPowerServiceDropClear } from '../src/power-service.ts';
import type { TileKind } from '../src/types.ts';
...
test('facility meters stay on the inner plinth and raised service wires clear every private access lane', () => {
  ...
  assert.ok(vehicleHeight > .30 && vehicleHeight < .40);
  const kinds = Object.entries(TOOL_DEFS).filter(([, definition]) => definition?.footprint).map(([kind]) => kind as TileKind);
  for (const kind of kinds) for (const rotation of [0, 1, 2, 3] as const) for (const elevation of [0, .5, 2]) {
    const state = createCity(19, true, 40);
    for (const tile of state.tiles) Object.assign(tile, {kind:'empty',elevation:0,level:0,anchor:-1,hasPowerLine:false,hasPipe:false});
    const x = 12, z = 12, id = z * state.size + x;
    const nominal = TOOL_DEFS[kind]!.footprint!, [width, depth] = rotation % 2 ? [nominal[1], nominal[0]] : nominal;
    for (let dz = 0; dz < depth; dz++) for (let dx = 0; dx < width; dx++) {
      Object.assign(state.tiles[(z + dz) * state.size + x + dx], {kind,level:1,anchor:id,rotation,elevation});
    }
    ...
    for (let dz = 0; dz < 4; dz++) for (let dx = 0; dx < 4; dx++) {
      Object.assign(state.tiles[(z + dz) * state.size + x - 5 + dx], {kind:'power',level:1,anchor:sourceId,rotation:0,elevation});
    }
    state.tiles[(z + 1) * state.size + x - 1].hasPowerLine = true;
    state.tiles[(z + 1) * state.size + x - 1].elevation = elevation;
    for (let dx = 0; dx < width; dx++) state.tiles[(z + depth) * state.size + x + dx].kind = 'road';
    recalculate(state);
    const before = JSON.stringify(state), layout = getPowerLayout(state);
    const service = layout.services.find(service => service.building === id)!;
    assert.ok(service, `${kind} rotation ${rotation} needs its existing large-facility service`);
    const powerPole = layout.poles.find(pole => pole.id === service.pole)!;
    const offset = state.tiles[powerPole.id].kind === 'road' ? .86 : .5;
    const pole = new THREE.Vector3(powerPole.x - state.size / 2 + offset, powerPole.elevation, powerPole.z - state.size / 2 + offset);
    const placement = powerServiceModelPlacement(state, service, pole), plan = getFacilityAccess(state, state.tiles[id])!;
    assert.equal(plan.connected, true);
    assert.equal(placement.driveway, true);
    assert.equal(placement.overhead, true, `${kind} rotation ${rotation} retains its visible service cable`);
    const cabinet = placement.cabinet;
    assert.ok(Math.abs(cabinet.y - plan.baseY - .052) < 1e-9, 'The footing rests on the new visible plinth');
    const source = ['power', 'wind', 'solar'].includes(kind);
    const radius = Math.hypot((source ? .20 : .105) + .055, (source ? .16 : .07) + .055) / 2;
    for (const dx of [-radius, radius]) for (const dz of [-radius, radius]) {
      assert.equal(sampleFacilityAccessHeight(plan, cabinet.x + dx, cabinet.z + dz), null,
        `${kind} rotation ${rotation} cannot put any corner of its meter footing in the driveway`);
    }
    const footprintIds = new Set(getFootprint(state, state.tiles[id]).map(point => point.z * state.size + point.x));
    assert.equal(isPowerServiceDropClear(state, state.tiles[service.pole], [cabinet.x + state.size / 2, cabinet.z + state.size / 2], footprintIds,
      tile => !!TOOL_DEFS[tile.kind]?.footprint || ['residential','commercial','industrial'].includes(tile.kind) && tile.level > 0), true,
    'Moving the visible cabinet cannot introduce a public-road crossing');
    for (const point of powerWirePoints(placement.cableStart, placement.contact, .055, 256)) {
      const surface = sampleFacilityAccessHeight(plan, point.x, point.z);
      if (surface === null) continue;
      assert.ok(point.y - .007 >= surface + vehicleHeight + .10,
        `${kind} rotation ${rotation} at elevation ${elevation} leaves insufficient vehicle clearance`);
    }
    assert.equal(JSON.stringify(state), before, 'Render placement cannot change block supply or saved infrastructure');
  }
});

test('an inset facility feed stays clear of a raised or lowered entrance ramp from a real street curb', () => {
  let slopedSamples = 0;
  for (const rotation of [0, 1, 2, 3] as const) for (const [facilityHeight, streetHeight] of [[0, 2], [2, 0], [.5, 0], [0, .5]]) {
    const state = createCity(21, true, 40);
    for (const tile of state.tiles) Object.assign(tile, {kind:'empty',elevation:0,level:0,anchor:-1,hasPowerLine:false,hasPipe:false});
    const id = 12 * 40 + 12;
    for (let z = 12; z <= 13; z++) for (let x = 12; x <= 13; x++) Object.assign(state.tiles[z * 40 + x], {kind:'police',level:1,anchor:id,rotation,elevation:facilityHeight});
    for (let z = 8; z <= 11; z++) for (let x = 2; x <= 5; x++) Object.assign(state.tiles[z * 40 + x], {kind:'power',level:1,anchor:8*40+2,rotation:0,elevation:streetHeight});
    for (let x = 6; x <= 17; x++) Object.assign(state.tiles[11 * 40 + x], {kind:'road',hasPowerLine:true,elevation:streetHeight});
    recalculate(state);
    const layout = getPowerLayout(state), service = layout.services.find(service => service.building === id)!;
    assert.ok(service);
    const pole = layout.poles.find(pole => pole.id === service.pole)!;
    assert.equal(state.tiles[pole.id].kind, 'road');
    const placement = powerServiceModelPlacement(state, service, new THREE.Vector3(pole.x - 20 + .86, pole.elevation, pole.z - 20 + .86));
    assert.equal(placement.overhead, true);
    const plan = getFacilityAccess(state, state.tiles[id])!;
    for (const point of powerWirePoints(placement.cableStart, placement.contact, .055, 512)) {
      const surface = sampleFacilityAccessHeight(plan, point.x, point.z);
      if (surface === null) continue;
      if (Math.abs(surface - plan.baseY - .052) > .01) slopedSamples++;
      assert.ok(point.y - .007 >= surface + .434,
