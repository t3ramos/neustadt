...
export function interiorRoadNetwork(points: [number, number][]): RoadNetwork {
  ...
  const extended = new Map(original),
    directions = [
      [0, -1],
      [1, 0],
      [0, 1],
  ...
  for (const p of original.values()) {
    const neighbors = directions
      ...
      .filter((n) => original.has(roadPointKey(n)));
    if (neighbors.length !== 1) continue;
    const dx = p.x - neighbors[0].x,
      dz = p.z - neighbors[0].z;
    for (let i = 1; i <= 2; i++) {
      const next = { x: p.x + dx * i, z: p.z + dz * i };
      extended.set(roadPointKey(next), next);
    }
  }
  const network = buildRoadNetwork({
    tiles: [...extended.values()].map((p) => ({ ...p, kind: 'road' }) as Tile),
  });
  network.junctions = network.junctions.filter((j) =>
    j.cells.some((p) => original.has(roadPointKey(p))),
  );
