...
import { CitizenDialogue } from '../../src/citizens/system.ts';
import { createCitizenSpeechSelector } from '../../src/citizens/dialogue/catalog.ts';
import {
...
import {
  ...
  refreshVehicleLabel,
  type VehicleKind,
} from '../../src/rendering/vehicles/model.ts';
import { createCity } from '../../src/simulation/city-simulation.ts';
import { createCityStorage, type CityStorageDatabase } from '../../src/persistence/storage.ts';
import { setLocale } from '../../src/i18n/index.ts';

function roadCity() {
  const state = createCity(917, true, 40);
  for (const tile of state.tiles)
    Object.assign(tile, { kind: 'road', elevation: 0, level: 0, anchor: -1 });
  return state;
}
...
function drivable(state: ReturnType<typeof roadCity>, kind: VehicleKind = 'sedan'): DrivableCar {
  ...
  model.position.set(0.5, drivingSurfaceHeight(state, 0.5, -1), -1);
  return {
    model,
    from: { x: 20, z: 19 },
    to: { x: 20, z: 20 },
...
test('active NPC speech changes language without replacing its selection, lifetime, or cooldown', (t) => {
  t.after(() => setLocale('de'));
  setLocale('de');
  const selector = createCitizenSpeechSelector(59),
    dialogue = new CitizenDialogue();
...
test('live vehicle labels switch both ways while preserving the fleet and active driving state', (t) => {
  ...
  const state = roadCity(),
    fleet = (['sedan', 'taxi', 'van', 'truck'] as const).map((kind) => drivable(state, kind));
  fleet.forEach((car, i) => (car.model.position.x = i * 2));
  const statuses: DrivingStatus[] = [],
    system = createDrivingController(state, () => fleet, {
      onStatus: (value) => statuses.push(value),
    });
  t.after(() => system.dispose());
  system.enter(fleet[2].model.id);
  system.keyDown('KeyW');
  system.update(0.1);
  const status = system.getStatus(),
    camera = system.camera.matrixWorld.clone();
  const cars = fleet.map((car) => ({
    id: car.model.id,
    position: car.model.position.clone(),
    rotation: car.model.rotation.clone(),
    geometry: car.model.children[0],
  }));
  assert.equal(status.label, 'Transporter');
  setLocale('en');
  system.refreshLocale();
  ...
  assert.deepEqual(
    fleet.map((car) => car.model.userData.vehicleLabel),
    ['Sedan', 'Taxi', 'Van', 'Truck'],
  );
  fleet.forEach((car, i) => {
    assert.equal(car.model.id, cars[i].id);
    assert.deepEqual(car.model.position, cars[i].position);
    assert.ok(car.model.rotation.equals(cars[i].rotation));
    assert.equal(car.model.children[0], cars[i].geometry);
  });
  setLocale('de');
  system.refreshLocale();
  assert.deepEqual(
    fleet.map((car) => car.model.userData.vehicleLabel),
    ['Limousine', 'Taxi', 'Transporter', 'Lastwagen'],
  );
...
test('an existing collision message re-translates immediately without rerunning physics', (t) => {
  ...
  const state = roadCity(),
    car = drivable(state),
    system = createDrivingController(state, () => [car]);
  t.after(() => system.dispose());
  ...
  system.enter(car.model.id);
  system.keyDown('KeyW');
  for (let step = 0; step < 720; step++) system.update(1 / 120);
  const before = system.getStatus(),
    position = car.model.position.clone();
  ...
  assert.deepEqual(system.getStatus(), {
    ...before,
    label: 'Sedan',
    blocked: 'Building in the way',
  });
  assert.deepEqual(car.model.position, position);
  setLocale('de');
  system.refreshLocale();
  assert.deepEqual(system.getStatus(), before);
});
...
test('untyped fallback cars receive translated driving labels and keep custom labels', (t) => {
  ...
  const state = roadCity(),
    car = drivable(state);
  delete car.model.userData.vehicleKind;
  delete car.model.userData.vehicleLabel;
  const system = createDrivingController(state, () => [car]);
  t.after(() => system.dispose());
  system.enter(car.model.id);
  assert.equal(system.getStatus().label, 'City car 01');
  setLocale('de');
  system.refreshLocale();
  assert.equal(system.getStatus().label, 'Stadtauto 01');
  car.model.userData.vehicleLabel = 'Custom model';
...
test('storage recovery guidance is localized for load, save, and backup failures', async (t) => {
  ...
  assert.match(
    (await storage.loadSavedCity()).error ?? '',
    /^Browser storage could not be fully read\./,
  );
  await assert.rejects(storage.saveCityRaw('city'), /^Error: Your city could not be saved\./);
  await assert.rejects(
    storage.backupLegacy('legacy'),
    /^Error: The additional backup could not be created\./,
  );
  setLocale('de');
