...
import {
  TOOL_DEFS, applyCitizenIncident, build, createCity, deserializeCity, expandCity, getFootprint,
...
const unlock = (city: CityState, ...tools: Tool[]) => {
  for (const tool of tools) if (!city.progression.unlocked.includes(tool)) city.progression.unlocked.push(tool);
};
...
const district = (zones = true): CityState => {
  ...
  if (zones) {
    for (const [x, kind, level] of [[15, 'residential', 3], [17, 'commercial', 1], [19, 'industrial', 1]] as const) {
...
test('default generation is deterministic, ten times the old area, and starts with a viable city', () => {
  const first = createCity(7813), second = createCity(7813);
  ...
  assert.ok(new Set(first.tiles.map(tile => tile.elevation)).size > 4, 'Terrain needs actual elevation differences');
  ...
  assert.ok(first.tiles.some(tile => tile.kind === 'residential' && tile.connected && tile.powered && tile.watered));
});
...
test('supported map sizes contain exactly one correctly indexed tile per coordinate', () => {
  for (const size of [40, 64, 96, 128]) {
    ...
    city.tiles.forEach((tile, i) => {
      assert.equal(tile.x, i % size); assert.equal(tile.z, Math.floor(i / size));
...
test('construction deduplicates painted tiles and never charges twice for an existing road', () => {
  const city = flatCity(), point = { x: 10, z: 10 }, money = city.money;
...
test('preview gives the exact build price and footprint without mutating any city state', () => {
  ...
  const points = [{ x: 10, z: 10 }, { x: 25, z: 25 }];
  ...
  assert.equal(tileAt(city, 25, 25).kind, 'empty', 'A landmark is a single placement, never a paintbrush');
});

test('large civic buildings occupy one atomic footprint with one charge and one operating bill', () => {
  for (const [kind, area] of [['power', 16], ['police', 4], ['fire', 6], ['hospital', 9], ['school', 6], ['university', 20], ['stadium', 30], ['airport', 60]] as const) {
    const city = flatCity();
    ...
    for (const member of members) assert.deepEqual(getFootprint(city, member), getFootprint(city, anchor));
...
test('a landmark rotates its footprint and cannot overlap existing construction or the map boundary', () => {
  ...
  for (const points of [[{ x: 10, z: 10 }], [{ x: 36, z: 35 }]]) {
    ...
    assert.equal(JSON.stringify(city), before, 'Rejected atomic placement must leave every field untouched');
  }
...
test('demolishing any occupied landmark cell removes the complete building and charges once', () => {
  ...
  const result = construct(city, [{ x: 15, z: 14 }, { x: 10, z: 10 }], 'bulldoze');
  assert.equal(city.tiles.some(tile => tile.kind === 'stadium'), false);
  assert.equal(city.money, money - result.cost);
  ...
  for (const point of line(10, 10, 15, 14)) {
    ...
    assert.equal(tile.kind, 'empty'); assert.equal(tile.anchor, -1);
  }
});
...
test('unaffordable, out-of-map and underwater construction does not spend money or change land', () => {
  ...
  assert.equal(JSON.stringify(city), before);
  assert.equal(build(city, [{ x: -1, z: 10 }, { x: city.size, z: 10 }], 'road').ok, false);
  assert.equal(JSON.stringify(city), before);
...
test('roads carry traffic access while independent power and pipe networks deliver utilities', () => {
  ...
  const connected = tileAt(city, 15, 9), isolated = tileAt(city, 25, 22);
  assert.equal(connected.connected, true); assert.equal(connected.powered, true); assert.equal(connected.watered, true);
  assert.equal(isolated.connected, false); assert.equal(isolated.powered, false); assert.equal(isolated.watered, false);
...
test('a road without a utility layer does not magically supply a distant building', () => {
  const city = district(false);
  for (const tile of city.tiles) { tile.hasPipe = false; tile.hasPowerLine = false; }
...
test('pipes and power lines coexist with streets and buildings without replacing their surface', () => {
  ...
  const points = [{ x: 10, z: 10 }, { x: 11, z: 10 }];
  construct(city, points, 'pipe'); construct(city, points, 'powerline');
  assert.equal(tileAt(city, 10, 10).kind, 'road'); assert.equal(tileAt(city, 11, 10).kind, 'residential');
  for (const point of points) {
    const tile = tileAt(city, point.x, point.z);
    assert.equal(tile.hasPipe, true); assert.equal(tile.hasPowerLine, true);
  }
  const money = city.money;
...
test('severing water supply leaves electricity intact; severing electricity shuts down the water pump', () => {
  ...
  recalculate(city);
  assert.equal(home.watered, false); assert.equal(home.powered, true);
...
test('bulldozing the sole utility route disconnects service beyond the gap and clears both layers', () => {
  const city = district();
  const home = tileAt(city, 15, 9);
  assert.equal(home.powered, true); assert.equal(home.watered, true);
  construct(city, { x: 11, z: 8 }, 'bulldoze');
  const gap = tileAt(city, 11, 8);
  assert.equal(gap.hasPipe, false); assert.equal(gap.hasPowerLine, false);
  assert.equal(home.powered, false); assert.equal(home.watered, false);
});
...
test('road and railway bridges preserve their water elevation across save and demolition', () => {
  for (const tool of ['road', 'rail'] as const) {
    ...
    unlock(city, tool);
    const water = city.tiles.find(tile => tile.kind === 'water')!;
    const point = { x: water.x, z: water.z }, elevation = water.elevation, money = city.money;
...
test('bridge rubble retains water rules and can be rebuilt into a loadable bridge', () => {
  const city = createCity(55, true, 40), water = city.tiles.find(tile => tile.kind === 'water')!;
  construct(city, water, 'road'); water.kind = 'rubble'; recalculate(city);
  const money = city.money;
  assert.equal(build(city, [water], 'residential').ok, false); assert.equal(city.money, money);
...
test('service funding changes operating costs and resident outcomes and can be restored', () => {
  const city = createCity(438, false, 40), funded = { ...city.stats };
  city.funding = { police: 0, fire: 0, health: 0, education: 0 }; recalculate(city);
  assert.ok(city.stats.expenses < funded.expenses);
  assert.ok(city.stats.education < funded.education);
  assert.ok(city.stats.health < funded.health); assert.ok(city.stats.safety < funded.safety);
  city.funding = { police: 100, fire: 100, health: 100, education: 100 }; recalculate(city);
...
test('a burning power station supplies no electricity and an unpowered pump supplies no water', () => {
  const city = district();
  assert.ok(city.stats.powerSupply > 0); assert.ok(city.stats.waterSupply > 0);
  tileAt(city, 3, 3).fire = 6; recalculate(city);
  assert.equal(city.stats.powerSupply, 0);
  assert.equal(tileAt(city, 10, 3).powered, false);
  assert.equal(city.stats.waterSupply, 0);
  assert.equal(city.tiles.some(tile => tile.watered), false);
});

test('an overloaded grid can leave its pump without electricity and therefore supplies no water', () => {
  const city = district();
  for (const tile of city.tiles) if (tile.kind === 'empty') { tile.kind = 'industrial'; tile.level = 3; }
  recalculate(city);
...
test('save and load preserve deterministic growth and disaster continuation including v2 fields', () => {
  ...
  city.name = 'Hafenstadt'; city.tax = 11; city.settings.disastersEnabled = true; city.settings.weather = 'rain';
  for (let i = 0; i < 8; i++) tick(city);
  const loaded = deserializeCity(serializeCity(city));
  for (const key of ['name', 'seed', 'month', 'money', 'tax', 'revision'] as const) assert.equal(loaded[key], city[key]);
  assert.deepEqual(loaded.tiles, city.tiles); assert.deepEqual(loaded.settings, city.settings);
  assert.deepEqual(loaded.progression, city.progression);
  for (let i = 0; i < 12; i++) { tick(city); tick(loaded); }
  assert.deepEqual(loaded.tiles, city.tiles); assert.deepEqual(loaded.stats, city.stats); assert.equal(loaded.money, city.money);
  triggerDisaster(city, 'fire'); triggerDisaster(loaded, 'fire');
...
test('loading rejects corrupted files, invalid economy, terrain and broken building ownership', () => {
  ...
  const corruptions: ((raw: any) => void)[] = [
    raw => { raw.version = 999; }, raw => { raw.tiles.pop(); },
    raw => { raw.tiles[0].kind = 'unrecognized-building'; }, raw => { raw.money = 'unlimited'; },
    raw => { raw.tiles[0].x = -1; }, raw => { raw.tiles[0].elevation = 999; },
    raw => { raw.tiles[0].hasPipe = 'yes'; }, raw => { raw.tiles[0].anchor = raw.tiles.length + 1; },
    raw => { raw.settings.disastersEnabled = 'true'; },
  ];
  for (const corrupt of corruptions) {
    const raw = JSON.parse(saved); corrupt(raw);
...
test('loans enforce the credit limit and repayment cannot overdraw the treasury', () => {
  const city = flatCity(91), money = city.money;
  for (let i = 0; i < 5; i++) assert.ok(takeLoan(city).ok);
  assert.equal(city.loan, 50_000); assert.equal(city.money, money + 50_000);
  assert.equal(takeLoan(city).ok, false); assert.equal(city.money, money + 50_000);
  city.money = 9_999;
  assert.equal(repayLoan(city).ok, false); assert.equal(city.money, 9_999); assert.equal(city.loan, 50_000);
  city.money = 10_000;
  assert.ok(repayLoan(city).ok); assert.equal(city.money, 0); assert.equal(city.loan, 40_000);
  assert.equal(repayLoan(city).ok, false); assert.equal(city.money, 0);
});

test('monthly accounting charges interest and records the resulting treasury in history', () => {
  const city = flatCity(91), expenses = city.stats.expenses;
  assert.ok(takeLoan(city).ok); assert.equal(city.stats.expenses - expenses, 50);
  const money = city.money, balance = city.stats.balance;
  tick(city);
  assert.equal(city.month, 1); assert.equal(city.money, money + balance);
...
test('manual disasters are gated by the setting and each enabled disaster causes real damage', () => {
  for (const kind of ['fire', 'earthquake', 'storm'] as const) {
    ...
    assert.equal(JSON.stringify(city.tiles), before, `${kind} must not cause damage while disabled`);
    city.settings.disastersEnabled = true;
    triggerDisaster(city, kind);
    assert.notEqual(JSON.stringify(city.tiles), before, `${kind} must affect actual city tiles`);
    assert.ok(city.tiles.some(tile => tile.fire > 0 || tile.kind === 'rubble'), `${kind} needs visible damage`);
...
test('landmarks add jobs and demand only once and cease operation when their power plant is removed', () => {
  for (const [kind, jobs, demand] of [['stadium', 120, 'residentialDemand'], ['airport', 180, 'commercialDemand'], ['seaport', 120, 'industrialDemand']] as const) {
    const city = district();
    if (kind === 'seaport') for (const tile of city.tiles) if (tile.z >= 12) { tile.kind = 'water'; tile.elevation = -1; }
...
test('rail reduces traffic while parks and trees reduce pollution and improve local land value', () => {
  const city = district(), home = tileAt(city, 15, 9), traffic = home.traffic;
  construct(city, line(15, 10, 17, 10), 'rail'); assert.ok(home.traffic < traffic);
  const pollution = home.pollution, value = home.landValue;
  construct(city, { x: 15, z: 7 }, 'park');
  assert.ok(home.pollution < pollution); assert.ok(home.landValue > value);
  const greener = home.pollution;
  construct(city, [{ x: 16, z: 7 }, { x: 14, z: 7 }], 'tree');
...
test('funded operational fire stations suppress nearby fire but cannot protect themselves while burning', () => {
  ...
  const home = tileAt(protectedCity, 15, 9); home.fire = 6;
  tick(protectedCity);
  assert.ok(home.fire <= 2, 'Operational fire crew should rapidly reduce fire duration');
  tick(protectedCity); assert.equal(home.fire, 0); assert.equal(home.kind, 'residential');
  const burningCity = district(); construct(burningCity, { x: 12, z: 9 }, 'fire');
  const station = tileAt(burningCity, 12, 9); station.fire = 6;
  tick(burningCity);
...
test('fire cannot spread into a zoned lot before a building has developed', () => {
  ...
  construct(city, [{ x: 10, z: 10 }, { x: 11, z: 10 }], 'residential');
  tileAt(city, 10, 10).level = 1; tileAt(city, 10, 10).fire = 6;
  const vacant = tileAt(city, 11, 10);
  for (let month = 0; month < 6; month++) tick(city);
  assert.equal(vacant.kind, 'residential'); assert.equal(vacant.level, 0); assert.equal(vacant.fire, 0);
});
...
test('housing requires employment beyond the founding village', () => {
  ...
  assert.ok(city.stats.population > 0); assert.ok(city.stats.population <= 220, 'Housing alone cannot sustain unlimited growth');
  ...
  recalculate(city); assert.ok(city.stats.residentialDemand > demand);
});
...
test('a mixed district can grow from initial funds without loans and reach a sustainable budget', () => {
  ...
  construct(city, roads, 'road'); construct(city, roads, 'pipe'); construct(city, roads, 'powerline');
  for (let x = 11; x <= 27; x++) {
    construct(city, [{ x, z: 9 }, { x, z: 13 }], 'residential');
    construct(city, { x, z: 15 }, 'commercial'); construct(city, { x, z: 19 }, 'industrial');
  }
  let lowest = city.money;
  for (let month = 0; month < 72; month++) { tick(city); lowest = Math.min(lowest, city.money); }
  assert.ok(lowest > 0); assert.ok(city.stats.population >= 500); assert.ok(city.stats.jobs > 0);
  assert.ok(city.stats.balance > 0); assert.equal(city.loan, 0);
});
...
test('expanding a legacy-sized city preserves residents, economy, calendar and original coordinates', () => {
  ...
  const residents = city.tiles.filter(tile => tile.kind === 'residential').map(tile => ({ x: tile.x, z: tile.z, level: tile.level }));
  const saved = { money: city.money, month: city.month, seed: city.seed, population: city.stats.population };
  const larger = expandCity(city, 128);
  assert.equal(larger.size, 128); assert.equal(larger.tiles.length, 128 ** 2);
  assert.equal(larger.money, saved.money); assert.equal(larger.month, saved.month); assert.equal(larger.seed, saved.seed);
  assert.equal(larger.stats.population, saved.population);
  for (let i = 0; i < 40; i++) {
    assert.equal(tileAt(larger, i, 40).elevation, tileAt(larger, i, 39).elevation, 'The south edge continues at the saved height');
    assert.equal(tileAt(larger, 40, i).elevation, tileAt(larger, 39, i).elevation, 'The east coast continues without a boundary step');
  }
  for (const home of residents) { const tile = tileAt(larger, home.x, home.z); assert.equal(tile.kind, 'residential'); assert.equal(tile.level, home.level); }
...
test('version-one saves migrate single-cell facilities without destroying existing homes', () => {
  ...
  legacy.version = 1; delete legacy.progression; delete legacy.settings; delete legacy.citizenEffects;
  for (const tile of legacy.tiles) {
    delete tile.elevation; delete tile.hasPipe; delete tile.hasPowerLine; delete tile.anchor; delete tile.rotation;
  }
  for (const [x, z, kind, level] of [[15, 14, 'residential', 3], [16, 14, 'residential', 2], [19, 19, 'police', 1], [27, 27, 'power', 1], [29, 18, 'waterpump', 1]] as const) {
    Object.assign(legacy.tiles[z * 40 + x], { kind, level });
  }
  const loaded = deserializeCity(JSON.stringify(legacy));
  assert.equal(loaded.version, 2); assert.equal(loaded.size, 40); assert.equal(loaded.money, legacy.money);
  assert.deepEqual(loaded.citizenEffects, { populationLoss: 0, happinessPenalty: 0, incidents: [] });
  assert.equal(loaded.settings.dayNightCycle, true); assert.equal(loaded.settings.timeOfDay, 14);
  assert.equal(tileAt(loaded, 15, 14).kind, 'residential'); assert.equal(tileAt(loaded, 15, 14).level, 3);
  assert.equal(tileAt(loaded, 16, 14).kind, 'residential'); assert.equal(tileAt(loaded, 16, 14).level, 2);
  for (const kind of ['police', 'power', 'waterpump'] as const) {
    const anchors = loaded.tiles.filter(tile => tile.kind === kind && isBuildingAnchor(loaded, tile));
    assert.equal(anchors.length, 1);
    assert.equal(getFootprint(loaded, anchors[0]).length, 1, 'Import first preserves the old map exactly');
  }
  assert.doesNotThrow(() => deserializeCity(serializeCity(loaded)));
  const original = JSON.stringify(loaded), expanded = expandCity(loaded, 128);
  assert.equal(JSON.stringify(loaded), original, 'Expansion returns a new city rather than modifying the saved original');
  assert.equal(expanded.money, loaded.money); assert.equal(expanded.stats.population, loaded.stats.population);
  for (const [x, level] of [[15, 3], [16, 2]]) {
    assert.equal(tileAt(expanded, x, 14).kind, 'residential'); assert.equal(tileAt(expanded, x, 14).level, level);
  }
  for (const kind of ['police', 'power', 'waterpump'] as const) {
    const anchors = expanded.tiles.filter(tile => tile.kind === kind && isBuildingAnchor(expanded, tile));
    assert.equal(anchors.length, 1);
    assert.ok(getFootprint(expanded, anchors[0]).length > 1, 'Expansion upgrades the old facility into its full-sized building');
  }
...
test('one thousand simulated months remain finite, bounded and deterministically loadable', () => {
  ...
  const loaded = deserializeCity(serializeCity(city));
  assert.equal(loaded.month, 1000); assert.equal(loaded.history.length, 120); assert.ok(loaded.events.length <= 40);
  assert.equal(loaded.history.at(-1)!.month, 1000); assert.deepEqual(loaded.stats, city.stats);
  assert.ok(loaded.tiles.every(tile => tile.level >= 0 && tile.level <= 4 && Number.isFinite(tile.pollution)));
  assert.ok(Object.values(loaded.stats).every(Number.isFinite));
  tick(city); tick(loaded);
  assert.deepEqual(loaded.stats, city.stats); assert.equal(loaded.money, city.money);
});


const citizenIncident = (overrides: Partial<Omit<CitizenIncident, 'id'|'month'>> = {}): Omit<CitizenIncident, 'id'|'month'> => ({
  x: 15.25, y: 1.5, z: 9.75, nx: 0, ny: 1, nz: 0, witnessed: false, kind: 'impact', ...overrides,
});
...
test('each citizen incident removes exactly one resident without modifying building levels or double counting', () => {
  for (const kind of ['impact', 'abduction'] as const) {
    const city = district(), population = city.stats.population;
    const levels = city.tiles.map(tile => tile.level), month = city.month;
    ...
    assert.deepEqual(city.tiles.map(tile => tile.level), levels);
    assert.equal(city.month, month);
...
test('only witnessed incidents add two happiness penalty points, capped at thirty', () => {
  const witnessed = district(), hidden = district();
...
test('an empty city cannot lose citizens and incident losses cannot exceed its housing population', () => {
  const city = flatCity(), empty = JSON.stringify(city);
...
test('incident history keeps the most recent sixty-four while losses and IDs continue increasing', () => {
  const city = createCity(531, false, 40), population = city.stats.population;
  let lastId = -1;
  for (let i = 0; i < 80; i++) {
    assert.ok(applyCitizenIncident(city, citizenIncident()).ok);
...
test('citizen incidents reject malformed positions, surface normals and metadata without mutation', () => {
  ...
  const badInputs: Record<string, unknown>[] = [
    ...
    { x: -8.01 }, { x: city.size + 8.01 }, { z: -8.01 }, { z: city.size + 8.01 },
    { y: -16.01 }, { y: 128.01 }, { x: '15' }, { witnessed: 1 }, { witnessed: 'true' },
    { kind: 'unknown' }, { nx: 1.01 }, { ny: -1.01 }, { nz: Number.NaN },
    { nx: 0, ny: 0, nz: 0 },
  ];
  for (const bad of badInputs) {
    const before = JSON.stringify(city);
    const result = applyCitizenIncident(city, { ...citizenIncident(), ...bad } as Omit<CitizenIncident, 'id'|'month'>);
    assert.equal(result.ok, false, `Should reject ${JSON.stringify(bad)}`);
    assert.equal(JSON.stringify(city), before, 'Rejected physical incidents cannot change simulation state');
  }
  assert.ok(applyCitizenIncident(city, citizenIncident({ x: -8, y: -16, z: city.size + 8, nx: -1, ny: 0, nz: 1 })).ok);
  assert.ok(applyCitizenIncident(city, citizenIncident({ x: city.size + 8, y: 128, z: -8, nx: 1, ny: 0, nz: 0 })).ok);
});
...
test('save validation rejects malformed citizen effects and invalid stored incident data', () => {
  ...
  const corruptions: ((raw: any) => void)[] = [
    raw => { raw.citizenEffects.populationLoss = -1; },
    raw => { raw.citizenEffects.populationLoss = 0.5; },
    raw => { raw.citizenEffects.happinessPenalty = 31; },
    raw => { raw.citizenEffects.incidents[0].x = raw.size + 9; },
    raw => { raw.citizenEffects.incidents[0].y = 129; },
    raw => { raw.citizenEffects.incidents[0].month = raw.month + 1; },
    raw => { raw.citizenEffects.incidents[0].witnessed = 'true'; },
    raw => { raw.citizenEffects.incidents[0].kind = 'unknown'; },
    raw => { raw.citizenEffects.incidents[0].nx = 0; raw.citizenEffects.incidents[0].ny = 0; raw.citizenEffects.incidents[0].nz = 0; },
    raw => { raw.citizenEffects.incidents = Array.from({ length: 65 }, (_, i) => ({ ...raw.citizenEffects.incidents[0], id: i + 1 })); },
  ];
  for (const corrupt of corruptions) {
    const raw = JSON.parse(saved); corrupt(raw);
    assert.throws(() => deserializeCity(JSON.stringify(raw)), `Should reject ${corrupt.toString()}`);
  }
});

test('expansion preserves citizen impact grid coordinates, losses and penalties exactly once', () => {
  const city = district();
  assert.ok(applyCitizenIncident(city, citizenIncident({ x: 12.25, y: 3.125, z: 19.875, witnessed: true })).ok);
  const effects = structuredClone(city.citizenEffects), population = city.stats.population;
  const expanded = expandCity(city, 128);
  assert.deepEqual(expanded.citizenEffects, effects, 'Incident coordinates are grid coordinates and do not shift with world origin');
...
test('fresh starter cities begin at stage zero with every zone at building level two or below', () => {
  for (const size of [40, 128]) {
    ...
    const zones = city.tiles.filter(tile => ['residential', 'commercial', 'industrial'].includes(tile.kind));
...
test('stage-zero growth never develops a zone beyond building level two', () => {
  const city = district();
  for (const tile of city.tiles) if (['residential', 'commercial', 'industrial'].includes(tile.kind)) tile.level = 2;
  city.progression.rank = 0;
  recalculate(city);
  for (let month = 0; month < 120; month++) {
    tick(city);
    assert.equal(city.progression.rank, 0);
    assert.ok(city.tiles.every(tile => !['residential', 'commercial', 'industrial'].includes(tile.kind) || tile.level <= 2));
  }
});

test('higher city stages permit every zone type to grow to levels three and four respectively', () => {
  for (const [rank, initial, maximum] of [[1, 2, 3], [2, 3, 4]]) {
    const city = district();
    ...
    for (let month = 0; month < 80; month++) {
      tick(city);
      tracked.forEach((tile, i) => { peaks[i] = Math.max(peaks[i], tile.level); assert.ok(tile.level <= maximum); });
    }
    tracked.forEach((tile, i) => assert.equal(peaks[i], maximum, `Stage ${rank} must permit ${tile.kind} growth to level ${maximum}`));
  }
});
...
test('day-night settings default for new and older cities and persist through save and expansion', () => {
  const city = createCity(602, false, 40);
  assert.equal(city.settings.dayNightCycle, true); assert.equal(city.settings.timeOfDay, 14);
  const previousVersion = JSON.parse(serializeCity(city));
  delete previousVersion.settings.dayNightCycle; delete previousVersion.settings.timeOfDay;
  const migrated = deserializeCity(JSON.stringify(previousVersion));
  assert.equal(migrated.settings.dayNightCycle, true); assert.equal(migrated.settings.timeOfDay, 14);
  for (const hour of [0, 23.75]) {
    city.settings.dayNightCycle = false; city.settings.timeOfDay = hour;
    const loaded = deserializeCity(serializeCity(city));
    assert.equal(loaded.settings.dayNightCycle, false); assert.equal(loaded.settings.timeOfDay, hour);
    const expanded = expandCity(loaded, 128);
    assert.equal(expanded.settings.dayNightCycle, false); assert.equal(expanded.settings.timeOfDay, hour);
  }
});
...
test('save validation rejects invalid day-night switches and hours outside the twenty-four-hour clock', () => {
  ...
  for (const [key, values] of [
    ...
    const raw = JSON.parse(saved); raw.settings[key] = value;
    assert.throws(() => deserializeCity(JSON.stringify(raw)), `Reject invalid ${key}: ${String(value)}`);
  }
});

test('the unlocked citizen interaction tool cannot paint or mutate construction tiles', () => {
  const city = district(); unlock(city, 'citizen');
  const before = JSON.stringify(city);
