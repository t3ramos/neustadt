...
function car(state: ReturnType<typeof city>, x = 0, z = 0): DrivableCar {
  ...
  model.add(
    new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.12, 0.32), new THREE.MeshStandardMaterial()),
  );
...
test('throttle accelerates to a finite speed, coasting retains inertia, and handbrake stops without reversing', () => {
  const state = city(),
    motion = createVehicleMotion(state, 0, -8);
  run(state, motion, { throttle: 1, steer: 0, handbrake: false }, 3);
  assert.ok(motion.speed > 1.8 && motion.speed <= 70 / 36);
  const movingSpeed = motion.speed,
    position = motion.z;
  run(state, motion, { throttle: 0, steer: 0, handbrake: false }, 0.25);
...
test('brake first removes forward momentum; holding S then engages limited reverse', () => {
  const state = city(),
    motion = createVehicleMotion(state, 0, 0);
  run(state, motion, { throttle: 1, steer: 0, handbrake: false }, 1);
  run(state, motion, { throttle: -1, steer: 0, handbrake: false }, 0.2);
  assert.ok(motion.speed > 0);
  run(state, motion, { throttle: -1, steer: 0, handbrake: false }, 2);
  assert.ok(motion.speed < 0 && motion.speed >= -0.68);
});
...
test('left steering bends motion left in the chase camera; reverse steering reverses yaw', () => {
  ...
  assert.ok(motion.x > 0.02 && motion.yaw > 0);
  const camera = new THREE.PerspectiveCamera(55, 1, 0.035, 160);
  ...
  assert.ok(
    left.x < straight.x,
    'Positive yaw must project to the left from behind a +Z-forward car',
  );
...
test('fixed-step input produces the same vehicle path at 30, 60 and 144 rendered frames per second', () => {
  ...
  const simulate = (fps: number) => {
    const vehicle = car(state),
      system = createDrivingController(state, () => [vehicle]);
    system.enter(vehicle.model.id);
    system.keyDown('KeyW');
    system.keyDown('KeyA');
    ...
    const result = [
      ...
      vehicle.model.rotation.y,
      system.getStatus().speed,
    ];
    system.dispose();
    return result;
  };
  ...
  for (const fps of [60, 144])
    simulate(fps).forEach((value, i) =>
      assert.ok(Math.abs(value - baseline[i]) < 1e-9, `fps=${fps} component=${i}`),
    );
});
...
test('a car enters the empty part of a facility lot and stops at its actual visible structure', () => {
  ...
  for (let z = 21; z < 24; z++)
    for (let x = 19; x < 22; x++) {
      ...
      tile.kind = 'hospital';
      tile.level = 1;
      tile.anchor = 21 * state.size + 19;
    }
  const motion = createVehicleMotion(state, 0, -0.5);
  run(state, motion, { throttle: 1, steer: 0, handbrake: false }, 6);
  assert.ok(
    motion.z > 1.5 && motion.z < 1.7,
    `Car must cross the empty lot border but stop at the hospital wall: z=${motion.z}`,
  );
  assert.equal(motion.blocked, 'Gebäude im Weg');
  assert.equal(drivingObstacle(state, 0, 1.8, 0), 'Gebäude im Weg');
  const zoned = state.tiles[20 * state.size + 20];
  zoned.kind = 'residential';
  zoned.level = 0;
  assert.equal(drivingObstacle(state, 0.5, 0.5, 0), undefined);
  zoned.level = 1;
  assert.equal(drivingObstacle(state, 0.5, 0.5, 0), 'Gebäude im Weg');
});

test('cars cannot enter water or leave map bounds; road bridges remain driveable above the water', () => {
  const state = city();
  for (let x = 0; x < state.size; x++) {
    const tile = state.tiles[21 * state.size + x];
    tile.kind = 'water';
    tile.elevation = -1;
  }
  const motion = createVehicleMotion(state, 0, -0.5);
  run(state, motion, { throttle: 1, steer: 0, handbrake: false }, 6);
  assert.ok(motion.z + 0.3385 / 2 < 1);
  assert.equal(motion.blocked, 'Hier ist Wasser');
  for (const tile of state.tiles) {
    tile.kind = 'road';
    tile.elevation = -1;
  }
  ...
  assert.ok(edge.z + 0.3385 / 2 < 20);
  assert.equal(edge.blocked, 'Stadtrand erreicht');
});

test('road hills lift the vehicle and tilt its suspension without nonfinite transforms or sinking', () => {
  const state = city();
  for (const tile of state.tiles) tile.elevation = Math.max(0, Math.min(8, (tile.z - 17) * 0.5));
  const motion = createVehicleMotion(state, 0, -1);
  const initialHeight = motion.y;
  run(state, motion, { throttle: 1, steer: 0, handbrake: false }, 2);
  assert.ok(motion.y > initialHeight + 0.15);
  assert.ok(motion.pitch < -0.1);
  assert.ok(motion.y >= drivingSurfaceHeight(state, motion.x, motion.z) - 0.026);
  for (const value of [motion.x, motion.y, motion.z, motion.pitch, motion.roll, motion.yaw])
...
test('ordinary asphalt curbs allow driving onto grass and back, while off-road speed stays limited', () => {
  ...
  const motion = createVehicleMotion(state, 0, -0.5);
  run(state, motion, { throttle: 1, steer: 0, handbrake: false }, 3);
  assert.ok(motion.z > 0.5, 'The road pavement thickness must not be mistaken for a cliff');
  assert.ok(motion.speed <= 25 / 36 + 1e-6);
  setVehicleVelocity(motion, 0, 0);
  run(state, motion, { throttle: -1, steer: 0, handbrake: false }, 7);
  assert.ok(motion.z < -0.2, 'The same curb must be passable in the uphill direction');
...
test('hover selects and glows one car, respects occlusion distance, and survives moving to its steering icon', () => {
  const state = city(),
    vehicle = car(state),
    hovers: (DrivingHover | null)[] = [];
  const system = createDrivingController(state, () => [vehicle], {
    onHover: (info) => hovers.push(info),
  });
  const view = new THREE.PerspectiveCamera(55, 2, 0.1, 160);
  view.position.set(0, 3, -3);
  view.lookAt(0, 0, 0);
  ...
  assert.equal(hover?.id, vehicle.model.id);
  assert.ok(Number.isFinite(hover?.screenX));
  system.update(0.016);
  ...
  system.update(0.25);
  assert.equal(system.group.visible, true);
  assert.equal(system.enter(vehicle.model.id), true);
  assert.equal(system.group.visible, false);
  assert.equal(hovers.at(-1), null);
  system.exit();
  raycaster.ray.origin.copy(vehicle.model.position);
  raycaster.ray.origin.y += 5;
  raycaster.far = 2;
  assert.equal(
    system.hover(raycaster, view, bounds),
    null,
    'A nearer terrain/building hit must occlude the car',
  );
  system.dispose();
});
...
test('entering a vehicle flies toward a finite perspective chase camera and controls stay independent of city pause', () => {
  ...
  state.speed = 0;
  const view = new THREE.PerspectiveCamera(55, 2, 0.1, 160);
  view.position.set(20, 30, -20);
  view.lookAt(vehicle.model.position);
  view.updateMatrixWorld();
  system.hover(new THREE.Raycaster(new THREE.Vector3(0, 5, 0), new THREE.Vector3(0, -1, 0)), view, {
    left: 0,
    top: 0,
    width: 1200,
    height: 600,
  ...
  assert.equal(system.enter(vehicle.model.id), true);
  system.resize(1200, 600);
  assert.equal(system.camera.aspect, 2);
  assert.ok(system.camera.position.distanceTo(vehicle.model.position) > 20);
  assert.equal(system.keyDown('KeyW'), true);
  assert.equal(system.keyDown('KeyQ'), false);
  for (let i = 0; i < 180; i++) system.update(1 / 120);
  assert.ok(system.camera.position.distanceTo(vehicle.model.position) < 4);
  assert.ok(vehicle.model.position.z > 0.1);
  ...
  assert.equal(system.keyDown('Escape'), true);
  assert.equal(system.active, false);
  assert.equal(system.keyDown('KeyW'), false);
  system.dispose();
});
...
test('hover expires after leaving the canvas or after the autonomous car leaves a stationary pointer', () => {
  ...
  system.hover(ray, camera, viewport);
  system.leaveHover();
  system.update(0.25);
  assert.equal(system.group.visible, true, 'Allow enough time to reach the wheel button');
  for (let i = 0; i < 4; i++) system.update(0.25);
  assert.equal(system.group.visible, false);
  assert.equal(hovers.at(-1), null);
  system.hover(ray, camera, viewport);
  vehicle.model.position.x = 1;
  for (let i = 0; i < 4; i++) system.update(0.25);
  assert.equal(system.group.visible, false);
  assert.equal(hovers.at(-1), null);
  system.dispose();
...
test('traffic handback refuses a distant road and only synchronizes a car already on its lane', () => {
  ...
  vehicle.model.position.set(2, 0.01, 1.8);
  vehicle.model.rotation.y = 0;
  const original = vehicle.model.position.clone();
  assert.equal(handBackToTraffic(state, vehicle), false);
  assert.deepEqual(vehicle.model.position, original);
  vehicle.model.position.set(-1.63, drivingSurfaceHeight(state, -1.63, 1.8), 1.8);
  assert.equal(handBackToTraffic(state, vehicle), true);
  assert.equal(vehicle.from.x, 18);
  assert.equal(vehicle.to.x, 18);
...
test('replacing the traffic list safely releases a stale selected car and clears held keys', () => {
  ...
  traffic = [];
  system.update(0.016);
  assert.equal(system.selectedCar, null);
  assert.equal(system.getStatus().active, false);
  assert.equal(system.enter(vehicle.model.id), false);
  system.dispose();
  system.dispose();
});
...
test('the speed governor uses 50 km/h by developed streets, 70 outside town, and 25 off road', () => {
  ...
  for (let z = 0; z < state.size; z++)
    Object.assign(state.tiles[z * state.size + 23], { kind: 'residential', level: 1 });
  ...
  assert.equal(getDrivingSpeedLimit(state, -1.5, 0.5), 25);
...
test('handbrake plus steering retains world momentum as a real sideways slide, and tire grip recovers on release', () => {
  const state = city(),
    normal = createVehicleMotion(state, 5, 5),
    drift = createVehicleMotion(state, 5, 5);
  for (const motion of [normal, drift]) setVehicleVelocity(motion, 0, 50 / 36);
  run(state, normal, { throttle: 0, steer: 1, handbrake: false }, 0.5);
  run(state, drift, { throttle: 0, steer: 1, handbrake: true }, 0.5);
  assert.ok(Math.abs(drift.lateralSpeed) > Math.abs(normal.lateralSpeed) * 2.5);
  assert.ok(
    Math.hypot(drift.vx, drift.vz) > 0.7,
    'A steering handbrake should not simply stop the car',
  );
  assert.ok(drift.yaw > 0.35 && Math.abs(Math.atan2(drift.vx, drift.vz) - drift.yaw) > 0.35);
  assert.ok(
    drift.x < normal.x * 0.1 + 4.5,
    'The mass should continue along its old path while the body turns',
  );
  assert.equal(drift.drifting, true);
  run(state, drift, { throttle: 0, steer: 0, handbrake: false }, 0.5);
  assert.ok(Math.abs(drift.lateralSpeed) < 0.04);
...
test('ramming a stopped traffic car transfers momentum and keeps the two bodies from passing through each other', () => {
  const state = city(),
    player = car(state, 0, -1),
    parked = car(state, 0, 0),
    hits: VehicleCollision[] = [];
  parked.speed = 0;
  const system = createDrivingController(state, () => [player, parked], {
    onCollision: (hit) => hits.push(hit),
  });
  system.enter(player.model.id);
  system.keyDown('KeyW');
  for (let frame = 0; frame < 240; frame++) system.update(1 / 120);
  assert.ok(parked.model.position.z > 0.5, 'The other car must visibly move from the impact');
  assert.ok(parked.model.position.z - player.model.position.z >= 0.3385 - 0.0001);
  assert.equal(
    system.controlsCar(parked),
    true,
    'Traffic must not overwrite the knocked vehicle on the next frame',
  );
  assert.ok(hits.length >= 1 && hits[0].speed > 0.7);
  assert.equal(hits[0].otherId, parked.model.id);
  assert.ok(system.getStatus().collisionCount! >= 1);
  system.dispose();
});
...
test('a glancing vehicle collision produces a sideways deflection and yaw instead of a full head-on stop', () => {
  const state = city(),
    player = car(state, 0, -1),
    target = car(state, 0.145, 0),
    hits: VehicleCollision[] = [];
  target.speed = 0;
  const system = createDrivingController(state, () => [player, target], {
    onCollision: (hit) => hits.push(hit),
  });
  system.enter(player.model.id);
  system.keyDown('KeyW');
  ...
  assert.ok(
    Math.abs(target.model.position.x - 0.145) > 0.005 || Math.abs(player.model.position.x) > 0.005,
  );
  assert.ok(Math.abs(target.model.rotation.y) > 0.03 || Math.abs(player.model.rotation.y) > 0.03);
  for (const vehicle of [player, target])
    for (const value of [
      ...vehicle.model.position.toArray(),
      ...vehicle.model.quaternion.toArray(),
    ])
      assert.ok(Number.isFinite(value));
  system.dispose();
});
...
test('overlapping stationary cars separate even without throttle and a fast crossing NPC is swept between frames', () => {
  const state = city(),
    player = car(state),
    neighbor = car(state, 0.05, 0);
  player.speed = neighbor.speed = 0;
  const system = createDrivingController(state, () => [player, neighbor]);
  system.enter(player.model.id);
  system.update(1 / 120);
  assert.ok(Math.abs(player.model.position.x - neighbor.model.position.x) >= 0.183 - 0.00001);
  system.dispose();

  const a = car(state, 5, 0),
    crossing = car(state, 5, -2),
    collisions: VehicleCollision[] = [];
  a.speed = 0;
  crossing.speed = 3;
  const fast = createDrivingController(state, () => [a, crossing], {
    onCollision: (hit) => collisions.push(hit),
  });
  fast.enter(a.model.id);
  fast.update(1 / 120);
  ...
  assert.ok(
    crossing.model.position.z < 1,
    'The swept contact must resolve near the impact instead of tunneling past',
  );
  fast.dispose();
});
...
test('pedestrian sweeps contain each physical step, real world velocity, and the selected variant dimensions', () => {
  const state = city(),
    truck = car(state),
    sweeps: VehicleSweep[] = [];
  truck.model.userData.vehicleDimensions = {
    width: 0.237,
    length: 0.5745,
    height: 0.259,
    wheelBase: 0.3365,
  ...
  const system = createDrivingController(state, () => [truck], {
    onVehicleSweep: (sweep) => sweeps.push(sweep),
  });
  system.enter(truck.model.id);
  system.keyDown('KeyW');
  system.update(0.25);
  ...
  for (const sweep of sweeps) {
    ...
    assert.equal(sweep.width, 0.237);
    assert.equal(sweep.length, 0.5745);
    assert.equal(sweep.height, 0.259);
...
test('the front of a longer truck remains hoverable and its glow uses the actual variant dimensions', () => {
  ...
  const system = createDrivingController(state, () => [truck]);
  const view = new THREE.PerspectiveCamera(55, 1, 0.1, 160);
  view.position.set(0, 3, -3);
  view.lookAt(0, 0, 0);
  view.updateMatrixWorld();
  const hover = system.hover(
    new THREE.Raycaster(new THREE.Vector3(0, 5, 0.26), new THREE.Vector3(0, -1, 0)),
    view,
...
test('exiting off the street keeps the exact car pose and state replacement releases all physical recoveries', () => {
  ...
  assert.equal(system.active, false);
  assert.equal(system.controlsCar(vehicle), true);
  system.update(0.2);
  assert.ok(
...
test('taking over waiting traffic starts stationary and rear-end impulses do not invent a waiting car velocity', () => {
  const state = city(),
    waiting = car(state);
  waiting.speed = 0.65;
  waiting.model.userData.trafficWaiting = true;
  ...
  takeover.update(0.25);
  assert.deepEqual(waiting.model.position, position);
  takeover.dispose();

  const player = car(state, 0, -1),
    stopped = car(state),
    hits: VehicleCollision[] = [];
  stopped.speed = 0.65;
  stopped.model.userData.trafficWaiting = true;
  const collision = createDrivingController(state, () => [player, stopped], {
    onCollision: (hit) => hits.push(hit),
  });
  collision.enter(player.model.id);
  collision.keyDown('KeyW');
  for (let frame = 0; frame < 130; frame++) collision.update(1 / 120);
  assert.ok(
    hits[0]?.speed > 1,
    'The waiting car has no forward velocity to subtract from the rear impact',
  );
  collision.dispose();
});
...
test('hitting an actual building emits one world crash with pre-impact speed and a surface contact point', () => {
  const state = city(),
    vehicle = car(state, 0.5, -1),
    hits: VehicleCollision[] = [];
  for (let x = 0; x < state.size; x++)
    Object.assign(state.tiles[22 * state.size + x], {
      kind: 'residential',
      level: 1,
      variation: 0,
    });
  ...
  assert.equal(hits[0].otherId, null);
  assert.equal(hits[0].vehicleId, vehicle.model.id);
  assert.ok(hits[0].speed > 0.6);
  assert.ok(hits[0].normal.z > 0.9);
  assert.ok(hits[0].point.z > vehicle.model.position.z);
  assert.equal(system.getStatus().collisionCount, 1);
  assert.equal(system.getStatus().blocked, 'Gebäude im Weg');
  system.dispose();
});
...
test('traffic handback preserves a curved lane and its real incoming neighbor in every direction', () => {
  ...
  for (let turn = 0; turn < 4; turn++)
    for (const progress of [0.15, 0.43, 0.82]) {
      const state = city(),
        vehicle = car(state),
        incoming = directions[turn],
        outgoing = directions[(turn + 1) % 4];
      for (const tile of state.tiles) tile.kind = 'empty';
      const from = { x: 20, z: 20 },
        previous = { x: from.x - incoming.x, z: from.z - incoming.z },
        to = { x: from.x + outgoing.x, z: from.z + outgoing.z };
      for (const point of [previous, from, to])
        state.tiles[point.z * state.size + point.x].kind = 'road';
      const pose = lanePose(previous, from, to, progress),
        x = pose.x - state.size / 2,
        z = pose.z - state.size / 2;
      vehicle.model.position.set(x, drivingSurfaceHeight(state, x, z), z);
      vehicle.model.rotation.y = pose.yaw;
      assert.equal(handBackToTraffic(state, vehicle), true);
      assert.deepEqual(vehicle.previous, previous);
      assert.deepEqual(vehicle.from, from);
      assert.deepEqual(vehicle.to, to);
      const resumed = lanePose(vehicle.previous!, vehicle.from, vehicle.to, vehicle.progress);
      assert.ok(
        Math.hypot(resumed.x - state.size / 2 - x, resumed.z - state.size / 2 - z) < 0.0001,
        'Handback must keep the actual curved location',
      );
