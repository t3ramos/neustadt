...
test('legacy annual reports retain signs and decimal money while changing numeric separators', () => {
  const event = {
    title: 'Jahresbericht',
    message: '12.345 Einwohner · 83 % Zufriedenheit · +1.234,56 € monatlich.',
  };
  setLocale('en');
  assert.equal(localizedEventTitle(event), 'Annual report');
  assert.equal(
    localizedEventMessage(event),
    '12,345 residents · 83% happiness · +€1,234.56 per month.',
  );
...
test('legacy welcome and victory translations preserve user city names exactly', () => {
  ...
  const welcome = {
    title: `Willkommen in ${name}`,
    message:
      'Deine Stadt ist bereit. Erschließe neue Wohngebiete, halte die Versorgung im Blick und baue eine Metropole.',
  };
  const victory = {
    message: `${name} hat sechs Monate lang mindestens 25.000 Einwohner, hohe Lebensqualität und einen positiven Haushalt gehalten. Du hast die Kampagne gemeistert. Baue im freien Spiel weiter!`,
  };
  setLocale('en');
  assert.equal(localizedEventTitle(welcome), `Welcome to ${name}`);
  assert.equal(
    localizedEventMessage(welcome),
    'Your city is ready. Develop new residential districts, keep services running, and build a metropolis.',
  );
  assert.ok(
    localizedEventMessage(victory).startsWith(`${name} maintained at least 25,000 residents`),
  );
...
test('every current legacy title has English text including all development stages', () => {
  const titles = [
    'Willkommen in Lindenbucht',
    'Ein neuer Anfang',
    'Brandschäden',
    'Eine Stadt wächst',
    'Jahresbericht',
    'Die Stadtkasse ist im Minus',
    'Einwohner ziehen fort',
    'Kredit ausgezahlt',
    'Keine Schäden',
    'Brand in der Stadt',
    'Erdbeben',
    ...
    'Ausbaustufe erreicht: Kleinstadt',
    'Ausbaustufe erreicht: Großstadt',
    'Ausbaustufe erreicht: Metropole',
  ];
  setLocale('en');
  ...
  assert.equal(
    localizedEventTitle({ title: 'Ausbaustufe erreicht: Kleinstadt' }),
    'Development stage reached: Small Town',
  );
});
...
test('legacy simulation journals translate every static and dynamic event family', () => {
  ...
  assert.equal(
    localizedEventMessage({ message: messages[3] }),
    '2,500 residents! The state is supporting your city with €5,000.',
  );
  assert.equal(
    localizedEventMessage({ message: messages[6] }),
    '€10,000 has been paid out. Monthly interest: 0.5%.',
  );
});
...
test('all legacy quest reward names match their current English counterparts', () => {
  ...
  for (const [de, en] of names) {
    ...
    assert.equal(
      localizedEventMessage({
        message: `${de} — hole 1.500 € und 1.000 EP im Stadtziel-Fenster ab.`,
      }),
      `${en} — collect €1,500 and 1,000 XP in the city goals window.`,
    );
  }
});
...
test('legacy challenge starts, rewards and failures translate full descriptions and badge status', () => {
  ...
  for (const [de, en, months, description, descriptionEn] of challenges) {
    assert.equal(
      localizedEventMessage({
        message: `${de} gestartet. Du hast ${months} Monate. ${description}`,
      }),
      `${en} started. You have ${months} months. ${descriptionEn}`,
    );
    assert.equal(
      localizedEventMessage({
        message: `${de}: +20.000 € und +1.200 EP. Dein Abzeichen bleibt erhalten.`,
      }),
      `${en}: +€20,000 and +1,200 XP. Your badge is permanent.`,
    );
    assert.equal(
      localizedEventMessage({
        message: `${de}: Die Zeit ist abgelaufen. Du kannst es erneut versuchen.`,
      }),
      `${en}: Time has run out. You can try again.`,
    );
  }
  assert.equal(
    localizedEventMessage({
...
test('legacy rank milestones translate their entire descriptions', () => {
  ...
  for (const description of descriptions) {
    const translated = localizedEventMessage({
      message: `15.000 Einwohner erreicht. ${description}`,
    });
...
test('custom imports resembling known templates do not lose unknown names or descriptions', () => {
  ...
  assert.equal(
    localizedEventTitle({ title: 'Ausbaustufe erreicht: Meine Stadt' }),
    'Original event: Ausbaustufe erreicht: Meine Stadt',
  );
