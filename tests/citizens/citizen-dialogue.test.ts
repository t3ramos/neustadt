...
import {
  CITIZEN_LINES,
  CITIZEN_LINES_EN,
  CITIZEN_SPEECH_LIMITS,
  createCitizenSpeechSelector,
  type CitizenDialogueTopic,
} from '../../src/citizens/dialogue/catalog.ts';
...
test('residents have hundreds of genuinely authored, unique, readable German remarks', () => {
  ...
  assert.ok(allLines.length >= 680, `Only ${allLines.length} authored lines`);
  assert.equal(
    new Set(allLines).size,
    allLines.length,
    'Different contexts must not merely duplicate the same remarks',
  );
  const normalized = allLines.map((line) =>
    line.toLocaleLowerCase('de-DE').replace(/[^\p{L}\p{N}]/gu, ''),
  );
  assert.equal(
    new Set(normalized).size,
    allLines.length,
    'Punctuation or capitalisation do not create a new remark',
  );
  for (const topic of topics) {
    assert.ok(
      CITIZEN_LINES[topic].length >= 12,
      `${topic} needs enough observations for a real conversation pool`,
    );
    for (const line of CITIZEN_LINES[topic]) {
      assert.ok(
        line.length >= 20 && line.length <= 115,
        `${topic}: unreadable bubble length ${line.length}: ${line}`,
      );
...
test('every authored observation has a distinct, readable English counterpart at the same index', () => {
  ...
  assert.equal(englishLines.length, 680);
  const normalized = englishLines.map((line) =>
    line.toLocaleLowerCase('en-US').replace(/[^\p{L}\p{N}]/gu, ''),
  );
  assert.equal(
    new Set(normalized).size,
    englishLines.length,
    'English observations must not collapse into repeated stock phrases',
  );
  for (const topic of topics) {
    assert.equal(
      CITIZEN_LINES_EN[topic].length,
      CITIZEN_LINES[topic].length,
      `${topic} lost a translated observation`,
    );
    CITIZEN_LINES_EN[topic].forEach((line, index) => {
      assert.notEqual(line, CITIZEN_LINES[topic][index], `${topic}[${index}] was left in German`);
      assert.ok(
        line.length >= 20 && line.length <= 115,
        `${topic}: unreadable English bubble length ${line.length}: ${line}`,
      );
      assert.equal(line.trim(), line);
  ...
  assert.ok(
    CITIZEN_LINES_EN.night.every((line) => !/lights|illumination|bright/i.test(line)),
    'Night alone must not claim working lights',
  );
  assert.ok(
    CITIZEN_LINES_EN.pets.every((line) => !/\b(cat|dog|bark|barks|meow|meows)\b/i.test(line)),
    'Generic pet observations must work for either cat or dog',
  );
});
  ...
  for (const topic of topics) {
    ...
    for (let i = 0; i < CITIZEN_LINES[topic].length; i++) {
      ...
      assert.ok(
        ...
        `${topic} repeated an observation after changing language`,
      );
      seenIndexes.add(index);
      assert.equal(
        expected.en,
        CITIZEN_LINES_EN[topic][index],
        `${topic} changed meaning between languages`,
      );
    }
  }
});
  ...
  for (const topic of topics) {
    ...
    for (let cycle = 0; cycle < 4; cycle++) {
      ...
      assert.deepEqual(
        new Set(actual),
        expected,
        `${topic}: cycle ${cycle} repeated or skipped an authored remark`,
      );
    }
  }
});
...
test('new neighbours do not all echo a recent line when fresh observations are available', () => {
  ...
  for (let actorId = 0; actorId < 120; actorId++) {
    ...
    assert.ok(
      !said.slice(-CITIZEN_SPEECH_LIMITS.globalHistory).includes(line),
      'Nearby residents formed a chorus',
    );
    said.push(line);
  }
});
...
test('selection never invents a remark from a different event or setting', () => {
  ...
  assert.ok(
    CITIZEN_LINES.night.every((line) => !/Lichter|Beleuchtung|hell/i.test(line)),
    'Night alone must not claim working lights',
  );
  assert.ok(
    CITIZEN_LINES.pets.every((line) => !/bellt|miaut|Katze|Hund/.test(line)),
    'Generic pet observations must work for either cat or dog',
  );
});
...
test('speech memories stay bounded as many residents arrive and leave', () => {
  ...
  assert.ok(
    debug.actorHistory <= CITIZEN_SPEECH_LIMITS.actors * CITIZEN_SPEECH_LIMITS.actorHistory,
  );
  ...
  selector.clear();
  assert.deepEqual(selector.getDebug(), {
    actors: 0,
    topicBags: 0,
    actorHistory: 0,
    globalHistory: 0,
