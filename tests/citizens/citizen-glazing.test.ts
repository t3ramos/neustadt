...
function facadeCity(): { state: CityState; tile: Tile } {
  ...
  const home = state.tiles[5 * state.size + 5];
  home.kind = 'residential';
  home.level = 1;
  const tile = state.tiles[20 * state.size + 20];
...
function exteriorFacade(
  ...
  model.position.set(tile.x - state.size / 2 + 0.5, tile.elevation, tile.z - state.size / 2 + 0.5);
  ...
  model.traverse((object) => {
    ...
    if (size.y > 0.35 && size.x > 0.16 && size.z > 0.16)
      structures.push({ mesh: object, bounds, volume: size.x * size.y * size.z });
  });
  structures.sort((a, b) => b.volume - a.volume);
  const tangent: Axis = axis === 'x' ? 'z' : 'x',
  ...
  for (const structure of structures)
    for (const height of [0.5, 0.4, 0.6, 0.3, 0.7])
      // Sample between the new curtain wall's vertical fins, retaining the full
      // body-clearance probes rather than assuming five legacy wall positions.
      for (const offset of Array.from({ length: 19 }, (_, index) => -0.45 + index * 0.05)) {
        const point = structure.bounds.getCenter(new THREE.Vector3()),
          size = structure.bounds.getSize(new THREE.Vector3());
        point.y = structure.bounds.min.y + size.y * height;
        point[tangent] += size[tangent] * offset;
        ...
        if (hitNormal.dot(normal) < 0.99) continue;
        ...
        const clear = [
          ...
          return (
            nearby?.object === structure.mesh &&
            Math.abs(nearby.point[axis] - hit.point[axis]) < 0.0001
          );
        });
        if (clear) return { point: hit.point, normal: hitNormal };
      }
  assert.fail(
    `Commercial fixture has no clear ${sign > 0 ? '+' : '−'}${axis} structural facade for a physical throw`,
  );
}
...
for (const [axis, sign] of [
  ...
  for (const distance of [0.6, 0.602, 0.618, 0.638]) {
    test(`a ragdoll thrown from ${distance}m strikes the ${sign > 0 ? '+' : '−'}${axis} visible building facade and its saved stain stays on that surface`, () => {
      const { state, tile } = facadeCity(),
        { point: target, normal } = exteriorFacade(state, tile, axis, sign);
      const outerFace = target[axis];
      const origin = target.clone().addScaledVector(normal, distance);
      origin.y -= 0.32 * CITIZEN_SCALE;
      const velocity = normal.clone().multiplyScalar(-MAX_HAND_SPEED),
        world = createCitizenPhysicsWorld(),
        building = createBuildingCollider(state, tile);
      assert.ok(building);
      ...
      try {
        ...
        for (let elapsed = 0; elapsed < 1; elapsed += CITIZEN_PHYSICS_STEP)
          world.step(CITIZEN_PHYSICS_STEP);
        ...
        assert.ok(
          hitNormal.dot(normal) > 0.99,
          `Contact normal must face outward from the raycast facade; hit=${JSON.stringify(hit)}, target=${JSON.stringify(target)}`,
        );
        assert.ok(
          Math.abs(hit.point[axis] - outerFace) < 0.0001,
        ...
        const applied = applyCitizenIncident(state, {
          x: hit.point.x + state.size / 2,
          y: hit.point.y,
          z: hit.point.z + state.size / 2,
          nx: hit.normal.x,
          ny: hit.normal.y,
          nz: hit.normal.z,
          witnessed: false,
        ...
        assert.equal(state.stats.population, before - 1);
        const restored = deserializeCity(serializeCity(state)),
          incident = restored.citizenEffects.incidents[0];
        assert.ok(
          Math.abs(incident[axis] - state.size / 2 - outerFace) < 0.0001,
        ...
        try {
          ...
          const mark = marks.children[0],
            markNormal = new THREE.Vector3(0, 0, 1).applyQuaternion(mark.quaternion);
          assert.ok(markNormal.dot(normal) > 0.99);
          assert.ok(
            Math.abs(mark.position[axis] - outerFace) < 0.003,
