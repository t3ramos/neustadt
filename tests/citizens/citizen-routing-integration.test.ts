...
import {
  createCitizens,
  type CitizenSystem,
  type CitizenVehicleSweep,
} from '../../src/citizens/system.ts';
import { SIDEWALK_OFFSET } from '../../src/citizens/routing.ts';
import { applyCitizenIncident, createCity } from '../../src/simulation/city-simulation.ts';
...
function city(roads: readonly (readonly [number, number])[]): CityState {
  ...
  for (const [x, z] of roads) {
    const home = [
      ...
      .map(([gx, gz]) => state.tiles[gz * state.size + gx])
      .find((t) => t?.kind === 'empty');
    if (home) {
...
function positionOnMap(state: CityState, point: Pick<Walker, 'x' | 'z'>) {
  ...
  return {
    ...
    x,
    z,
    dx: point.x - (x + 0.5 - state.size / 2),
    dz: point.z - (z + 0.5 - state.size / 2),
  };
}
...
function verifyFrame(
  state: CityState,
  before: readonly Walker[],
  after: readonly Walker[],
  ...
  for (const walker of after) {
    ...
    if (old) {
      const dx = Math.abs(walker.x - old.x),
        dz = Math.abs(walker.z - old.z);
      assert.ok(
        dx < 1e-9 || dz < 1e-9,
        'A walker must turn at the vertex rather than jump diagonally',
      );
      const maximumStep = 0.16 * FRAME * (walker.crossing || old.crossing ? 1.6 : 1);
      assert.ok(
        Math.hypot(dx, dz) <= maximumStep + 0.00001,
        'Walking remains bounded by normal or brisk crossing pace',
      );
    }
    ...
    if (walker.crossing) {
      ...
      assert.equal(point.x, junction[0]);
      assert.equal(point.z, junction[1]);
      assert.ok(
        Math.abs(Math.abs(point.dx) - SIDEWALK_OFFSET) < 1e-9 ||
          Math.abs(Math.abs(point.dz) - SIDEWALK_OFFSET) < 1e-9,
        'Crossing stays inside its painted edge band',
      );
    }
    if (onAsphalt(state, walker)) {
      assert.equal(
        walker.crossing,
        true,
        'Asphalt traversal requires an explicitly marked graph crossing',
      );
      crossings++;
    } else {
      assert.ok(
        Math.abs(Math.abs(point.dx) - SIDEWALK_OFFSET) < 1e-9 ||
          Math.abs(Math.abs(point.dz) - SIDEWALK_OFFSET) < 1e-9,
        'Walkers remain on the .405 pavement line',
      );
    }
  }
  return crossings;
...
test('actual walkers keep the same straight-road pavement and take no diagonal shortcuts', () => {
  const state = city(Array.from({ length: 9 }, (_, i) => [i + 6, 10] as const)),
    system = createCitizens(state);
  try {
    let before = system.getDebug().positions,
      movement = 0;
    assert.equal(before.length, 24);
    verifyFrame(state, [], before);
    for (let frame = 0; frame < 1000; frame++) {
      system.animate(FRAME, true);
      const after = system.getDebug().positions;
      verifyFrame(state, before, after);
      for (let i = 0; i < after.length; i++)
        movement += Math.hypot(after[i].x - before[i].x, after[i].z - before[i].z);
      before = after;
    }
    assert.ok(
      movement > 100,
      'Crowds must actually walk, rather than merely remain at valid spawn points',
    );
  } finally {
    system.dispose();
  }
...
test('actual walkers turn around an L junction on the inside and outside pavement', () => {
  ...
  try {
    ...
    assert.ok(bendVisits > 100);
    assert.deepEqual([...seenDirections].sort(), ['horizontal', 'vertical']);
    assert.deepEqual(
      [...completedRoutes].sort(),
      ['inside', 'outside'],
      'Residents individually complete both pavement turns between the north and east arms',
    );
  } finally {
    system.dispose();
  }
...
test('actual T-junction crossings use only painted bands and expose crossing=true', () => {
  ...
  try {
    ...
    for (let frame = 0; frame < 1600; frame++) {
      system.animate(FRAME, true);
      const after = system.getDebug().positions;
      asphaltFrames += verifyFrame(state, before, after, [10, 10]);
      before = after;
    }
    assert.ok(
      asphaltFrames > 100,
      'Residents must actually traverse the marked junction during the run',
    );
  } finally {
    system.dispose();
  }
...
test('repeated traffic-only vehicle sweeps never kill, throw or remove waiting pedestrians', () => {
  ...
  const system = createCitizens(state, (incident) => {
    ...
    applyCitizenIncident(state, incident);
  });
  try {
    const originalPopulation = state.stats.population,
      originalCount = system.getDebug().count;
    ...
    const sweeps: CitizenVehicleSweep[] = [
      {
        previous: { x: center - 2, y: 1.04, z: center },
        current: { x: center + 2, y: 1.04, z: center },
        yaw: Math.PI / 2,
        width: 0.18,
        length: 0.34,
      ...
      {
        previous: { x: center, y: 1.04, z: center - 2 },
        current: { x: center, y: 1.04, z: center + 0.2 },
        yaw: 0,
        width: 0.18,
        length: 0.34,
    ...
    assert.equal(state.stats.population, originalPopulation);
    assert.equal(system.getDebug().count, originalCount);
  ...
  try {
    ...
    for (let frame = 0; frame < 1400 && !corner; frame++) {
      ...
      corner = system.getDebug().positions.find((p) => {
        ...
        return (
          Math.abs(Math.abs(dx) - SIDEWALK_OFFSET) < 1e-9 &&
          Math.abs(Math.abs(dz) - SIDEWALK_OFFSET) < 1e-9
        );
      });
    }
    ...
    const center = 10.5 - state.size / 2;
    const current = {
      x: horizontal ? center : start.x - 1.2,
      y: 1.04,
      z: horizontal ? start.z - 1.2 : center,
    };
    const event: CitizenVehicleSweep = {
      previous: {
        x: current.x - (horizontal ? 0 : 0.2),
        y: current.y,
        z: current.z - (horizontal ? 0.2 : 0),
      },
      current,
      yaw: horizontal ? 0 : Math.PI / 2,
      width: 0.18,
      length: 0.34,
      velocity: { x: horizontal ? 0 : 3, y: 0, z: horizontal ? 3 : 0 },
      vehicleId: 33,
      trafficOnly: true,
    };
    ...
    for (let frame = 0; frame < 20; frame++) {
      ...
      assert.ok(
        Math.hypot(waiting.x - start.x, waiting.z - start.z) < 1e-9,
        'Approaching cars hold the walker at the curb, before an overlap',
      );
      assert.equal(onAsphalt(state, waiting), false);
      assert.equal(waiting.state, 'walking');
    }
    assert.equal(incidents.length, 0);
    ...
    assert.ok(
      Math.hypot(resumed.x - start.x, resumed.z - start.z) > 0.1,
      'Once traffic clears, the walker resumes crossing',
    );
    assert.equal(incidents.length, 0);
    assert.equal(system.getDebug().ragdolls, 0);
  } finally {
