...
import {
  createCitizens,
  SEVERE_VEHICLE_IMPACT_SPEED,
  sweepVehicleFootprint,
  type CitizenSystem,
...
function passingCar(position: Point, speed: number, lateralOffset = 0): CitizenVehicleSweep {
  return {
    previous: { x: position.x - 0.4, y: position.y, z: position.z + lateralOffset },
    current: { x: position.x + 0.4, y: position.y, z: position.z + lateralOffset },
    yaw: Math.PI / 2,
    width: 0.18,
    length: 0.34,
    height: 0.18,
...
test('vehicle sweeps catch a pedestrian between two non-overlapping endpoints', () => {
  ...
  assert.ok(hit);
  assert.ok(Math.abs(hit.time - 0.39375) < 1e-8);
  assert.ok(hit.normal.x > 0.99);
  assert.ok(Math.abs(hit.normal.y) < 1e-8);
  assert.ok(Math.abs(hit.normal.z) < 1e-8);
});
...
test('vehicle footprints rotate their long axis and preserve narrow-side misses', () => {
  ...
  assert.equal(sweepVehicleFootprint({ x: 0.35, y: 0.09, z: 0 }, event), null);
  assert.ok(sweepVehicleFootprint({ x: 0.35, y: 0.09, z: 0 }, { ...event, yaw: Math.PI / 2 }));
  const angle = Math.PI / 4;
  assert.ok(
    sweepVehicleFootprint(
      { x: Math.sin(angle) * 0.35, y: 0.09, z: Math.cos(angle) * 0.35 },
      { ...event, yaw: angle },
    ),
  );
  assert.equal(
    sweepVehicleFootprint(
      { x: Math.cos(angle) * 0.18, y: 0.09, z: -Math.sin(angle) * 0.18 },
      { ...event, yaw: angle },
    ),
    null,
  );
  ...
  assert.equal(sweepVehicleFootprint({ x: 0, y: 0.09, z: 0.126 }, sweep), null);
  assert.ok(sweepVehicleFootprint({ x: 0, y: 0.09, z: 0.124 }, sweep));
});

test('an elevated vehicle cannot strike pedestrians below its deck', () => {
  const event: CitizenVehicleSweep = {
    previous: { x: -2, y: 3, z: 0 },
    current: { x: 2, y: 3, z: 0 },
    yaw: Math.PI / 2,
    width: 0.2,
    length: 0.4,
  ...
  assert.equal(sweepVehicleFootprint({ x: 0, y: 0.09, z: 0 }, event), null);
...
test('a moderate vehicle bump knocks a resident down, then allows recovery without a casualty', () => {
  ...
  try {
    const person = system.getDebug().positions[0];
    system.sweepVehicleImpact(passingCar(person, 0.3));
...
test('a severe ram registers one witnessed casualty when the thrown resident contacts the ground', () => {
  ...
  try {
    const person = system.getDebug().positions[0],
      event = passingCar(person, SEVERE_VEHICLE_IMPACT_SPEED + 0.35);
    system.sweepVehicleImpact(event);
    system.sweepVehicleImpact(event);
    assert.equal(system.getDebug().ragdolls, 1);
    assert.equal(
      incidents.length,
      0,
      'The actor must first enter physics; the vehicle sweep alone must not create a floating stain',
    );
    for (let frame = 0; frame < 480 && !incidents.length; frame++) system.animate(1 / 60, false);
    assert.equal(incidents.length, 1);
    assert.equal(
      system.getDebug().debris,
      6,
      'severe impact preserves six independent visible body parts',
    );
    assert.equal(incidents[0].kind, 'impact');
    assert.equal(incidents[0].witnessed, true);
    assert.ok(incidents[0].ny > 0.9, 'The persisted impact should occur on the ground');
    ...
    assert.equal(
      system.getDebug().debris,
      6,
      'body parts remain visible while settling after impact',
    );
    const next = structuredClone(state);
    next.month++;
    next.revision++;
    system.update(next);
    assert.equal(system.getDebug().debris, 6, 'worker snapshots retain active physical debris');
    advance(system, 9);
    assert.equal(system.getDebug().debris, 0, 'bounded cleanup after the visible settling period');
  } finally {
    system.dispose();
  }
});
...
test('a close pass causes near-miss speech with a cooldown while avoiding physical injury', () => {
  ...
  try {
    const person = system.getDebug().positions[0],
      event = passingCar(person, 1.2, 0.18);
    system.sweepVehicleImpact(event);
    const first = system
      .getDebug()
      .speech.find((entry) => entry.actorId === person.id && entry.kind === 'nearMiss');
    ...
    assert.equal(
      repeated?.expiresAt,
      first.expiresAt,
      'Repeated passes must not restart speech on every frame',
    );
    advance(system, 3.1);
    system.sweepVehicleImpact(event);
    assert.ok(
      !system.getDebug().speech.some((entry) => entry.kind === 'nearMiss'),
      'Expired speech stays suppressed until its cooldown ends',
    );
    advance(system, 0.5);
    system.sweepVehicleImpact(event);
    assert.ok(
      system
        ...
        .speech.some(
          (entry) =>
            entry.actorId === person.id &&
            entry.kind === 'nearMiss' &&
            entry.expiresAt > first.expiresAt,
        ),
    );
    assert.equal(system.getDebug().ragdolls, 0);
    assert.equal(incidents.length, 0);
...
test('ordinary traffic sweeps produce awareness without killing or knocking down a resident', () => {
  ...
  try {
    const person = system.getDebug().positions[0],
      event = { ...passingCar(person, 2), trafficOnly: true };
    system.sweepVehicleImpact(event);
    assert.ok(
      system
        .getDebug()
        .speech.some(
          (entry) =>
            entry.actorId === person.id &&
            ['nearMiss', 'vehicle', 'observation'].includes(entry.kind),
        ),
    );
    advance(system, 4);
    assert.equal(system.getDebug().ragdolls, 0);
