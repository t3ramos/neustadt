...
import { CITIZEN_SCALE, createCitizens, type CitizenSystem } from '../src/citizens.ts';
...
const TORSO_HEIGHT=.32*CITIZEN_SCALE;
...
function isolatedCity(segments:readonly (readonly [number,number])[], police=false):CityState {
  ...
  for(const [first,last] of segments)for(let x=first;x<=last;x++) {
  ...
  if(police) {
    ...
    for(let z=7;z<=8;z++)for(let x=10;x<=11;x++) {
      const tile=state.tiles[z*state.size+x];tile.kind='police';tile.level=1;tile.anchor=anchor;
    }
  }
  recalculate(state);
...
function hand(system:CitizenSystem,position:Point) {
  ...
  const ray=(point:Point)=>new THREE.Ray(
    new THREE.Vector3(point.x,point.y,point.z-1),new THREE.Vector3(0,0,1),
  );
  ...
  return {
    moveTo(target:Point,seconds:number) {
      ...
      for(let frame=1;frame<=frames;frame++) {
        ...
        cursor={x:start.x+(target.x-start.x)*t,y:start.y+(target.y-start.y)*t,z:target.z};
        time+=dt*1000;assert.equal(system.pointerMove(ray(cursor),time),true);system.animate(dt,false);
      }
    },
    wait(seconds:number,walking=false) {
      const frames=Math.ceil(seconds*60),dt=seconds/frames;
      for(let frame=0;frame<frames;frame++){time+=dt*1000;system.animate(dt,walking);}
    },
...
test('grabbing, lifting and throwing applies exactly one witnessed casualty to the city',()=>{
  const state=isolatedCity([[8,12]],true),before={population:state.stats.population,happiness:state.stats.happiness,loss:state.citizenEffects.populationLoss};
  ...
  try {
    system.setEnabled(true);assert.equal(system.getDebug().count,1);
    const person=system.getDebug().positions[0],drag=hand(system,person);
    drag.moveTo({x:person.x,y:person.y+3,z:person.z},1);
    drag.wait(.3);
    // A deliberate downward flick, followed immediately by release.
    drag.moveTo({x:person.x,y:person.y+2.4,z:person.z},.06);
    drag.release();drag.wait(4);
    assert.equal(incidents.length,1);
    assert.equal(incidents[0].kind,'impact');assert.equal(incidents[0].witnessed,true);
    ...
    assert.equal(system.getDebug().count,0);assert.equal(system.getDebug().ragdolls,0);
    assert.equal(system.group.getObjectByName('persistent-citizen-splatters')?.children.length,1);
    drag.wait(2);assert.equal(incidents.length,1);
  } finally {system.dispose();}
});

test('carrying an isolated resident beyond the map and releasing records an unwitnessed abduction',()=>{
  const state=isolatedCity([[8,12]]),before={population:state.stats.population,happiness:state.stats.happiness,loss:state.citizenEffects.populationLoss};
  const incidents:Incident[]=[];
  const system=createCitizens(state,incident=>{
    incidents.push(incident);assert.equal(applyCitizenIncident(state,incident).ok,true);system.update(state);
  });
  try {
    ...
    drag.wait(.5);assert.equal(incidents.length,0);
    drag.release();drag.wait(3);
    assert.equal(incidents.length,1);
    assert.equal(incidents[0].kind,'abduction');assert.equal(incidents[0].witnessed,false);
    assert.equal(state.stats.population,before.population-1);
    assert.equal(state.citizenEffects.populationLoss,before.loss+1);
    assert.equal(state.citizenEffects.happinessPenalty,0);assert.equal(state.stats.happiness,before.happiness);
    assert.equal(state.citizenEffects.incidents.length,1);
    assert.equal(system.getDebug().count,0);assert.equal(system.holding,false);
...
test('gently relocated residents resume walking on their new disconnected street',()=>{
  const state=isolatedCity([[5,10],[28,33]]),incidents:Incident[]=[];
  ...
  try {
    ...
    const targetX=(first+last)/2-state.size/2+.5;
    ...
    drag.wait(.4);drag.release();
    assert.equal(system.getDebug().ragdolls,0);assert.equal(incidents.length,0);
    const landing=system.getDebug().positions[0];
    assert.ok(Math.abs(landing.x-targetX)<.7,'Resident should be placed on the destination street');
    const visited:number[]=[];
    for(let frame=0;frame<900;frame++) {
      system.animate(1/60,true);
      const point=system.getDebug().positions[0];visited.push(point.x);
      assert.ok(point.x>=first-state.size/2&&point.x<=last-state.size/2+1,
        `Resident left the destination street for the old neighbourhood at x=${point.x}`);
      assert.ok(Math.abs(point.z-person.z)<.5);
    }
    assert.ok(Math.max(...visited)-Math.min(...visited)>.5,'Resident should resume walking, not freeze at the drop location');
