...
test('T and four-way crossings discover every external incoming approach once', () => {
  for (const count of [3, 4]) {
    ...
    const junction = network.junctions[0]; assert.deepEqual(junction.cells, [{ x: 0, z: 0 }]);
    assert.equal(junction.approaches.length, count); assert.equal(network.approaches.size, count);
    for (const approach of junction.approaches) {
      assert.equal(approach.id, approachKey(approach.from, approach.entry));
      assert.equal(approach.junctionId, junction.id); assert.equal(network.junctionAt.get('0,0'), junction);
...
test('adjacent crossings share one controller with no internal approach or exit', () => {
  const network = roads([[-1, 0], [0, 0], [1, 0], [2, 0], [0, -1], [1, 1]]);
  assert.equal(network.junctions.length, 1);
  const junction = network.junctions[0];
  assert.deepEqual(junction.cells, [{ x: 0, z: 0 }, { x: 1, z: 0 }]); assert.equal(junction.approaches.length, 4);
  for (const approach of junction.approaches) assert.equal(network.junctionAt.has(roadPointKey(approach.from)), false);
});
...
test('a one-tile connector between crossings is included in their shared occupied space', () => {
  ...
  assert.equal(network.junctions.length, 1);
  assert.deepEqual(network.junctions[0].cells, [{ x: 0, z: 0 }, { x: 1, z: 0 }, { x: 2, z: 0 }]);
...
test('crossings with two full intervening road tiles retain separate controllers', () => {
  ...
  assert.equal(network.junctionAt.has('1,0'), false); assert.equal(network.junctionAt.has('2,0'), false);
...
test('junction IDs, approach order and planned routes do not depend on tile insertion order', () => {
  ...
  const first = roads(points), reversed = roads([...points].reverse());
  assert.deepEqual(first.junctions, reversed.junctions);
  for (const a of first.junctions[0].approaches) for (let choice = 0; choice < 5; choice++) {
    assert.deepEqual(planJunctionMovement(first, a.from, a.entry, choice), planJunctionMovement(reversed, a.from, a.entry, choice));
  }
});

test('each approach can reach every other external exit along a contiguous complete cluster route', () => {
  for (const spacing of [1, 2]) {
    const network = roads(unique([...crossAt(), ...crossAt(spacing)])), junction = network.junctions[0];
    for (const approach of junction.approaches) {
      ...
      for (let choice = 0; choice < junction.approaches.length - 1; choice++) {
        ...
        assert.equal(movement.junctionId, junction.id); assert.equal(movement.approachId, approach.id);
        assert.deepEqual(movement.path[0], approach.entry); assert.deepEqual(movement.path.at(-1), movement.exit);
        assert.notDeepEqual(movement.exit, approach.from); contiguous([approach.from, ...movement.path]);
        for (const p of movement.path.slice(0, -1)) assert.equal(network.junctionAt.get(roadPointKey(p))?.id, junction.id);
        assert.equal(network.junctionAt.has(roadPointKey(movement.exit)), false);
        const beforeExit = movement.path.at(-2)!;
        assert.deepEqual(movement.exitDirection, { x: movement.exit.x - beforeExit.x, z: movement.exit.z - beforeExit.z });
...
test('ordinary roads and removed entrances cannot produce stale junction movements', () => {
  const from = { x: -1, z: 0 }, entry = { x: 0, z: 0 };
  const initial = roads(crossAt()); assert.ok(planJunctionMovement(initial, from, entry));
...
test('driveway turns pre-plan contiguous cluster routes through a real exit before admission',()=>{
  ...
  const m=planExternalMovement(network,{x:0,z:0},{x:1,z:0},42);assert.ok(m);
  assert.deepEqual(m.path.slice(0,2),[{x:0,z:0},{x:1,z:0}]);contiguous(m.path);
  assert.equal(network.junctionAt.has(roadPointKey(m.exit)),false);assert.equal(m.approachId,'external:42');
});
