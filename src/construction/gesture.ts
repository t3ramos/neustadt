...
export const FACILITY_TOOLS = new Set<Tool>([
  'power',
  'waterpump',
  'police',
  'fire',
...
export const SINGLE_TILE_TOOLS = new Set<Tool>([
  'inspect',
  'pan',
  'citizen',
  ...LINE_TOOLS,
...
export class ConstructionStroke {
  ...
  get area(): [number, number] | undefined {
    return ZONE_TOOLS.has(this.tool)
      ? [
          Math.abs(this.endpoint.x - this.anchor.x) + 1,
          Math.abs(this.endpoint.z - this.anchor.z) + 1,
        ]
      : undefined;
  }
  ...
  update(point: Point): void {
    if (
      (point.x === this.endpoint.x && point.z === this.endpoint.z) ||
    ...
    if (!ZONE_TOOLS.has(this.tool) && !(this.tool === 'road' && this.roadRoute)) {
      ...
      if (LINE_TOOLS.has(this.tool)) {
        ...
        while (ix < ax || iz < az) {
          if (
            ix < ax &&
            (iz >= az || (ix + 0.5) / Math.max(ax, 1) <= (iz + 0.5) / Math.max(az, 1))
          ) {
            x += Math.sign(dx);
            ix++;
          } else {
        ...
        for (let step = 1; step <= steps; step++)
          this.add({
            ...
            z: Math.round(from.z + (dz * step) / steps),
          });
      }
    }
    this.endpoint = { ...point };
  ...
  private add(point: Point): void {
    for (const cell of brushFootprint(point, this.tool, this.brush, this.size))
      this.cells.set(cell.z * this.size + cell.x, cell);
  }
}
