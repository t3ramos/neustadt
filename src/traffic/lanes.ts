...
export const ROAD_LANE_OFFSET = 0.16;
...
type Controls =
  readonly [Point, Point] | readonly [Point, Point, Point] | readonly [Point, Point, Point, Point];
interface LaneCurve {
  controls: Controls;
  incoming: Point;
  outgoing: Point;
...
function laneCurve(previous: Point, from: Point, to: Point, laneOffset: number): LaneCurve {
  const incomingDirection = direction(previous, from),
    outgoingDirection = direction(from, to);
  ...
  const entry = {
    x: -0.5 * incoming.x - incoming.z * offset,
    z: -0.5 * incoming.z + incoming.x * offset,
  };
  const exit = {
    x: 0.5 * outgoing.x - outgoing.z * offset,
    z: 0.5 * outgoing.z + outgoing.x * offset,
  };
  ...
  if (Math.abs(cross) < EPSILON && dot > 0) {
    ...
    controls =
      reach > EPSILON && exitReach > EPSILON && reach <= 2 && exitReach <= 2
        ? [entry, control, exit]
        : [
            entry,
            { x: entry.x + incoming.x * 0.5, z: entry.z + incoming.z * 0.5 },
            { x: exit.x - outgoing.x * 0.5, z: exit.z - outgoing.z * 0.5 },
            exit,
          ];
  } else {
...
export function lanePathLength(
  previous: Point,
  from: Point,
  to: Point,
  laneOffset = ROAD_LANE_OFFSET,
): number {
...
export function lanePose(
  ...
  const point = curvePoint(curve.controls, t),
    tangent = curveTangent(curve, t);
  return {
    x: from.x + 0.5 + point.x,
    z: from.z + 0.5 + point.z,
    tangentX: tangent.x,
    tangentZ: tangent.z,
