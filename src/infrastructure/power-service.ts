...
export const powerPoleCenter = (tile: Tile): [number, number] => {
  const offset = tile.kind === 'road' ? 0.86 : 0.5;
...
function lotTerminal(footprint: Point[], pole: Tile): [number, number] {
  ...
  if (dx >= dz && dx > 0)
    return [x < minX ? minX + 0.05 : maxX - 0.05, Math.max(minZ + 0.5, Math.min(maxZ - 0.5, z))];
  return [Math.max(minX + 0.5, Math.min(maxX - 0.5, x)), z < minZ ? minZ + 0.05 : maxZ - 0.05];
}
...
function crossesAsphalt(
  ...
  return (
    intersects(0.165, 0.165, 0.835, 0.835) ||
    ((north || isolated) && intersects(0.165, 0, 0.835, 0.34)) ||
    ((south || isolated) && intersects(0.165, 0.66, 0.835, 1)) ||
    (east && intersects(0.66, 0.165, 1, 0.835)) ||
    (west && intersects(0, 0.165, 0.34, 0.835))
  );
}

export function isPowerServiceDropClear(
  state: CityState,
  pole: Tile,
  target: [number, number],
  ...
  const start: [number, number] = [poleX, poleZ + 0.077];
  const minX = Math.max(0, Math.floor(Math.min(start[0], target[0]) - 0.007));
  const maxX = Math.min(state.size - 1, Math.floor(Math.max(start[0], target[0]) + 0.007));
  const minZ = Math.max(0, Math.floor(Math.min(start[1], target[1]) - 0.007));
  const maxZ = Math.min(state.size - 1, Math.floor(Math.max(start[1], target[1]) + 0.007));
  for (let z = minZ; z <= maxZ; z++)
    for (let x = minX; x <= maxX; x++) {
      ...
      if (
        !footprintIds.has(id) &&
        obstructs(tile) &&
        intersectsRectangle(start, target, x, z, x + 1, z + 1)
      )
        return false;
    }
  return true;
...
export function powerServiceCandidates(
  state: CityState,
  footprint: Point[],
  obstructs: (tile: Tile) => boolean,
): PowerServiceCandidate[] {
...
export function selectPowerServiceComponent(
  ownComponent: number,
  membership: Int32Array,
  supply: number[],
  candidates: PowerServiceCandidate[],
  resolve = (component: number) => component,
): number {
  ownComponent = ownComponent >= 0 ? resolve(ownComponent) : ownComponent;
  if (ownComponent >= 0 && supply[ownComponent] > 0) return ownComponent;
  let best = ownComponent,
    bestSupply = 0;
  for (const candidate of candidates) {
    const member = membership[candidate.pole],
      component = member >= 0 ? resolve(member) : member;
    if (component >= 0 && supply[component] > bestSupply) {
      best = component;
      bestSupply = supply[component];
    }
  }
  return best;
}
...
export function applyFacilityPowerFeeds(
  ...
  for (const tile of [...facilities].sort(
    (a, b) => a.z * state.size + a.x - b.z * state.size - b.x,
  )) {
    const building = tile.z * state.size + tile.x,
      component = membership[building];
  ...
  for (let cursor = 0; cursor < queue.length; cursor++) {
    for (const feed of watchers.get(queue[cursor]) ?? []) {
      ...
      const candidate = feed.candidates.find(
        (item) => membership[item.pole] >= 0 && root(membership[item.pole]) === from,
      )!;
      incoming.set(feed.building, candidate);
      parent[own] = from;
      supply[from] += supply[own];
      supply[own] = 0;
      queue.push(own);
    }
  }
  for (let id = 0; id < membership.length; id++)
    if (membership[id] >= 0) membership[id] = root(membership[id]);
  return incoming;
}
