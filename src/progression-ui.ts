...
import { tr, localeCode, formatNumber, formatCurrency } from './i18n';
import {
...
const friendlyTool=(tool:Tool)=>TOOL_DEFS[tool]?.name??({inspect:tr('Untersuchen','Inspect'),pan:tr('Kamera','Camera'),bulldoze:tr('Abreißen','Demolish'),raise:tr('Anheben','Raise'),lower:tr('Absenken','Lower'),level:tr('Einebnen','Flatten'),pipe:tr('Wasserrohre','Water pipes'),powerline:tr('Stromleitungen','Power lines'),wind:tr('Windkraft','Wind power'),solar:tr('Solarpark','Solar farm'),university:tr('Universität','University'),recycling:tr('Recyclingzentrum','Recycling center')} as Partial<Record<Tool,string>>)[tool]??tool;
const reward=(money:number,xp:number)=>`<span class="progression-reward">${icon('coins')} ${formatCurrency(money)}</span><span class="progression-reward progression-xp">${icon('sparkles')} ${format(xp)} ${tr('EP','XP')}</span>`;
...
export function renderProgressionSidebar(state:CityState):string {
  ...
  const populationLabel=rank.next?`${format(state.stats.population)} / ${format(rank.next.population)} ${tr('Einwohner','residents')}`:`${format(state.stats.population)} ${tr('Einwohner','residents')}`;
...
function questCard(state:CityState,quest:QuestDefinition,index:number):string {
  ...
  const status=claimed?tr('Abgeholt','Claimed'):progress.complete?tr('Belohnung bereit','Reward ready'):tr('In Arbeit','In progress');
...
function goalContent(state:CityState):string {
  ...
  return `<div class="progression-goal-hero ${campaign.complete?'is-victorious':''}"><span class="progression-goal-symbol">${icon(campaign.complete?'trophy':'trees')}</span><span class="progression-goal-eyebrow">${campaign.complete?tr('STADTZIEL GEMEISTERT','CITY GOAL ACHIEVED'):tr('DEINE VISION FÜR ','YOUR VISION FOR ')+escape(state.name.toLocaleUpperCase(localeCode()))}</span><h3>${campaign.complete?tr('Du hast Zukunft gebaut.','You built a better future.'):tr(`Ein guter Ort. Für ${format(25000)} Menschen.`,`A place to call home. For ${format(25000)} people.`)}</h3><p>${campaign.complete?tr(`${escape(state.name)} ist eine lebenswerte Zukunftsstadt. Dein Erfolg bleibt bestehen. Baue weiter, entdecke neue Stadtteile und meistere die Challenges.`,`${escape(state.name)} is a thriving city of tomorrow. Your achievement is permanent. Keep building, discover new neighborhoods and take on challenges.`):tr('Entwickle eine Metropole, in der Menschen gerne leben. Halte alle Ziele sechs Monate lang gleichzeitig und erwirtschafte dabei jeden Monat einen Überschuss.','Develop a metropolis where people love to live. Meet every target for six consecutive months while earning a surplus each month.')}</p>${campaign.complete?`<span class="progression-freeplay-badge">${icon('infinity')} ${tr('Freies Spiel · Deine Geschichte geht weiter','Free play · Your story continues')}</span>`:`<div class="progression-goal-totals"><span><strong>${format(state.stats.population)}</strong> / ${format(25000)} ${tr('Einwohner','residents')}</span><span><strong>${format(Math.min(6,happyMonths))}</strong> / 6 ${tr('stabile Monate','stable months')}</span></div>`}</div>
    ...
    <div class="progression-goal-budget ${state.stats.balance>0?'positive':'negative'}">${icon('landmark')}<div><strong>${tr('Monatlicher Haushalt','Monthly budget')}</strong><span>${state.stats.balance>0?tr('Die Stadt erzielt einen Überschuss.','The city is earning a surplus.'):tr('Für das Stadtziel brauchst du einen positiven monatlichen Haushalt.','The city goal requires a monthly budget surplus.')}</span></div><b>${state.stats.balance>0?'+':''}${formatCurrency(state.stats.balance)}</b></div>
...
export function renderProgressionDialog(state:CityState,tab:ProgressionTab='quests'):string {
  const tabs:{id:ProgressionTab;name:string;icon:string}[]=[{id:'quests',name:tr('Aufträge','Quests'),icon:'list-checks'},{id:'ranks',name:tr('Ausbaustufen','City stages'),icon:'flag'},{id:'challenges',name:tr('Challenges','Challenges'),icon:'medal'},{id:'goal',name:tr('Stadtziel','City goal'),icon:'trophy'}];
