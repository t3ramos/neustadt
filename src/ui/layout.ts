...
export function placeFloatingPanel(
  ...
  const ys = [
    pointer.y - 40,
    pointer.y - height - gap,
    pointer.y + gap,
    bounds.top,
    bounds.bottom - height,
  ];
  for (const obstacle of obstacles) {
    xs.push(obstacle.right + 8, obstacle.left - width - 8);
    ys.push(obstacle.bottom + 8, obstacle.top - height - 8);
  }
  const pointerArea = {
    left: pointer.x - 14,
    top: pointer.y - 14,
  ...
  for (const x of xs)
    for (const y of ys) {
      const left = clamp(x, bounds.left, bounds.right - width),
        top = clamp(y, bounds.top, bounds.bottom - height);
      const rect = { left, top, right: left + width, bottom: top + height };
      const score =
        overlap(rect, pointerArea) * 1000 +
        obstacles.reduce((total, obstacle) => total + overlap(rect, obstacle), 0) * 10 +
        Math.hypot(left - pointer.x - gap, top - pointer.y + 40);
