import { perMinute,ECONOMY_STEP_SECONDS } from './game-time';
import { initializeWeather, chooseWeather } from './weather-cycle';
...
export function createCity(seed=2026,empty=false,size=128):CityState {
 ...
 const state:CityState={version:2,citizenEffects:{populationLoss:0,happinessPenalty:0,incidents:[]},name:'Lindenbucht',size,seed,tiles:[],money:85000,month:0,tickProgress:0,speed:1,tax:9,funding:{police:100,fire:100,health:100,education:100},loan:0,stats:blankStats(),events:[],history:[],revision:0,milestone:0,progression:createProgression(),settings:{disastersEnabled:false,weather:'clear',dynamicWeather:true,dayNightCycle:true,timeOfDay:14,buildingLights:true}};
...
export function tick(state:CityState):void {
 ...
 if(state.month%12===0)addEvent(state,['Stadtbilanz','City balance'],[`${state.stats.population.toLocaleString('de-DE')} Einwohner · ${state.stats.happiness} % Zufriedenheit · ${state.stats.balance>=0?'+':''}${perMinute(state.stats.balance).toLocaleString('de-DE')} € pro Spielminute.`,`${state.stats.population.toLocaleString('en-US')} residents · ${state.stats.happiness}% happiness · ${state.stats.balance>=0?'+':''}€${perMinute(state.stats.balance).toLocaleString('en-US')} per simulation minute.`],state.stats.balance>=0?'good':'warning');
...
export function triggerDisaster(state:CityState,kind:DisasterKind):BuildResult {
 ...
 if(!['fire','earthquake','storm'].includes(kind))return result(false,tr('Diese Katastrophe ist nicht verfügbar.','This disaster is not available.'));
 ...
 else {let count=0;const radius=kind==='storm'?8:6;const victims=candidates.filter(t=>Math.abs(t.x-target.x)+Math.abs(t.z-target.z)<=radius&&(t===target||random(state.seed,t.x,t.z,state.month+1)<(kind==='storm'?.25:.65)));
...
export function expandCity(original:CityState,newSize=128):CityState {
 ...
 recalculate(state);state.money=original.money;state.month=original.month;state.tickProgress=original.tickProgress??0;state.progression.unlocked=[...new Set([...state.progression.unlocked,...legacyKinds])];
...
export function deserializeCity(raw:string):CityState {
 ...
 if(data.tickProgress===undefined)state.tickProgress=0;else {requireNumber(data.tickProgress,tr('Spielzeit','Play time'),0,ECONOMY_STEP_SECONDS);if((data.tickProgress as number)>=ECONOMY_STEP_SECONDS)throw new Error(tr('Ungültiger Spielstand: Spielzeit.','Invalid saved game: play time.'));}
