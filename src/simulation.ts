...
const definition=(name:string,cost:number,upkeep:number,category:ToolDefinition['category'],color:string,icon:string,description:string,footprint?:[number,number]):ToolDefinition=>({name,cost,upkeep,category,color,icon,description,...(footprint?{footprint}:{})});
export const TOOL_DEFS:Partial<Record<Tool,ToolDefinition>>={
 residential:definition('Wohngebiet',40,0,'zones','#75b87c','House','Wohnraum wächst bei Straßen-, Strom- und Wasseranschluss.'),
 commercial:definition('Gewerbegebiet',60,0,'zones','#68a8dc','Store','Geschäfte, Büros und Arbeitsplätze. Benötigt Kunden und Versorgung.'),
 industrial:definition('Industriegebiet',70,0,'zones','#d7b35c','Factory','Fabriken, Lagerhallen und Werkstätten schaffen Jobs und Umweltbelastung.'),
 road:definition('Straße',18,.4,'transport','#899398','Route','Erschließt Grundstücke. Stromleitungen und Wasserrohre werden separat verlegt. Über Wasser entsteht eine Brücke.'),
 rail:definition('Bahnstrecke',35,.6,'transport','#a4abb0','TrainFront','Entlastet den Straßenverkehr im Umkreis. Brückenbau über Wasser möglich.'),
 pipe:definition('Wasserrohr',8,.08,'utilities','#63bdd1','Droplets','Unterirdisches Leitungsnetz. Versorgt Grundstücke bis zwei Felder Abstand. Mit dem Wasserwerk verbinden.'),
 powerline:definition('Stromleitung',12,.08,'utilities','#edbd67','Cable','Eigenes Stromnetz mit sichtbaren Masten im Freiland. Kann unter Straßen und Gebäuden verlaufen.'),
 power:definition('Kraftwerk',6500,320,'utilities','#edbd67','Zap','4 × 4 Felder. Liefert 6.000 Stromeinheiten an angeschlossene Leitungen.',[4,4]),
 waterpump:definition('Wasserwerk',2200,180,'utilities','#63bdd1','Droplets','2 × 2 Felder. Mit Stromanschluss 6.000 Einheiten Wasser im eigenen Rohrnetz.',[2,2]),
 wind:definition('Windpark',3600,65,'utilities','#b6d2c3','Wind','2 × 2 Felder. Sauberer Strom: 1.200 Einheiten ohne Luftbelastung.',[2,2]),
 solar:definition('Solarpark',6200,100,'utilities','#84a9d1','Sun','4 × 3 Felder. Saubere Energie mit Speichersystem: 3.200 Einheiten.',[4,3]),
 park:definition('Stadtpark',150,5,'nature','#7faf71','Trees','Erhöht Grundstückswert und Lebensqualität in der Nachbarschaft.'),
 tree:definition('Bäume',12,0,'nature','#608765','TreePine','Begrünt freie Flächen und verbessert die lokale Luftqualität.'),
 police:definition('Polizeiwache',1900,125,'services','#79a8c1','Shield','2 × 2 Felder. Sicherheit im Umkreis, abhängig von Versorgung und Polizeibudget.',[2,2]),
 fire:definition('Feuerwache',1600,95,'services','#d58569','Flame','3 × 2 Felder. Versorgte Löschzüge verhindern Brandschäden in der Nachbarschaft.',[3,2]),
 hospital:definition('Klinik',3200,150,'services','#cf97a9','HeartPulse','3 × 3 Felder. Gesundheitsversorgung für die umliegenden Stadtviertel.',[3,3]),
 school:definition('Schule',1800,100,'services','#bba5cf','GraduationCap','3 × 2 Felder. Bildung fördert Wachstum und Zufriedenheit.',[3,2]),
 university:definition('Universität',12000,280,'services','#bba5cf','BookOpen','5 × 4 Felder. Forschung, 160 Arbeitsplätze und höhere Bildung im weiten Umkreis.',[5,4]),
 recycling:definition('Recyclinghof',5200,110,'services','#89b394','Recycle','3 × 3 Felder. Reduziert die Umweltbelastung in einem großen Umkreis.',[3,3]),
 stadium:definition('Stadion',9000,180,'special','#b7c08a','Trophy','6 × 5 Felder. Sportarena mit 120 Jobs und höherer Wohnraumnachfrage.',[6,5]),
 airport:definition('Flughafen',16000,290,'special','#abc2cf','Plane','10 × 6 Felder. Terminal und Startbahn, 180 Jobs und höhere Gewerbenachfrage.',[10,6]),
 seaport:definition('Hafen',9500,160,'special','#81aeb8','Ship','5 × 3 Felder. Kaimauer muss direkt ans Wasser grenzen. Mit R drehen. 120 Jobs und Industriebonus.',[5,3]),
 raise:definition('Gelände anheben',35,0,'terrain','#adba88','Mountain','Hebt freies Gelände um 5 Meter an. Wasser lässt sich zu Bauland aufschütten.'),
 lower:definition('Gelände absenken',35,0,'terrain','#8aaab6','ArrowDownToLine','Senkt freies Gelände um 5 Meter ab. Unter dem Meeresspiegel entsteht Wasser.'),
 level:definition('Gelände einebnen',35,0,'terrain','#bcab85','AlignVerticalDistributeCenter','Gleicht freies Gelände an die Höhe des zuerst angeklickten Felds an. 35 € je 5 Meter und Feld.'),
};
...
function addEvent(state:CityState,title:string,message:string,type:GameEvent['type']='info'):void {state.events.unshift({id:(state.events[0]?.id??0)+1,month:state.month,title,message,type});state.events=state.events.slice(0,40);}
...
export function createCity(seed=2026,empty=false,size=128):CityState {
 ...
 if(!empty){
  ...
  addEvent(state,'Willkommen in Lindenbucht','Deine Stadt ist bereit. Neue Viertel, Forschung und Stadtaufträge warten auf dich. Straßen, Stromleitungen und Wasserrohre bilden eigene Netze.','good');
 }else addEvent(state,'Ein neuer Anfang','Baue Straßen, Kraftwerk und Wasserwerk. Verbinde Stromleitungen und Wasserrohre mit den Anlagen und erschließe Wohn- und Arbeitsgebiete.');
...
export function recalculate(state:CityState):void {
 ...
 const power=makeNetwork(state,t=>t.fire===0&&(t.hasPowerLine||isFacility(t.kind)||isZone(t.kind)&&t.level>0)),water=makeNetwork(state,t=>t.hasPipe||t.kind==='waterpump');
...
function planBuild(state:CityState,points:Point[],tool:Tool,options:BuildOptions={}):BuildPlan {
 const rotation=options.rotation??0,out:BuildPlan={cost:0,valid:[],invalid:[],message:'Hier ist kein Bau möglich.',count:0,changes:[],rotation};
 if(![0,1,2,3].includes(rotation)){out.message='Ungültige Ausrichtung.';return out;}
 if(tool==='inspect'||tool==='pan'||tool!=='bulldoze'&&!TOOL_DEFS[tool]){out.message='Wähle ein Bauwerkzeug.';return out;}
 if(!isToolUnlocked(state,tool)){out.message='Dieses Bauwerk wird durch den Stadtaufstieg freigeschaltet.';out.invalid=points.map(point);return out;}
 ...
 if(tool==='level'&&(target===undefined||!Number.isFinite(target)||target<MIN_ELEVATION||target>MAX_ELEVATION||Math.abs(target/TERRAIN_STEP-Math.round(target/TERRAIN_STEP))>.001)){out.message='Wähle eine gültige Geländehöhe in 5-Meter-Schritten.';out.invalid=points.map(point);return out;}
 if(TOOL_DEFS[tool]?.footprint){
  ...
  for(let z=p.z;z<p.z+d;z++)for(let x=p.x;x<p.x+w;x++){const p2={x,z};members.push(p2);const t=tileAt(state,x,z);if(!t)reject(p2,'Das gesamte Gebäude muss innerhalb der Karte liegen.');else if(t.elevation<0)reject(p2,'Das Gebäude benötigt eine vollständig trockene Grundfläche.');else if(!['empty','tree','rubble'].includes(t.kind))reject(p2,'Das gesamte Baufeld muss frei sein. Reiße die Bebauung zuerst ab.');else if(Math.abs(t.elevation-(base??0))>.001)reject(p2,'Ebne die gesamte Grundfläche vor dem Bau ein.');}
  if(!out.invalid.length&&tool==='seaport'){
   ...
   if(edge.filter(e=>(tileAt(state,e.x,e.z)?.elevation??0)<0).length<2){out.invalid=members;out.message='Die Kaimauer muss mit mindestens zwei Feldern direkt ans Wasser grenzen. Drehe den Hafen mit R.';}
  }
  ...
  if(!inBounds(state,p.x,p.z)){reject(p,'Außerhalb des Stadtgebiets.');continue;}const t=tileAt(state,p.x,p.z)!,id=p.z*state.size+p.x;if(seen.has(id))continue;seen.add(id);
  if(tool==='bulldoze'){
   const root=rootTile(state,t),rootId=root.z*state.size+root.x;if(rootId!==id&&seen.has(rootId))continue;seen.add(rootId);
   if(['empty','water'].includes(t.kind)&&!t.hasPipe&&!t.hasPowerLine){reject(p,'Hier gibt es nichts abzureißen.');continue;}
  ...
  if(tool==='pipe'||tool==='powerline'){
   if(tool==='pipe'?t.hasPipe:t.hasPowerLine){reject(p,'Diese Leitung ist bereits vorhanden.');continue;}add({points:[point(p)],cost:TOOL_DEFS[tool]!.cost+(t.elevation<0?12:0)});continue;
  }
  if(terrain){
   if(!['empty','tree','water'].includes(t.kind)){reject(p,'Gelände unter Bebauung kann nicht verändert werden.');continue;}
   const elevation=tool==='level'?target!:t.elevation+(tool==='raise'?TERRAIN_STEP:-TERRAIN_STEP);
   if(elevation<MIN_ELEVATION||elevation>MAX_ELEVATION){reject(p,'Die maximale Geländehöhe oder Wassertiefe ist erreicht.');continue;}
   if(Math.abs(elevation-t.elevation)<.001){reject(p,'Dieses Feld hat bereits die Zielhöhe.');continue;}
   add({points:[point(p)],cost:Math.round(Math.abs(elevation-t.elevation)/TERRAIN_STEP)*TOOL_DEFS[tool]!.cost,elevation});continue;
  }
  if(t.kind===tool){reject(p,'Dieses Grundstück ist bereits bebaut.');continue;}
  if(t.elevation<0&&tool!=='road'&&tool!=='rail'){reject(p,'Hier ist Wasser. Baue an Land oder errichte eine Brücke.');continue;}
  if(!['empty','tree','rubble','water'].includes(t.kind)){reject(p,'Reiße die vorhandene Bebauung zuerst ab.');continue;}
  add({points:[point(p)],cost:TOOL_DEFS[tool]!.cost+(t.elevation<0?90:t.kind==='tree'?2:0)});
 }
 if(out.count){out.message=state.money<out.cost?`Nicht genügend Geld. Benötigt: ${out.cost.toLocaleString('de-DE')} €.`:`${out.count} ${TOOL_DEFS[tool]?.footprint?'Gebäude':'Felder'} · ${out.cost.toLocaleString('de-DE')} €${out.invalid.length?` · ${out.invalid.length} Felder nicht bebaubar`:''}`;}
 return out;
}
...
export function build(state:CityState,points:Point[],tool:Tool,options:BuildOptions={}):BuildResult {
 ...
 const label=tool==='bulldoze'?'abgerissen':['raise','lower','level'].includes(tool)?'bearbeitet':'gebaut';return result(true,`${plan.count} ${TOOL_DEFS[tool]?.footprint?'Gebäude':'Felder'} ${label} · ${plan.cost.toLocaleString('de-DE')} €`,plan.cost,plan.count);
}
...
export function tick(state:CityState):void {
 ...
 if(destroyed)addEvent(state,'Brandschäden',`${destroyed} Gebäude wurden zerstört. Räume die Grundstücke und baue die Feuerwehr aus.`,'warning');
 recalculate(state);state.money=clamp(state.money+state.stats.balance,-100000,1000000000);
 const milestones=[2500,5000,10000,20000,40000];while(state.milestone<milestones.length&&state.stats.population>=milestones[state.milestone]){const population=milestones[state.milestone],reward=[5000,10000,15000,25000,40000][state.milestone];state.money=Math.min(1000000000,state.money+reward);state.milestone++;addEvent(state,'Eine Stadt wächst',`${population.toLocaleString('de-DE')} Einwohner! Das Land fördert deine Stadt mit ${reward.toLocaleString('de-DE')} €.`, 'good');}
 if(state.month%12===0)addEvent(state,'Jahresbericht',`${state.stats.population.toLocaleString('de-DE')} Einwohner · ${state.stats.happiness} % Zufriedenheit · ${state.stats.balance>=0?'+':''}${state.stats.balance.toLocaleString('de-DE')} € monatlich.`,state.stats.balance>=0?'good':'warning');
 if(state.money<0&&state.month%3===0)addEvent(state,'Die Stadtkasse ist im Minus','Erhöhe Steuern, reduziere Ausgaben oder nimm einen Kredit auf.','warning');
 if(oldPopulation>100&&state.stats.population<oldPopulation*.9&&state.month%3===0)addEvent(state,'Einwohner ziehen fort','Prüfe Straßenanschluss, Stromleitungen, Wasserrohre und Arbeitsplätze.','warning');
...
export function takeLoan(state:CityState):BuildResult {if(state.loan>=50000)return result(false,'Das Kreditlimit von 50.000 € ist erreicht.');const amount=Math.min(10000,50000-state.loan);state.loan+=amount;state.money=Math.min(1000000000,state.money+amount);state.progression.counters.loansTaken=(state.progression.counters.loansTaken??0)+1;recalculate(state);addEvent(state,'Kredit ausgezahlt',`${amount.toLocaleString('de-DE')} € wurden ausgezahlt. Monatlicher Zins: 0,5 %.`);return result(true,`${amount.toLocaleString('de-DE')} € Kredit aufgenommen.`,-amount,1);}
export function repayLoan(state:CityState):BuildResult {if(state.loan<=0)return result(false,'Die Stadt hat keine offenen Kredite.');const amount=Math.min(10000,state.loan);if(state.money<amount)return result(false,'Nicht genügend Geld für die Rückzahlung.',amount);state.loan-=amount;state.money-=amount;recalculate(state);return result(true,`${amount.toLocaleString('de-DE')} € Kredit zurückgezahlt.`,amount,1);}
export function triggerDisaster(state:CityState,kind:DisasterKind):BuildResult {
 if(!state.settings.disastersEnabled)return result(false,'Aktiviere zuerst den Katastrophenmodus in den Einstellungen.');
 if(!['fire','earthquake','flood','storm'].includes(kind))return result(false,'Diese Katastrophe ist nicht verfügbar.');
 const candidates=state.tiles.filter(t=>hasStructure(t)&&isBuildingAnchor(state,t));
 if(!candidates.length){addEvent(state,'Keine Schäden','Im betroffenen Gebiet stehen noch keine Gebäude.');state.revision++;return result(false,'Es gibt noch keine Gebäude.');}
 ...
 if(kind==='fire'){for(const p of getFootprint(state,target))tileAt(state,p.x,p.z)!.fire=6;addEvent(state,'Brand in der Stadt',`Ein Gebäude bei ${target.x}, ${target.z} brennt. Eine versorgte Feuerwache begrenzt die Schäden.`,'warning');}
  ...
  const label=kind==='earthquake'?'Erdbeben':kind==='flood'?'Überschwemmung':'Schwerer Sturm';addEvent(state,label,`${count} Gebäude beschädigt, ${broken} Leitungsfelder unterbrochen. Räume Trümmer, repariere die Netze und stelle die Versorgung wieder her.`,'warning');
 }
 state.progression.counters.disastersTriggered=(state.progression.counters.disastersTriggered??0)+1;recalculate(state);updateProgression(state);return result(true,'Katastrophe ausgelöst.',0,1);
}
...
export function expandCity(original:CityState,newSize=128):CityState {
 if(![64,96,128].includes(newSize)||newSize<original.size)throw new Error('Eine Stadt kann nur auf 64, 96 oder 128 Felder erweitert werden.');
 ...
 addEvent(state,'Neue Horizonte',`Das Stadtgebiet umfasst jetzt ${newSize} × ${newSize} Felder. Bestehende Wohn- und Arbeitsgebiete wurden erhalten; öffentliche Gebäude haben eigene große Grundstücke.`,'good');state.revision++;return state;
}
function validateCitizenIncident(value:unknown,size:number):Omit<CitizenIncident,'id'|'month'> {
 const data=requireRecord(value,'Einwohnerereignis');
 requireNumber(data.x,'Ereignisposition X',-8,size+8);requireNumber(data.z,'Ereignisposition Z',-8,size+8);requireNumber(data.y,'Ereignishöhe',-16,128);
 for(const axis of ['nx','ny','nz'])requireNumber(data[axis],'Oberflächennormale',-1,1);
 if(Math.hypot(data.nx as number,data.ny as number,data.nz as number)<.001)throw new Error('Ungültiger Spielstand: Oberflächennormale.');
 requireBoolean(data.witnessed,'Zeugenstatus');if(!['impact','abduction'].includes(data.kind as string))throw new Error('Ungültiger Spielstand: Einwohnerereignis.');
...
function validateCitizenEffects(value:unknown,size:number,month:number):void {
 const effects=requireRecord(value,'Einwohnerereignisse');requireNumber(effects.populationLoss,'Einwohnerverluste',0,2000000,true);requireNumber(effects.happinessPenalty,'Stimmungseinfluss',0,30,true);
 if(!Array.isArray(effects.incidents)||effects.incidents.length>64)throw new Error('Ungültiger Spielstand: Einwohnerereignisliste.');
 let previousId=0,previousMonth=0;for(const value of effects.incidents){validateCitizenIncident(value,size);const data=requireRecord(value,'Einwohnerereignis');const id=requireNumber(data.id,'Ereignisnummer',1,Number.MAX_SAFE_INTEGER,true),eventMonth=requireNumber(data.month,'Ereignismonat',0,month,true);if(id<=previousId||eventMonth<previousMonth)throw new Error('Ungültiger Spielstand: Ereignisreihenfolge.');previousId=id;previousMonth=eventMonth;}
}
...
export function applyCitizenIncident(state:CityState,incident:Omit<CitizenIncident,'id'|'month'>):BuildResult {
 try{validateCitizenIncident(incident,state.size);}catch{return result(false,'Ungültiges Einwohnerereignis.');}
 ...
 if(!Number.isSafeInteger(nextId))return result(false,'Der Ereigniszähler ist ausgeschöpft.');
 if(basePopulation-effects.populationLoss<=0)return result(false,'Die Stadt hat keine Einwohner.');
 ...
 return result(true,incident.witnessed?'Einwohner verloren. Die Zeugen sind beunruhigt.':'Ein Einwohner ist verschwunden.',0,1);
}
function requireRecord(value:unknown,label:string):Record<string,unknown> {if(!value||typeof value!=='object'||Array.isArray(value))throw new Error(`Ungültiger Spielstand: ${label}.`);return value as Record<string,unknown>;}
function requireNumber(value:unknown,label:string,min:number,max:number,integer=false):number {if(typeof value!=='number'||!Number.isFinite(value)||value<min||value>max||integer&&!Number.isInteger(value))throw new Error(`Ungültiger Spielstand: ${label}.`);return value;}
function requireString(value:unknown,label:string,max:number):string {if(typeof value!=='string'||!value.trim()||value.length>max)throw new Error(`Ungültiger Spielstand: ${label}.`);return value;}
function requireBoolean(value:unknown,label:string):void {if(typeof value!=='boolean')throw new Error(`Ungültiger Spielstand: ${label}.`);}
function validateProgression(value:unknown,month:number):void {
 const p=requireRecord(value,'Stadtfortschritt');requireNumber(p.xp,'Erfahrung',0,1000000000);requireNumber(p.rank,'Stadtrang',0,20,true);requireBoolean(p.victory,'Kampagnenziel');
 for(const key of ['completedQuests','claimedQuests','unlocked','completedChallenges']){if(!Array.isArray(p[key])||p[key].length>200)throw new Error('Ungültiger Spielstand: Fortschrittsliste.');for(const id of p[key] as unknown[])requireString(id,'Fortschrittskennung',100);}
 const tools=[...Object.keys(TOOL_DEFS),'empty','water','rubble','inspect','bulldoze','pan','citizen'];if((p.unlocked as string[]).some(t=>!tools.includes(t)))throw new Error('Ungültiger Spielstand: Bauwerkzeug.');
 const counters=requireRecord(p.counters,'Fortschrittszähler');if(Object.keys(counters).length>200)throw new Error('Ungültiger Spielstand: Fortschrittszähler.');for(const [key,v]of Object.entries(counters)){requireString(key,'Zählername',100);requireNumber(v,'Fortschrittszähler',-1000000000,1000000000);}
 if(p.activeChallenge!==null){const c=requireRecord(p.activeChallenge,'Herausforderung');requireString(c.id,'Herausforderung',100);requireNumber(c.startedMonth,'Startmonat',0,month,true);if(!['active','completed','failed'].includes(c.status as string))throw new Error('Ungültiger Spielstand: Herausforderung.');}
}
export function deserializeCity(raw:string):CityState {
 if(typeof raw!=='string'||raw.length>12000000)throw new Error('Der Spielstand ist zu groß oder ungültig.');let parsed:unknown;try{parsed=JSON.parse(raw);}catch{throw new Error('Der Spielstand enthält kein gültiges JSON.');}
 const data=requireRecord(parsed,'Dateiformat'),legacy=data.version===1,size=data.size as number;if(!legacy&&data.version!==2||legacy&&size!==40||![40,64,96,128].includes(size))throw new Error('Dieser Spielstand wird nicht unterstützt.');
 requireString(data.name,'Stadtname',64);requireNumber(data.seed,'Zufallswert',0,4294967295,true);requireNumber(data.money,'Stadtkasse',-100000,1000000000);requireNumber(data.month,'Monat',0,1000000,true);requireNumber(data.speed,'Geschwindigkeit',0,3,true);requireNumber(data.tax,'Steuern',0,25);requireNumber(data.loan,'Kredit',0,50000);requireNumber(data.revision,'Version',0,Number.MAX_SAFE_INTEGER,true);requireNumber(data.milestone,'Meilenstein',0,5,true);
 const funding=requireRecord(data.funding,'Budget');for(const key of ['police','fire','health','education'])requireNumber(funding[key],`Budget ${key}`,0,150);
 if(!Array.isArray(data.tiles)||data.tiles.length!==size*size)throw new Error('Ungültiger Spielstand: Kartenfelder.');
 for(let i=0;i<data.tiles.length;i++){
  const t=requireRecord(data.tiles[i],'Grundstück'),kind=t.kind as TileKind;if(t.x!==i%size||t.z!==Math.floor(i/size)||!KINDS.includes(kind))throw new Error('Ungültiger Spielstand: Grundstückskoordinaten oder Gebäude.');
  requireNumber(t.level,'Gebäudestufe',0,4,true);requireNumber(t.variation,'Gebäudevariante',0,1000000,true);requireNumber(t.age,'Gebäudealter',0,1000001,true);requireNumber(t.fire,'Brand',0,100,true);for(const key of ['pollution','landValue','traffic'])requireNumber(t[key],key,0,100);for(const key of ['powered','watered','connected'])requireBoolean(t[key],'Versorgungsdaten');
  if(legacy){const water=legacyWaterTerrain(t.x as number,t.z as number);if(kind==='water'&&!water||water&&!['water','road','rail','rubble'].includes(kind))throw new Error('Ungültiger Spielstand: Wasserfläche.');Object.assign(t,{elevation:water?-1:0,hasPipe:kind==='road',hasPowerLine:kind==='road',anchor:isFacility(kind)?i:-1,rotation:0});}
  else {requireNumber(t.elevation,'Geländehöhe',MIN_ELEVATION,MAX_ELEVATION);if(Math.abs((t.elevation as number)/TERRAIN_STEP-Math.round((t.elevation as number)/TERRAIN_STEP))>.001)throw new Error('Ungültiger Spielstand: Geländestufen.');requireBoolean(t.hasPipe,'Wasserrohr');requireBoolean(t.hasPowerLine,'Stromleitung');requireNumber(t.anchor,'Gebäudegrundfläche',-1,size*size-1,true);requireNumber(t.rotation,'Gebäudeausrichtung',0,3,true);if(kind==='water'&&(t.elevation as number)>=0||(t.elevation as number)<0&&!['water','road','rail','rubble'].includes(kind))throw new Error('Ungültiger Spielstand: Bebauung im Wasser.');}
  if(!isBuilding(kind)&&t.level!==0||isFacility(kind)&&t.level!==1)throw new Error('Ungültiger Spielstand: Gebäudestufe.');
  if(isFacility(kind)?(t.anchor as number)<0:t.anchor!==-1)throw new Error('Ungültiger Spielstand: Gebäudeanker.');
 }
 const state=data as unknown as CityState;
 for(const t of state.tiles){if(t.anchor<0)continue;const anchor=state.tiles[t.anchor];if(!anchor||anchor.anchor!==t.anchor||anchor.kind!==t.kind||anchor.rotation!==t.rotation||anchor.elevation!==t.elevation)throw new Error('Ungültiger Spielstand: Zusammengehörige Gebäudeteile.');const [w,d]=dimensions(t.kind,t.rotation);if(t.x<anchor.x||t.z<anchor.z||t.x>=anchor.x+w||t.z>=anchor.z+d)throw new Error('Ungültiger Spielstand: Gebäudegrundfläche.');}
 for(const t of state.tiles){if(!isFacility(t.kind)||!isBuildingAnchor(state,t))continue;const [w,d]=dimensions(t.kind,t.rotation),count=getFootprint(state,t).length;if(count!==1&&count!==w*d)throw new Error('Ungültiger Spielstand: Unvollständiges Gebäude.');}
 const stats=requireRecord(data.stats,'Statistik');for(const key of Object.keys(blankStats()))requireNumber(stats[key],`Statistik ${key}`,-1000000000,1000000000);
 if(!Array.isArray(data.events)||data.events.length>40)throw new Error('Ungültiger Spielstand: Meldungen.');for(const item of data.events){const e=requireRecord(item,'Meldung');requireNumber(e.id,'Meldungsnummer',0,Number.MAX_SAFE_INTEGER,true);requireNumber(e.month,'Meldungsmonat',0,data.month as number,true);requireString(e.title,'Meldungstitel',180);requireString(e.message,'Meldungstext',2000);if(!['info','good','warning'].includes(e.type as string))throw new Error('Ungültiger Meldungstyp.');}
 if(!Array.isArray(data.history)||data.history.length>120)throw new Error('Ungültiger Spielstand: Verlauf.');let previousMonth=-1;for(const item of data.history){const p=requireRecord(item,'Verlauf'),month=requireNumber(p.month,'Verlaufsmonat',0,data.month as number,true);if(month<=previousMonth)throw new Error('Ungültige Reihenfolge im Verlauf.');previousMonth=month;requireNumber(p.population,'Einwohnerzahl',0,2000000,true);requireNumber(p.money,'Kontostand',-100000,1000000000);requireNumber(p.happiness,'Zufriedenheit',0,100);}
 if(legacy){state.version=2;state.settings={disastersEnabled:false,weather:'clear',dayNightCycle:true,timeOfDay:14,buildingLights:true};state.progression=createProgression(state);}else {const settings=requireRecord(data.settings,'Einstellungen');requireBoolean(settings.disastersEnabled,'Katastrophenmodus');if(settings.buildingLights===undefined)settings.buildingLights=true;requireBoolean(settings.buildingLights,'Gebäudelichter');if(settings.dayNightCycle===undefined)settings.dayNightCycle=true;if(settings.timeOfDay===undefined)settings.timeOfDay=14;requireBoolean(settings.dayNightCycle,'Tag-Nacht-Zyklus');requireNumber(settings.timeOfDay,'Tageszeit',0,24);if((settings.timeOfDay as number)>=24)throw new Error('Ungültiger Spielstand: Tageszeit.');if(!['clear','rain'].includes(settings.weather as string))throw new Error('Ungültiger Spielstand: Wetter.');validateProgression(data.progression,data.month as number);}
