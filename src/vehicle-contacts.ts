...
export interface VehicleContactBody {
  ...
  vx: number; vz: number; angularVelocity: number;
  halfWidth: number; halfLength: number; height: number; mass: number;
}
...
export interface VehicleContactPose { x: number; z: number; yaw: number; y?: number; }
export interface SweptVehicleContact extends VehicleContact { time: number; }

interface Vector { x: number; z: number; }
interface AxisOverlap { normal: Vector; overlap: number; }
...
function validGeometry(body: VehicleContactBody): boolean {
  return [body.x, body.z, body.y, body.yaw, body.halfWidth, body.halfLength, body.height].every(Number.isFinite)
    && body.halfWidth > 0 && body.halfLength > 0 && body.height > 0;
}
...
function projectionRadius(body: VehicleContactBody, basis: [Vector, Vector], axis: Vector): number {
  return body.halfWidth * Math.abs(dot(basis[0], axis)) + body.halfLength * Math.abs(dot(basis[1], axis));
}

function axisOverlaps(a: VehicleContactBody, b: VehicleContactBody): AxisOverlap[] {
  const aAxes = axes(a), bAxes = axes(b), delta = { x: b.x - a.x, z: b.z - a.z };
  return [...aAxes, ...bAxes].map(axis => {
    const distance = dot(delta, axis);
    return {
      normal: distance < 0 ? { x: -axis.x, z: -axis.z } : axis,
      overlap: projectionRadius(a, aAxes, axis) + projectionRadius(b, bAxes, axis) - Math.abs(distance),
    };
  });
}
...
function corners(body: VehicleContactBody): Vector[] {
  ...
  return [[-1, -1], [1, -1], [1, 1], [-1, 1]].map(([width, length]) => ({
...
function supportCenter(vertices: Vector[], axis: Vector): Vector {
  ...
  return { x: face.reduce((sum, p) => sum + p.x, 0) / face.length, z: face.reduce((sum, p) => sum + p.z, 0) / face.length };
}

function contactPoint(a: VehicleContactBody, b: VehicleContactBody, normal: Vector): Vector {
  const aCorners = corners(a), bCorners = corners(b);
  ...
  for (let edgeIndex = 0; edgeIndex < bCorners.length && polygon.length; edgeIndex++) {
    const first = bCorners[edgeIndex], next = bCorners[(edgeIndex + 1) % bCorners.length];
    ...
    for (const point of polygon) {
      ...
      const previousInside = previousDistance >= -CONTACT_EPSILON, pointInside = pointDistance >= -CONTACT_EPSILON;
      if (pointInside !== previousInside) {
        const fraction = clamp(previousDistance / (previousDistance - pointDistance), 0, 1);
        output.push({ x: previous.x + (point.x - previous.x) * fraction, z: previous.z + (point.z - previous.z) * fraction });
      }
      if (pointInside) output.push(point);
      previous = point; previousDistance = pointDistance;
    }
    polygon = output;
  }
  if (polygon.length) {
    ...
    for (let index = 1; index < polygon.length - 1; index++) {
      const p = polygon[index], q = polygon[index + 1];
    ...
    if (Math.abs(area) > CONTACT_EPSILON * CONTACT_EPSILON) return { x: x / (3 * area), z: z / (3 * area) };
    return { x: polygon.reduce((sum, p) => sum + p.x, 0) / polygon.length, z: polygon.reduce((sum, p) => sum + p.z, 0) / polygon.length };
  }
  const faceA = supportCenter(aCorners, normal), faceB = supportCenter(bCorners, { x: -normal.x, z: -normal.z });
  return { x: (faceA.x + faceB.x) / 2, z: (faceA.z + faceB.z) / 2 };
}

/** Actual rotated OBB intersection. y is the body base, not its center. */
export function findVehicleContact(a: VehicleContactBody, b: VehicleContactBody): VehicleContact | null {
  ...
  return { normal: minimum.normal, penetration: Math.max(0, minimum.overlap), point: contactPoint(a, b, minimum.normal) };
}
...
function relativeContactVelocity(a: VehicleContactBody, b: VehicleContactBody, armA: Vector, armB: Vector): Vector {
  return {
...
export function resolveVehicleContact(a: VehicleContactBody, b: VehicleContactBody): VehicleContactResolution | null {
  const contact = findVehicleContact(a, b);
  if (!contact || ![a.vx, a.vz, a.angularVelocity, b.vx, b.vz, b.angularVelocity].every(Number.isFinite)) return null;
  const invMassA = inverseMass(a), invMassB = inverseMass(b), totalInvMass = invMassA + invMassB;
  const invIA = inverseInertia(a, invMassA), invIB = inverseInertia(b, invMassB);
  const armA = { x: contact.point.x - a.x, z: contact.point.z - a.z }, armB = { x: contact.point.x - b.x, z: contact.point.z - b.z };
  const relative = relativeContactVelocity(a, b, armA, armB);
  const normalSpeed = dot(relative, contact.normal), closingSpeed = Math.max(0, -normalSpeed);
  let impulse = 0;
  const denominator = (direction: Vector): number => totalInvMass + crossYaw(armA, direction) ** 2 * invIA + crossYaw(armB, direction) ** 2 * invIB;
  const apply = (direction: Vector, magnitude: number): void => {
    a.vx -= direction.x * magnitude * invMassA; a.vz -= direction.z * magnitude * invMassA;
    b.vx += direction.x * magnitude * invMassB; b.vz += direction.z * magnitude * invMassB;
  ...
  if (totalInvMass > 0 && normalSpeed < 0) {
    const restitution = closingSpeed < .06 ? 0 : RESTITUTION;
    ...
    const tangentVelocity = { x: afterNormal.x - alongNormal * contact.normal.x, z: afterNormal.z - alongNormal * contact.normal.z };
    ...
    if (invMassA) a.angularVelocity = clamp(a.angularVelocity, -MAX_ANGULAR_SPEED, MAX_ANGULAR_SPEED);
    if (invMassB) b.angularVelocity = clamp(b.angularVelocity, -MAX_ANGULAR_SPEED, MAX_ANGULAR_SPEED);
    ...
    if (finalNormalSpeed < 0) {
      const crossA = crossYaw(armA, contact.normal), crossB = crossYaw(armB, contact.normal);
      const projectedNormalSpeed = (extraImpulse: number): number => finalNormalSpeed + extraImpulse * totalInvMass
        + (invMassB ? clamp(b.angularVelocity + crossB * extraImpulse * invIB, -MAX_ANGULAR_SPEED, MAX_ANGULAR_SPEED) - b.angularVelocity : 0) * crossB
        - (invMassA ? clamp(a.angularVelocity - crossA * extraImpulse * invIA, -MAX_ANGULAR_SPEED, MAX_ANGULAR_SPEED) - a.angularVelocity : 0) * crossA;
      let low = 0, high = -finalNormalSpeed / totalInvMass;
      for (let iteration = 0; iteration < 30; iteration++) {
        const middle = (low + high) / 2;
        if (projectedNormalSpeed(middle) < 0) low = middle; else high = middle;
      }
  ...
  if (totalInvMass > 0) {
    ...
    a.x -= contact.normal.x * separation * invMassA; a.z -= contact.normal.z * separation * invMassA;
    b.x += contact.normal.x * separation * invMassB; b.z += contact.normal.z * separation * invMassB;
  }
 ...
 */
function projectionGrowth(body: VehicleContactBody, axis: Vector, angle: number): { rate: number; horizon: number } {
  if (Math.abs(angle) < 1e-12) return { rate: 0, horizon: Infinity };
  const theta = body.yaw + Math.atan2(axis.z, axis.x), sin = Math.sin(theta), cos = Math.cos(theta);
  ...
  const nextQuadrant = angle > 0 ? Math.floor(quadrant + 1e-10) + 1 : Math.ceil(quadrant - 1e-10) - 1;
 ...
 */
export function sweepVehicleContact(aPrevious: VehicleContactPose, aCurrent: VehicleContactBody, bPrevious: VehicleContactPose, bCurrent: VehicleContactBody): SweptVehicleContact | null {
  if (!validGeometry(aCurrent) || !validGeometry(bCurrent)
    || ![aPrevious.x, aPrevious.z, aPrevious.yaw, bPrevious.x, bPrevious.z, bPrevious.yaw, aPrevious.y ?? aCurrent.y, bPrevious.y ?? bCurrent.y].every(Number.isFinite)) return null;
  const radiusA = Math.hypot(aCurrent.halfWidth, aCurrent.halfLength), radiusB = Math.hypot(bCurrent.halfWidth, bCurrent.halfLength);
  const combinedRadius = radiusA + radiusB;
  // Swept bounding circles enclose every intermediate orientation.
  for (const component of ['x', 'z'] as const) {
    if (Math.max(aPrevious[component], aCurrent[component]) + combinedRadius < Math.min(bPrevious[component], bCurrent[component])
      || Math.max(bPrevious[component], bCurrent[component]) + combinedRadius < Math.min(aPrevious[component], aCurrent[component])) return null;
  }
  const aStartY = aPrevious.y ?? aCurrent.y, bStartY = bPrevious.y ?? bCurrent.y;
  const aDy = aCurrent.y - aStartY, bDy = bCurrent.y - bStartY;
  const relativeY = bStartY - aStartY, relativeDy = bDy - aDy;
  let start = 0, end = 1;
  if (Math.abs(relativeDy) < 1e-12) {
    if (relativeY >= aCurrent.height || relativeY <= -bCurrent.height) return null;
  } else {
    const first = (-bCurrent.height - relativeY) / relativeDy, last = (aCurrent.height - relativeY) / relativeDy;
  ...
  const aDx = aCurrent.x - aPrevious.x, aDz = aCurrent.z - aPrevious.z;
  const bDx = bCurrent.x - bPrevious.x, bDz = bCurrent.z - bPrevious.z;
  const aAngle = shortestAngle(aPrevious.yaw, aCurrent.yaw), bAngle = shortestAngle(bPrevious.yaw, bCurrent.yaw);
  const relativeMovement = { x: bDx - aDx, z: bDz - aDz };
  const a = { ...aCurrent }, b = { ...bCurrent };
  let time = start, checks = 0, lastProbe = 0;
  const contactAt = (time: number): VehicleContact | null => {
    checks++;
    a.x = aPrevious.x + aDx * time; a.z = aPrevious.z + aDz * time; a.y = aStartY + aDy * time; a.yaw = aPrevious.yaw + aAngle * time;
    b.x = bPrevious.x + bDx * time; b.z = bPrevious.z + bDz * time; b.y = bStartY + bDy * time; b.yaw = bPrevious.yaw + bAngle * time;
  ...
  while (checks < MAX_SWEEP_ITERATIONS && time <= end + 1e-12) {
    ...
    let advancement = 0, largestGap = 0;
    for (const axis of axisOverlaps(a, b)) {
      ...
      const aGrowth = projectionGrowth(a, axis.normal, aAngle), bGrowth = projectionGrowth(b, axis.normal, bAngle);
      ...
      const safeAdvance = closingBound > 0 ? Math.min(horizon, -axis.overlap / closingBound) : horizon;
    ...
    if (checks >= 16 && checks <= 40 && checks - lastProbe >= 8 && largestGap < .002) {
      lastProbe = checks;
      let high = Math.min(end, time + advancement * 32), low = time;
      let highContact = contactAt(high);
      if (highContact) {
        for (let iteration = 0; iteration < 20 && checks < MAX_SWEEP_ITERATIONS && high - low > 1e-10; iteration++) {
          const middle = (low + high) / 2, middleContact = contactAt(middle);
          if (middleContact) { high = middle; highContact = middleContact; } else low = middle;
        }
