...
export function setLocale(value: Locale): void {
  locale = value === 'en' ? 'en' : 'de';
}
export function localeCode(): 'de-DE' | 'en-US' {
...
const legacyNames: Record<string, string> = Object.assign(Object.create(null), {
  ...
  Megastadt: 'Megacity',
  Weltmetropole: 'Global metropolis',
...
const legacyMessages: Record<string, string> = Object.assign(Object.create(null), {
  'Deine Stadt ist bereit. Neue Viertel, Forschung und Stadtaufträge warten auf dich. Straßen, Stromleitungen und Wasserrohre bilden eigene Netze.':
    'Your city is ready. New districts, research, and city quests await. Roads, power lines, and water pipes form separate networks.',
  'Deine Stadt ist bereit. Erschließe neue Wohngebiete, halte die Versorgung im Blick und baue eine Metropole.':
    'Your city is ready. Develop new residential districts, keep services running, and build a metropolis.',
  'Baue Straßen, Kraftwerk und Wasserwerk. Verbinde Stromleitungen und Wasserrohre mit den Anlagen und erschließe Wohn- und Arbeitsgebiete.':
    'Build roads, a power station, and a waterworks. Connect power lines and water pipes to the facilities, then develop residential and employment areas.',
...
const legacyDescriptions: Record<string, string> = Object.assign(Object.create(null), {
  'Baue eine gut versorgte Stadt mit Wohnvierteln, kleinen Betrieben und vollständiger Grundversorgung. Gebäude entwickeln sich bis Stufe 2.':
    'Build a well-served town with residential districts, small businesses, and all basic services. Buildings can develop to level 2.',
  'Bahn, Wind- und Solarenergie, Stadion, Hafen und Recycling werden verfügbar. Deine Stadt wächst mit dichterer Bebauung bis Gebäudestufe 3.':
    'Rail, wind and solar power, a stadium, a seaport, and recycling become available. Your city grows with denser development up to building level 3.',
  'Universität, Flughafen und die höchsten Wohn- und Geschäftshäuser prägen deine Metropole. Gebäude entwickeln sich bis Stufe 4.':
    'A university, an airport, and the tallest residential and commercial buildings shape your metropolis. Buildings can develop to level 4.',
  'Gewinne innerhalb von 60 Monaten 5.000 zusätzliche Einwohner gegenüber dem Start dieser Challenge.':
    'Gain 5,000 additional residents within 60 months of starting this challenge.',
  'Erreiche innerhalb von 120 Monaten 10.000 Einwohner bei einer Umweltbelastung unter 10.':
    'Reach 10,000 residents within 120 months while keeping pollution below 10.',
  'Steigere die Stadtkasse innerhalb von 60 Monaten um 150.000. Jeder neue Kredit beendet diese Challenge.':
    'Increase the city treasury by €150,000 within 60 months. Any new loan ends this challenge.',
});
...
function translateLegacyMessage(message: string): string {
  ...
  if (
    (match =
      /^([\d.,]+) Gebäude wurden zerstört\. Räume die Grundstücke und baue die Feuerwehr aus\.$/.exec(
        message,
      ))
  )
    return `${oldNumber(match[1])} buildings were destroyed. Clear the lots and expand fire protection.`;
  if (
    (match = /^([\d.,]+) Einwohner! Das Land fördert deine Stadt mit ([\d.,]+) €\.$/.exec(message))
  )
    return `${oldNumber(match[1])} residents! The state is supporting your city with €${oldNumber(match[2])}.`;
  if (
    (match =
      /^([\d.,]+) Einwohner · ([\d.,]+) % Zufriedenheit · ([+-]?[\d.,]+) € monatlich\.$/.exec(
        message,
      ))
  )
  ...
  if (
    (match =
      /^Ein Gebäude bei (-?\d+), (-?\d+) brennt\. Eine versorgte Feuerwache begrenzt die Schäden\.$/.exec(
        message,
      ))
  )
  ...
  if (
    (match =
      /^([\d.,]+) Gebäude beschädigt, ([\d.,]+) Leitungsfelder unterbrochen\. Räume Trümmer, repariere die Netze und stelle die Versorgung wieder her\.$/.exec(
        message,
      ))
  )
  ...
  if (
    (match =
      /^Das Stadtgebiet umfasst jetzt (\d+) × (\d+) Felder\. Bestehende Wohn- und Arbeitsgebiete wurden erhalten; öffentliche Gebäude haben eigene große Grundstücke\.$/.exec(
        message,
      ))
  )
  ...
  if (
    (match = /^(.+): \+([\d.,]+) € und \+([\d.,]+) EP\.( Dein Abzeichen bleibt erhalten\.)?$/s.exec(
      message,
    )) &&
    legacyNames[match[1]]
  )
  ...
  if (
    (match = /^(.+) gestartet\. Du hast (\d+) Monate\. (.+)$/s.exec(message)) &&
    legacyNames[match[1]] &&
    legacyDescriptions[match[3]]
  )
    return `${legacyNames[match[1]]} started. You have ${oldNumber(match[2])} months. ${legacyDescriptions[match[3]]}`;
  if (
    (match = /^(.+): Die Zeit ist abgelaufen\. Du kannst es erneut versuchen\.$/s.exec(message)) &&
    legacyNames[match[1]]
  )
    return `${legacyNames[match[1]]}: Time has run out. You can try again.`;
  if (
    (match = /^([\d.,]+) Einwohner erreicht\. (.+)$/s.exec(message)) &&
    legacyDescriptions[match[2]]
  )
    return `${oldNumber(match[1])} residents reached. ${legacyDescriptions[match[2]]}`;
  if (
    (match =
      /^(.+) hat sechs Monate lang mindestens 25\.000 Einwohner, hohe Lebensqualität und einen positiven Haushalt gehalten\. Du hast die Kampagne gemeistert\. Baue im freien Spiel weiter!$/s.exec(
        message,
      ))
  )
  ...
  if (
    (match = /^(.+) — hole ([\d.,]+) € und ([\d.,]+) EP im Stadtziel-Fenster ab\.$/s.exec(
      message,
    )) &&
    legacyNames[match[1]]
  )
...
export function localizedEventMessage(
  event: Pick<LocalizableEvent, 'message' | 'messageEn'>,
): string {
