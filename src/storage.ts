...
function decodeRecord(value: unknown, unknownTime: number): StoredCity | null {
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    if (record.storageVersion === 2 && typeof record.raw === 'string' && typeof record.savedAt === 'number' && Number.isFinite(record.savedAt)) {
      return record as StoredCity;
    }
  }
  let raw: string;
  try { raw = typeof value === 'string' ? value : JSON.stringify(value); }
  catch { raw = '{"invalidStorageRecord":true}'; }
...
function decodeLocalRecord(raw: string | null): StoredCity | null {
  ...
  try {
    ...
    if (typeof parsed === 'object' && parsed !== null && (parsed as Partial<StoredCity>).storageVersion === 2) {
      return decodeRecord(parsed, Number.POSITIVE_INFINITY);
    }
  } catch { /* A corrupt city must reach validation, not disappear. */ }
...
function browserDatabase(): CityStorageDatabase | null {
  ...
  function open(): Promise<IDBDatabase> {
    ...
    opening = new Promise<IDBDatabase>((resolve, reject) => {
      ...
      try { request = factory.open(CITY_DATABASE_NAME, 1); }
      catch (error) { fail(error); return; }
      request.onblocked = () => fail(new Error('IndexedDB upgrade is blocked by another open tab.'));
      ...
      request.onupgradeneeded = () => {
        ...
        if (!database.objectStoreNames.contains(CITY_DATABASE_STORE)) database.createObjectStore(CITY_DATABASE_STORE);
      };
      request.onsuccess = () => {
        const database = request.result;
        ...
        clearTimeout(timer);
        connection = database;
        const discard = () => { if (connection === database) connection = null; };
        database.onversionchange = () => { database.close(); discard(); };
  ...
  async function transaction<T>(mode: IDBTransactionMode, operation: (store: IDBObjectStore, value: (result: T) => void) => void): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      try {
        ...
        try { tx = database.transaction(CITY_DATABASE_STORE, mode, { durability: mode === 'readwrite' ? 'strict' : 'default' }); }
        catch { tx = database.transaction(CITY_DATABASE_STORE, mode); }
      } catch (error) {
      ...
      const timer = setTimeout(() => {
        ...
        try { tx.abort(); } catch { /* Transaction may have finished meanwhile. */ }
      ...
      try { operation(tx.objectStore(CITY_DATABASE_STORE), value => { result = value; }); }
      catch (error) { try { tx.abort(); } catch { /* already aborted */ } fail(error); }
    });
  }

  return {
    read: key => transaction<unknown>('readonly', (store, value) => {
      const request = store.get(key);
      request.onsuccess = () => value(request.result);
    }),
    write: (key, record) => transaction<void>('readwrite', store => { store.put(record, key); }),
    writeIfAbsent: (key, record) => transaction<void>('readwrite', store => {
      const request = store.get(key);
      request.onsuccess = () => { if (request.result === undefined) store.put(record, key); };
    }),
  };
}
...
export function createCityStorage(options: StorageOptions = {}) {
  ...
  async function loadSavedCity(): Promise<SavedCityResult> {
    ...
    if (database) {
      try { databaseRecord = decodeRecord(await database.read('current'), 0); }
      catch { databaseFailed = true; }
    }
    try {
      local = getLocal();
    ...
    const current = localRecord && (!databaseRecord || localRecord.savedAt > databaseRecord.savedAt) ? localRecord : databaseRecord;
    if (current) {
      if (Number.isFinite(current.savedAt)) lastSavedAt = Math.max(lastSavedAt, current.savedAt);
      const result: SavedCityResult = { raw: current.raw, source: current === databaseRecord ? 'indexeddb' : 'legacy' };
    ...
    try { if (local) legacyRaw = local.getItem(LEGACY_CITY_KEY); }
    catch { localFailed = true; }
    const result: SavedCityResult = { raw: legacyRaw, source: legacyRaw === null ? 'none' : 'legacy' };
    if (databaseFailed || localFailed) result.error = storageError('load').message;
    return result;
  }
  ...
  function saveCityRaw(raw: string): Promise<void> {
    return enqueue(async () => {
      ...
      try {
        ...
        if (previous && Number.isFinite(previous.savedAt)) lastSavedAt = Math.max(lastSavedAt, previous.savedAt);
      } catch { /* IndexedDB can still save when local storage is unavailable. */ }
      ...
      if (database) {
        try {
          ...
          try {
            ...
            if (fallbackBeforeSave !== null && local?.getItem(FALLBACK_CITY_KEY) === fallbackBeforeSave) local.removeItem?.(FALLBACK_CITY_KEY);
          } catch { /* Cleaning an obsolete fallback must not fail a committed save. */ }
          return;
        }
        catch { /* The old database transaction is aborted; try local v2 only. */ }
      }
      try {
        const local = getLocal();
  ...
  function backupLegacy(raw: string): Promise<void> {
    return enqueue(async () => {
      ...
      try { await database.writeIfAbsent('legacy-backup', record(raw)); }
      catch { throw storageError('backup'); }
    });
  }

  function safePreferenceGet(key: string): string | null {
    try { return getLocal()?.getItem(key) ?? null; }
    catch { return null; }
  }
