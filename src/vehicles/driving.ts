...
import { createDrivingCollisionWorld, type DrivingCollisionWorld } from './collisions';
import {
  findVehicleContact,
...
import { refreshVehicleLabel, vehicleKindLabel } from '../rendering/vehicles/model';
...
import { refreshFacilityServiceVehicleLabel } from '../rendering/buildings/models';
...
const REVERSE_SPEED = -0.68;
const BODY_CLEARANCE = 0.005;
...
const solidKinds = new Set<Tile['kind']>([
  'power',
  'waterpump',
  'police',
  'fire',
...
const damp = (current: number, target: number, response: number, dt: number) =>
  THREE.MathUtils.lerp(current, target, 1 - Math.exp(-response * dt));
...
export function localizedDrivingObstacle(reason: string): string {
  switch (reason) {
    case 'Stadtrand erreicht':
      return tr('Stadtrand erreicht', 'City boundary reached');
    case 'Hier ist Wasser':
      return tr('Hier ist Wasser', 'Water ahead');
    case 'Baum im Weg':
      return tr('Baum im Weg', 'Tree in the way');
    case 'Gebäude im Weg':
      return tr('Gebäude im Weg', 'Building in the way');
    case 'Hang zu steil':
      return tr('Hang zu steil', 'Slope too steep');
    case 'Zusammenstoß':
      return tr('Zusammenstoß', 'Collision');
    default:
      return reason;
  }
}
...
export function getVehicleDimensions(model: THREE.Group): VehicleDimensions {
  ...
  return {
    width: number(declared.width, DEFAULT_DIMENSIONS.width, 0.1, 0.45),
    length: number(declared.length, DEFAULT_DIMENSIONS.length, 0.18, 0.9),
    height: number(declared.height, DEFAULT_DIMENSIONS.height, 0.1, 0.5),
    wheelBase: number(declared.wheelBase, DEFAULT_DIMENSIONS.wheelBase, 0.15, 0.6),
    mass: number(declared.mass, DEFAULT_DIMENSIONS.mass, 0.5, 4),
  };
}

function tileAtWorld(state: CityState, x: number, z: number): Tile | undefined {
  const tx = Math.floor(x + state.size / 2),
    tz = Math.floor(z + state.size / 2);
  return tx < 0 || tz < 0 || tx >= state.size || tz >= state.size
    ? undefined
    : state.tiles[tz * state.size + tx];
}
...
export function drivingSurfaceHeight(state: CityState, x: number, z: number): number {
  ...
  if (tileAtWorld(state, x, z)?.kind === 'road')
    return sampleRoadHeight(state, x, z) + 0.052 + BODY_CLEARANCE;
  const tile = tileAtWorld(state, x, z),
    plan = tile ? getFacilityAccess(state, tile) : null;
  const base = plan?.connected
...
function pointObstacle(state: CityState, x: number, z: number, y?: number): string | undefined {
  ...
  if (tile.kind === 'water' || (tile.elevation < 0 && tile.kind !== 'road'))
    return 'Hier ist Wasser';
  if (collisionWorldFor(state).pointBlocked(x, z, y))
    return tile.kind === 'tree' ? 'Baum im Weg' : 'Gebäude im Weg';
  return undefined;
}

/** Footprints are a broad phase only: open lots and parking are actually driveable. */
export function drivingObstacle(
  state: CityState,
  x: number,
  z: number,
  yaw: number,
  dimensions: VehicleDimensions = DEFAULT_DIMENSIONS,
): string | undefined {
  const forwardX = Math.sin(yaw),
    forwardZ = Math.cos(yaw),
    rightX = Math.cos(yaw),
    rightZ = -Math.sin(yaw);
  for (const length of [-dimensions.length / 2, 0, dimensions.length / 2])
    for (const width of [-dimensions.width / 2, 0, dimensions.width / 2]) {
      const tile = tileAtWorld(
        state,
        x + forwardX * length + rightX * width,
        z + forwardZ * length + rightZ * width,
      );
      if (!tile) return 'Stadtrand erreicht';
      if (tile.kind === 'water' || (tile.elevation < 0 && tile.kind !== 'road'))
        return 'Hier ist Wasser';
    }
  return collisionWorldFor(state).collides(
    x,
    z,
    yaw,
    {
    ...
    drivingSurfaceHeight(state, x, z),
  );
}
...
export function createVehicleMotion(
  ...
  return {
    x,
    z,
    y: drivingSurfaceHeight(state, x, z),
    yaw,
    speed: 0,
    steering: 0,
    pitch: 0,
...
export function getDrivingSpeedLimit(state: CityState, x: number, z: number): number {
  ...
  for (let dz = -3; dz <= 3; dz++)
    for (let dx = -3; dx <= 3; dx++) {
      ...
      if (solidKinds.has(neighbor.kind) || (zonedKinds.has(neighbor.kind) && neighbor.level > 0))
        return 50;
    }
  return 70;
}
...
export function stepVehicle(
  ...
  if (dt > STEP + 1e-9) {
    ...
    while (remaining > 1e-9) {
      ...
      stepVehicle(state, motion, input, step, dimensions);
      remaining -= step;
    }
    return;
  }
  ...
  const fx = Math.sin(motion.yaw),
    fz = Math.cos(motion.yaw),
    rx = Math.cos(motion.yaw),
    rz = -Math.sin(motion.yaw);
  ...
  let acceleration = 0;
  if (input.handbrake)
    acceleration = -Math.sign(forwardSpeed) * (Math.abs(steer) > 0.1 ? 0.62 : 1.65);
  else if (throttle !== 0)
    acceleration =
      throttle *
      (Math.sign(forwardSpeed) !== Math.sign(throttle) && Math.abs(forwardSpeed) > 0.04
        ? 3.2
        : throttle > 0
          ? 1.02
          : 0.72);
  else acceleration = -Math.sign(forwardSpeed) * (0.15 + Math.abs(forwardSpeed) * 0.14);
  ...
  const grip = input.handbrake ? 0.7 : 18;
  lateralSpeed *= Math.exp(-grip * dt);
  const momentumDrag = input.handbrake ? Math.exp(-0.28 * dt) : 1;
  motion.vx = (fx * forwardSpeed + rx * lateralSpeed) * momentumDrag;
  motion.vz = (fz * forwardSpeed + rz * lateralSpeed) * momentumDrag;
  motion.steering = damp(motion.steering, steer * 0.62, 9, dt);
  const steeringAtSpeed = motion.steering / (1 + Math.abs(forwardSpeed) * 0.7);
  const yawTarget = clamp(
    ((Math.tan(steeringAtSpeed) * forwardSpeed) / dimensions.wheelBase) *
      (input.handbrake ? 1.65 : 1),
    input.handbrake ? -3.8 : -2.1,
    input.handbrake ? 3.8 : 2.1,
  );
  ...
  const slopeSurface = (x: number, z: number) => {
    const access = sampleFacilityAccessAt(state, x, z);
    return access !== null
      ? access - FACILITY_PAVEMENT_HEIGHT
      : tileAtWorld(state, x, z)?.kind === 'road'
  ...
  if (!obstacle && distance > 1e-7 && Math.abs(nextSurface - oldSurface) > distance * 2.2 + 0.00001)
    obstacle = 'Hang zu steil';
  if (obstacle) {
    ...
    const normal =
      canX && !canZ
        ? { x: 0, z: Math.sign(motion.vz) }
        : canZ && !canX
          ? { x: Math.sign(motion.vx), z: 0 }
          : { x: motion.vx / Math.max(speed, 0.0001), z: motion.vz / Math.max(speed, 0.0001) };
    const impactSpeed = Math.abs(motion.vx * normal.x + motion.vz * normal.z);
    if (impactSpeed > 0.12) {
      const support =
        (Math.abs(normal.x * Math.sin(motion.yaw) + normal.z * Math.cos(motion.yaw)) *
          dimensions.length) /
          2 +
        (Math.abs(normal.x * Math.cos(motion.yaw) - normal.z * Math.sin(motion.yaw)) *
          dimensions.width) /
          2;
      motion.worldImpact = {
        speed: impactSpeed,
        point: {
          x: motion.x + normal.x * support,
          y: motion.y + 0.08,
          z: motion.z + normal.z * support,
        },
  ...
  motion.drifting =
    Math.abs(motion.lateralSpeed) > 0.1 &&
    Math.hypot(motion.vx, motion.vz) > 0.3 &&
    Math.abs(Math.atan2(motion.lateralSpeed, Math.abs(motion.speed))) > 0.15;
  const facingX = Math.sin(motion.yaw),
    facingZ = Math.cos(motion.yaw),
    rightX = Math.cos(motion.yaw),
    rightZ = -Math.sin(motion.yaw);
  const supportLength = dimensions.wheelBase / 2,
    supportWidth = dimensions.width * 0.38;
  const front = drivingSurfaceHeight(
    state,
    motion.x + facingX * supportLength,
    motion.z + facingZ * supportLength,
  );
  const back = drivingSurfaceHeight(
    state,
    motion.x - facingX * supportLength,
    motion.z - facingZ * supportLength,
  );
  const right = drivingSurfaceHeight(
    state,
    motion.x + rightX * supportWidth,
    motion.z + rightZ * supportWidth,
  );
  const left = drivingSurfaceHeight(
    state,
    motion.x - rightX * supportWidth,
    motion.z - rightZ * supportWidth,
  );
  ...
  const ground = Math.max(
    drivingSurfaceHeight(state, motion.x, motion.z),
    (front + back) / 2,
    (right + left) / 2,
  );
  motion.y = Math.max(ground - 0.025, damp(motion.y, ground, 20, dt));
  motion.pitch = damp(
    motion.pitch,
    clamp(-Math.atan2(front - back, supportLength * 2), -0.8, 0.8),
    12,
    dt,
  );
  motion.roll = damp(
    motion.roll,
    clamp(
      Math.atan2(right - left, supportWidth * 2) - motion.angularVelocity * motion.speed * 0.02,
      -0.55,
      0.55,
    ),
    12,
...
function findTrafficRejoin(
  state: CityState,
  x: number,
  z: number,
  yaw: number,
): TrafficRejoin | null {
  let result: TrafficRejoin | null = null,
    nearest = Infinity;
  const half = state.size / 2;
  for (const tile of state.tiles) {
    if (
      tile.kind !== 'road' ||
      Math.abs(tile.x + 0.5 - half - x) > 1.3 ||
      Math.abs(tile.z + 0.5 - half - z) > 1.3
    )
      continue;
    const neighbors = [
      { x: tile.x + 1, z: tile.z },
      ...
      (p) =>
        ...
        p.x < state.size &&
        p.z < state.size &&
        state.tiles[p.z * state.size + p.x].kind === 'road',
    );
    for (const previous of neighbors)
      for (const to of neighbors) {
        ...
        const evaluate = (progress: number) => {
          const pose = lanePose(previous, tile, to, progress),
            px = pose.x - half,
            pz = pose.z - half;
          return { pose, px, pz, distance: Math.hypot(px - x, pz - z) };
        };
        let bestProgress = 0,
          bestDistance = Infinity;
        for (let i = 0; i <= 20; i++) {
          const trial = evaluate(i / 20);
          if (trial.distance < bestDistance) {
            bestDistance = trial.distance;
            bestProgress = i / 20;
          }
        }
        let lo = Math.max(0, bestProgress - 0.05),
          hi = Math.min(0.999999, bestProgress + 0.05);
        for (let i = 0; i < 18; i++) {
          const a = lo + (hi - lo) / 3,
            b = hi - (hi - lo) / 3;
          if (evaluate(a).distance < evaluate(b).distance) hi = b;
          else lo = a;
        }
        const progress = (lo + hi) / 2,
          { pose, px, pz, distance } = evaluate(progress),
          alignment = pose.tangentX * Math.sin(yaw) + pose.tangentZ * Math.cos(yaw);
        const score = distance + (1 - alignment) * 0.012;
        if (score >= nearest) continue;
        nearest = score;
        result = {
          previous,
          from: { x: tile.x, z: tile.z },
          to,
          progress,
          x: px,
          z: pz,
...
export function handBackToTraffic(
  state: CityState,
  car: DrivableCar,
  ...
  if (!route || route.distance > 0.025) return false;
...
export function createDrivingController(
  ...
  let selected: DrivableCar | null = null,
    hovered: DrivableCar | null = null;
  ...
  interface ControlledCar {
    motion: VehicleMotion;
    age: number;
    route: TrafficRejoin | null;
    routeTimer: number;
  }
  ...
  const camera = new THREE.PerspectiveCamera(55, 1, 0.035, Math.max(500, initialState.size * 4));
  ...
  const ring = new THREE.Mesh(new THREE.RingGeometry(0.245, 0.273, 48), ringMaterial);
  const disk = new THREE.Mesh(new THREE.CircleGeometry(0.25, 32), diskMaterial);
  ...
  const outlineGeometry = new THREE.EdgesGeometry(new THREE.BoxGeometry(0.215, 0.18, 0.365));
  const outline = new THREE.LineSegments(outlineGeometry, outlineMaterial);
  outline.position.y = 0.095;
  group.add(ring, disk, outline);
  group.traverse((object) => {
    object.userData.raytracingExclude = true;
  });
  ...
  const pickSphere = new THREE.Sphere(new THREE.Vector3(), 0.2),
  ...
  function vehicleLabel(car: DrivableCar): string {
    ...
    if (kind === 'sedan' || kind === 'taxi' || kind === 'van' || kind === 'truck')
      return vehicleKindLabel(kind);
  ...
  function pickRadius(car: DrivableCar): number {
    ...
    return Math.max(0.2, Math.hypot(dimensions.width, dimensions.length) / 2);
  }
  ...
  function getStatus(): DrivingStatus {
    return {
      active: !!selected,
      speed: motion
        ? Math.round(Math.hypot(motion.vx, motion.vz) * 36) * (motion.speed < -0.05 ? -1 : 1)
        : 0,
      label: selected ? vehicleLabel(selected) : '',
      speedLimit: motion ? getDrivingSpeedLimit(state, motion.x, motion.z) : 50,
      drifting: motion?.drifting ?? false,
      collisionCount,
  ...
  function projectHover(force = false): DrivingHover | null {
    ...
    world.copy(hovered.model.position);
    world.y += 0.43;
    world.project(hoverCamera);
    ...
    const info = {
      id: hovered.model.id,
      screenX: viewport.left + ((world.x + 1) * viewport.width) / 2,
      screenY: viewport.top + ((1 - world.y) * viewport.height) / 2,
      label: vehicleLabel(hovered),
    };
    const signature = `${info.id}:${Math.round(info.screenX)}:${Math.round(info.screenY)}`;
    if (force || signature !== lastHover) {
      lastHover = signature;
  ...
  function hover(
    ...
    hoverRay.copy(raycaster.ray);
    hoverNear = raycaster.near;
    hoverFar = raycaster.far;
    hoverCamera = view;
    viewport = bounds;
    let hit: DrivableCar | null = null,
      distance = Infinity;
    for (const car of getCars()) {
      if (!car.model.visible) continue;
      pickSphere.center.copy(car.model.position);
      pickSphere.center.y += getVehicleDimensions(car.model).height / 2;
      pickSphere.radius = pickRadius(car);
    ...
    if (hit) {
      const changed = hovered !== hit;
      hovered = hit;
      missedHover = 0;
      group.visible = true;
      return projectHover(changed);
    }
    // Crossing from the tiny car to its DOM steering button must not dismiss it.
    if (hovered && !missedHover) missedHover = 0.8;
    return projectHover();
  }
  ...
  function updateCamera(dt: number, snap = false): void {
    ...
    const fx = Math.sin(motion.yaw),
      fz = Math.cos(motion.yaw);
    const distance = 1.55 + Math.abs(motion.speed) * 0.2;
    targetLook.set(motion.x + fx * 0.48, motion.y + 0.18, motion.z + fz * 0.48);
    targetCamera.set(
      motion.x - fx * distance,
      motion.y + 0.92 + Math.abs(motion.speed) * 0.08,
      motion.z - fz * distance,
    );
    const limit = state.size / 2 - 0.25;
    targetCamera.x = clamp(targetCamera.x, -limit, limit);
    targetCamera.z = clamp(targetCamera.z, -limit, limit);
    // Keep the camera in front of the first building behind the vehicle and above hills.
    for (let fraction = 0.1; fraction <= 1.001; fraction += 0.1) {
      const x = THREE.MathUtils.lerp(motion.x, targetCamera.x, fraction),
        z = THREE.MathUtils.lerp(motion.z, targetCamera.z, fraction);
      const obstacle = pointObstacle(state, x, z);
      if (obstacle === 'Gebäude im Weg') {
        targetCamera.x = THREE.MathUtils.lerp(
          motion.x,
          targetCamera.x,
          Math.max(0.14, fraction - 0.12),
        );
        targetCamera.z = THREE.MathUtils.lerp(
          motion.z,
          targetCamera.z,
          Math.max(0.14, fraction - 0.12),
        );
      ...
      targetCamera.y = Math.max(targetCamera.y, sampleGroundHeight(state, x, z) + 0.48);
    }
    if (snap) {
      camera.position.copy(targetCamera);
      cameraLook.copy(targetLook);
    } else if (entryProgress < 1) {
      entryProgress = Math.min(1, entryProgress + dt / 0.95);
    ...
    camera.position.y = Math.max(
      camera.position.y,
      sampleGroundHeight(state, camera.position.x, camera.position.z) + 0.32,
    );
  ...
  function enter(id: number): boolean {
    ...
    motion =
      previousControl?.motion ??
      createVehicleMotion(state, car.model.position.x, car.model.position.z, car.model.rotation.y);
    if (!previousControl) {
      const speed =
        state.speed && !car.model.userData.trafficWaiting ? clamp(car.speed, 0, 0.8) : 0;
      setVehicleVelocity(motion, Math.sin(motion.yaw) * speed, Math.cos(motion.yaw) * speed);
    }
    controlled.delete(car);
    camera.far = Math.max(500, state.size * 4);
  ...
  function exit(): void {
    ...
    if (motion) controlled.set(selected, { motion, age: 0.5, route: null, routeTimer: 0 });
    selected = null;
    motion = null;
    keys.clear();
    emitStatus(true);
  ...
  function controlsCar(car: DrivableCar): boolean {
    return car === selected || controlled.has(car);
  }
  ...
  function body(car: DrivableCar): VehicleContactBody {
    const dimensions = getVehicleDimensions(car.model),
      m = car === selected ? motion : controlled.get(car)?.motion;
    const position = pose(car),
      speed = state.speed && !car.model.userData.trafficWaiting ? car.speed : 0;
    return {
      ...position,
      vx: m?.vx ?? Math.sin(position.yaw) * speed,
      vz: m?.vz ?? Math.cos(position.yaw) * speed,
      angularVelocity: m?.angularVelocity ?? 0,
      halfWidth: dimensions.width / 2,
      halfLength: dimensions.length / 2,
      height: dimensions.height,
  ...
  function emitSweep(
    car: DrivableCar,
    previous: { x: number; y: number; z: number; yaw: number },
    m: VehicleMotion,
  ): void {
    const dimensions = getVehicleDimensions(car.model);
    callbacks.onVehicleSweep?.({
      previous: { x: previous.x, y: previous.y, z: previous.z },
      ...
      height: dimensions.height,
      velocity: { x: m.vx, y: clamp((m.y - previous.y) / STEP, -3, 3), z: m.vz },
  ...
  function emitWorldImpact(car: DrivableCar, m: VehicleMotion): void {
    ...
    collisionCooldown.set(key, elapsed + 0.6);
    collisionCount++;
  ...
  function updateRecoveringCars(): void {
    for (const [car, entry] of controlled) {
      ...
      entry.age += STEP;
      entry.routeTimer -= STEP;
      let input: DrivingInput = { throttle: 0, steer: 0, handbrake: false };
      if (entry.age > 1.2 && Math.hypot(m.vx, m.vz) < 0.65) {
        if (entry.routeTimer <= 0) {
          entry.route = findTrafficRejoin(state, m.x, m.z, m.yaw);
          entry.routeTimer = 0.7;
        }
        ...
        if (route) {
          ...
          const desiredYaw =
            distance > 0.045
              ? Math.atan2(
                  route.x + Math.sin(route.yaw) * 0.35 - m.x,
                  route.z + Math.cos(route.yaw) * 0.35 - m.z,
                )
              : route.yaw;
          ...
          input = {
            throttle: m.speed < 0.24 ? 0.4 : 0,
            steer: clamp(delta * 1.5, -1, 1),
            handbrake: false,
          };
          if (
            distance < 0.025 &&
            Math.abs(Math.atan2(Math.sin(route.yaw - m.yaw), Math.cos(route.yaw - m.yaw))) < 0.12
          ) {
            assignTrafficRoute(state, car, route);
            controlled.delete(car);
            continue;
          ...
          if (Math.abs(delta) > 2.1)
            input = { throttle: -0.22, steer: -Math.sign(delta), handbrake: false };
        }
      }
  ...
  function resolveTrafficContacts(
    previous: Map<DrivableCar, { x: number; y: number; z: number; yaw: number }>,
    firstSubstep: boolean,
  ): void {
    const targets = getCars(),
      visited = new Set<string>();
    const sources = [...(selected ? [selected] : []), ...controlled.keys()];
    for (const car of sources)
      for (const other of targets) {
        if (car === other || !other.model.visible) continue;
        const pair =
          car.model.id < other.model.id
            ? `${car.model.id}:${other.model.id}`
            : `${other.model.id}:${car.model.id}`;
        ...
        const bPrevious =
          previous.get(other) ??
          (firstSubstep ? seenCarPoses.get(other) : undefined) ??
          pose(other);
        const originalA = { ...a },
          originalB = { ...b };
        ...
        if (!contact) {
          ...
          const interpolate = (target: VehicleContactBody, start: typeof aPrevious) => {
            ...
            const angle = Math.atan2(
              Math.sin(target.yaw - start.yaw),
              Math.cos(target.yaw - start.yaw),
            );
            target.yaw = start.yaw + angle * sweep.time;
          };
          interpolate(a, aPrevious);
          interpolate(b, bPrevious);
          // A tiny overlap makes the conservative first-contact sample resolvable.
          a.x += sweep.normal.x * 0.000001;
          a.z += sweep.normal.z * 0.000001;
          b.x -= sweep.normal.x * 0.000001;
          b.z -= sweep.normal.z * 0.000001;
        ...
        for (const [vehicle, next, original] of [
          [car, a, originalA],
          [other, b, originalB],
        ] as const) {
          if (
            drivingObstacle(state, next.x, next.z, next.yaw, getVehicleDimensions(vehicle.model))
          ) {
            next.x = original.x;
            next.z = original.z;
            next.yaw = original.yaw;
            next.vx *= 0.1;
            next.vz *= 0.1;
          }
        }
        const aMotion = applyBody(car, a),
          bMotion = applyBody(other, b);
        if (!previous.has(other)) previous.set(other, bPrevious);
        if (resolution.closingSpeed > 0.06 || resolution.impulse > 0.02)
          for (const affected of [car, other]) {
        ...
        if (resolution.closingSpeed > 0.06 && elapsed > cooldown) {
          collisionCooldown.set(pair, elapsed + 0.45);
          collisionCount++;
          aMotion.blocked = bMotion.blocked = 'Zusammenstoß';
          aMotion.blockedFor = bMotion.blockedFor = 1.25;
          bMotion.roll = clamp(
            bMotion.roll + resolution.impulse * (resolution.contact.normal.x || 0.4) * 0.2,
            -0.32,
            0.32,
          );
          callbacks.onCollision?.({
            vehicleId: car.model.id,
            otherId: other.model.id,
            point: {
              x: resolution.contact.point.x,
              y: Math.max(a.y, b.y) + 0.08,
              z: resolution.contact.point.z,
            },
            normal: { x: resolution.contact.normal.x, y: 0, z: resolution.contact.normal.z },
            speed: resolution.closingSpeed,
          });
        }
      }
  }
  ...
  function update(dt: number): void {
    ...
    if (hovered) {
      ...
      world.copy(hovered.model.position);
      world.y += hoverDimensions.height / 2;
      ...
      if (pointerOverCanvas && hit && hitDistance >= hoverNear ** 2 && hitDistance <= hoverFar ** 2)
        missedHover = 0;
      else if (!missedHover) missedHover = 0.8;
      if (missedHover > 0) {
        missedHover -= dt;
        if (missedHover <= 0) clearHover();
      }
      if (hovered) {
        group.position.copy(hovered.model.position);
        group.rotation.y = hovered.model.rotation.y;
        const haloScale = Math.max(0.273, hoverDimensions.length / 2 + 0.08) / 0.273;
        ring.scale.setScalar(haloScale);
        disk.scale.setScalar(haloScale);
        outline.scale.set(
          hoverDimensions.width / 0.183,
          hoverDimensions.height / 0.163,
          hoverDimensions.length / 0.3385,
        );
        outline.position.y = hoverDimensions.height / 2 + 0.01;
        ringMaterial.opacity = 0.8 + Math.sin(elapsed * 5) * 0.16;
        diskMaterial.opacity = 0.11 + Math.sin(elapsed * 5) * 0.035;
        projectHover();
      }
    }
    ...
    while (accumulator + 1e-9 >= STEP) {
      ...
      if (selected && motion)
        stepVehicle(state, motion, input, STEP, getVehicleDimensions(selected.model));
  ...
  return {
    ...
    setState(next) {
      ...
      state = next;
      collisionWorldFor(next).setState(next);
      camera.far = Math.max(500, next.size * 4);
      camera.updateProjectionMatrix();
    },
    hover,
    leaveHover() {
      pointerOverCanvas = false;
      if (hovered) missedHover = 0.8;
    },
    clearHover,
    enter,
    exit,
    ...
    refreshLocale() {
      for (const car of getCars()) {
        refreshVehicleLabel(car.model);
        refreshFacilityServiceVehicleLabel(car.model);
      }
      projectHover(true);
      emitStatus(true);
    },
    ...
    keyUp(code) {
      const handled = !!selected && driveKeys.has(code);
      keys.delete(code);
      return handled;
    },
    resize(width, height) {
