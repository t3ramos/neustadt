...
const isLand = (tile: Tile) => tile.elevation >= 0 && !['road', 'rail', 'water'].includes(tile.kind);
...
export function getPowerBlockMask(state: CityState): Uint8Array {
  ...
  for (let start = 0; start < state.tiles.length; start++) {
    ...
    const cells = [start];
    visited[start] = 1;
    let enclosed = true, zoned = false;
    for (let cursor = 0; cursor < cells.length; cursor++) {
      const tile = state.tiles[cells[cursor]];
      if (tile.x === 0 || tile.z === 0 || tile.x === state.size - 1 || tile.z === state.size - 1) enclosed = false;
