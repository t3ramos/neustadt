...
function makeWater(): THREE.MeshPhysicalMaterial {
  ...
  const heightAt = (x: number, z: number) =>
    Math.sin((x / size) * Math.PI * 16 + Math.sin((z / size) * Math.PI * 8)) * 0.5 +
    Math.sin((z / size) * Math.PI * 24) * 0.25;
  const normal = new THREE.Vector3();
  for (let z = 0; z < size; z++)
    for (let x = 0; x < size; x++) {
      normal
        .set(
          (heightAt(x - 1, z) - heightAt(x + 1, z)) * 0.4,
          (heightAt(x, z - 1) - heightAt(x, z + 1)) * 0.4,
          1,
        )
        .normalize();
 ...
 */
export async function createPathTracingSnapshot(
  source: THREE.Scene,
  night = false,
  isCanceled: () => boolean = () => false,
): Promise<PathTracingSnapshot> {
  const checkCanceled = () => {
  ...
  result.background =
    source.background instanceof THREE.Color
      ? source.background.clone()
      : new THREE.Color(0xcbded8);
  ...
  function materialFor(original: THREE.Material): THREE.MeshStandardMaterial {
    ...
    if (original instanceof THREE.MeshStandardMaterial) material = original.clone();
      ...
      if (basic.color) {
        material.emissive.copy(basic.color);
    ...
    for (let index = 0; index < objects.length; index++) {
      ...
      if (object instanceof THREE.Mesh) {
        ...
        if (object instanceof THREE.InstancedMesh) {
          ...
          for (let instance = 0; instance < object.count; instance++) {
            ...
            if (object.instanceColor) {
              ...
              copyMaterials.forEach((material) => {
          ...
          if (!merged)
            throw new Error(
              'Die Gelände-Geometrie konnte nicht für Raytracing vorbereitet werden.',
            );
          geometry = merged;
        } else {
        ...
        const mesh = new THREE.Mesh(
          geometry,
          Array.isArray(object.material) ? copyMaterials : copyMaterials[0],
        );
...
function disposeTracer(tracer: PhotoTracer, renderer: THREE.WebGLRenderer) {
  type PhysicalMaterial = THREE.Material & {
    bvh?: Disposable;
    attributesArray?: Disposable;
    materialIndexAttribute?: Disposable;
    materials?: Disposable;
    lights?: { tex?: Disposable };
    envMapInfo?: Disposable;
    stratifiedTexture?: Disposable;
    stratifiedOffsetTexture?: Disposable;
    textures?: THREE.Texture;
    iesProfiles?: THREE.Texture;
  };
  type PhotoRenderer = Disposable & {
    material?: PhysicalMaterial;
    _blendQuad?: FullScreenQuad;
    _compileFunction?: () => void;
  };
  const internal = tracer as PhotoTracer & {
    _pathTracer?: PhotoRenderer;
    _lowResPathTracer?: PhotoRenderer;
    _colorBackground?: Disposable;
    _internalBackground?: Disposable;
    _generator?: {
      geometry?: Disposable;
      staticGeometryGenerator?: {
        _intermediateGeometry?: Map<string, THREE.BufferGeometry>;
        _dummyMesh?: THREE.Mesh;
      };
    };
  };
  ...
  for (const pathRenderer of renderers) {
    ...
    if (pathRenderer._compileFunction)
      material.removeEventListener('recompilation' as 'dispose', pathRenderer._compileFunction);
    if (!disposed.has(material)) {
      for (const resource of [
        material.bvh,
        material.attributesArray,
        material.materialIndexAttribute,
        material.materials,
        material.lights?.tex,
        material.envMapInfo,
        material.stratifiedTexture,
        material.stratifiedOffsetTexture,
      ])
        release(resource);
      for (const texture of [material.textures, material.iesProfiles]) {
        ...
        const properties = renderer.properties?.get(texture) as
          { __renderTarget?: ArrayTarget } | undefined;
        const target = (texture.renderTarget ?? properties?.__renderTarget) as
...
export class CityPathTracer {
  ...
  private configure(tracer = this.backend?.tracer) {
    ...
    tracer.renderScale =
      this.quality === 'ultra' ? 1 : this.quality === 'performance' ? 0.55 : 0.75;
  ...
  async setEnabled(enabled: boolean): Promise<boolean> {
    ...
    if (
      ...
      return this.fail(
        'Raytracing benötigt WebGL 2 mit Float-Renderzielen. Die Stadt bleibt im Echtzeitmodus spielbar.',
      );
    }
    this.renderer.debug.checkShaderErrors = true;
    this.renderer.debug.onShaderError = (context, program, vertex, fragment) => {
      if (this.enabled)
        this.fail('Der Raytracing-Shader wird von diesem Grafiktreiber nicht unterstützt.');
      console.error(
        'Raytracing shader:',
  ...
  private async createBackend(isCanceled: () => boolean): Promise<PhotoBackend> {
    const [{ WebGLPathTracer: Tracer, DenoiseMaterial: Denoiser }, { GenerateMeshBVHWorker }] =
      await Promise.all([import('three-gpu-pathtracer'), import('three-mesh-bvh/worker')]);
    if (isCanceled()) throw new DOMException('Raytracing-Vorbereitung abgebrochen.', 'AbortError');
    const worker = new GenerateMeshBVHWorker() as InstanceType<typeof GenerateMeshBVHWorker> &
      Disposable;
  ...
  private rebuild(): Promise<boolean> {
    ...
    const isCanceled = () =>
      this.disposed ||
      !this.enabled ||
      request !== this.requestVersion ||
      revision !== this.sceneVersion;
    this.dirty = false;
    this.building = (async () => {
      ...
      try {
        ...
        candidate.snapshot = await createPathTracingSnapshot(
          this.source,
          this.isNight(),
          isCanceled,
        );
        ...
        await candidate.tracer.setSceneAsync(candidate.snapshot.scene, this.camera, {
          onProgress: (value) => {
            if (!isCanceled()) this.progress = `Raytracing-Geometrie: ${Math.round(value * 100)} %`;
          },
        });
        if (isCanceled()) return false;
        ...
        return this.fail(
          error instanceof Error
            ? `Raytracing konnte nicht gestartet werden: ${error.message}`
            : 'Raytracing konnte nicht gestartet werden.',
        );
      } finally {
  ...
  render(): boolean {
    ...
    try {
      ...
      this.fail(
        'Raytracing wurde beendet, weil der Grafiktreiber den Renderdurchlauf nicht ausführen konnte.',
      );
      return false;
    }
  }
  ...
  getStatus(): RenderStatus {
    if (!this.enabled)
      return {
        mode: 'realtime',
        samples: 0,
        status: this.error ? 'Raytracing nicht verfügbar' : 'Echtzeit',
        ...(this.error ? { error: this.error } : {}),
      };
    const tracer = this.backend?.tracer;
    if (!tracer || this.building || this.dirty)
      return { mode: 'raytracing', samples: tracer?.samples ?? 0, status: this.progress };
    const samples = tracer.samples;
    const status = tracer.isCompiling
      ? 'Raytracing-Shader wird kompiliert …'
      : samples < 1
        ? 'Erste Lichtstrahlen werden berechnet …'
        : tracer.pausePathTracing
          ? 'Fotoberechnung abgeschlossen'
          : 'Indirektes Licht und Reflexionen verfeinern sich …';
