...
const POOL_HALF = 0.445;
const POOL_LIFT = 0.0457;
const WARM_LIGHT = 0xffdfa2;
const LEGACY_PARTS = [
  { type: 'CylinderGeometry', position: [-0.413, 0.265, 0.405], scale: [0.025, 0.48, 0.025] },
  { type: 'BoxGeometry', position: [-0.343, 0.505, 0.405], scale: [0.16, 0.025, 0.035] },
  { type: 'BoxGeometry', position: [-0.275, 0.49, 0.405], scale: [0.07, 0.024, 0.052] },
] as const;
  ...
  for (const part of LEGACY_PARTS) {
    const child = model.children.find(
      (object) =>
        ...
        part.position.every(
          (value, axis) => Math.abs(object.position.getComponent(axis) - value) < 1e-8,
        ) &&
        part.scale.every(
          (value, axis) => Math.abs(object.scale.getComponent(axis) - value) < 1e-8,
        ) &&
        object.rotation.x === 0 &&
        object.rotation.y === 0 &&
        object.rotation.z === 0,
...
function radialTexture(): THREE.DataTexture {
  ...
  for (let z = 0; z < size; z++)
    for (let x = 0; x < size; x++) {
      const r = Math.hypot(((x + 0.5) / size) * 2 - 1, ((z + 0.5) / size) * 2 - 1);
...
function poolTriangle(): THREE.BufferGeometry {
  ...
  geometry.setAttribute(
    'position',
    new THREE.Float32BufferAttribute([0, 0, 0, 1, 0, 1, 1, 0, 0], 3),
  );
...
export function getRoadStreetlightFixture(
  state: CityState,
  tile: Tile,
): {
  mast: THREE.Vector3;
  ...
  const mastX = centerX - 0.47,
    bulbX = centerX - 0.332,
    z = centerZ + 0.47;
  const ground = sampleRoadHeight(state, mastX, z);
  return {
    mast: new THREE.Vector3(mastX, ground + 0.045, z),
    bulb: new THREE.Vector3(bulbX, sampleRoadHeight(state, bulbX, z) + 0.49, z),
    mastRadius: 0.014,
    mastHeight: 0.468,
    base: new THREE.Vector3(mastX, ground + 0.025, z),
    baseRadius: 0.036,
    baseHeight: 0.028,
  };
}
...
export function createStreetlights(initialState: CityState) {
  ...
  const pole = new THREE.CylinderGeometry(0.38, 0.5, 1, 10);
  const base = new THREE.CylinderGeometry(0.48, 0.62, 1, 10);
  ...
  const unlitLens = new THREE.MeshStandardMaterial({
    color: 0xc8cbbc,
    roughness: 0.24,
    metalness: 0.05,
  });
  const litLens = new THREE.MeshStandardMaterial({
    color: 0xffedc7,
    roughness: 0.24,
    metalness: 0.05,
    emissive: WARM_LIGHT,
  ...
  let capacity = 0,
    ...
    disposed = false;
  let blend = 0,
    enabled = initialState.settings.buildingLights ?? true;
  let nightStrength = 0;
  let lamps: Lamp[] = [],
    poweredLamps: Lamp[] = [];
  let batches: THREE.InstancedMesh[] = [];
  let bases: THREE.InstancedMesh, poles: THREE.InstancedMesh, arms: THREE.InstancedMesh;
  let housings: THREE.InstancedMesh,
    offLenses: THREE.InstancedMesh,
    onLenses: THREE.InstancedMesh,
  ...
  function applyLighting() {
    nightStrength = THREE.MathUtils.smoothstep(blend, 0.08, 0.72);
    litLens.emissiveIntensity = enabled ? 0.1 + 3.6 * blend : 0;
    poolMaterial.emissiveIntensity = enabled ? nightStrength : 0;
    poolMaterial.opacity = enabled ? 0.38 * nightStrength : 0;
  ...
  function update(state: CityState) {
    ...
    for (let index = 0; index < tiles.length; index++) {
      ...
      const mastX = fixture.mast.x,
        lampZ = fixture.mast.z,
        bulbX = fixture.bulb.x;
      const mastGround = fixture.mast.y - 0.045,
        bulbGround = fixture.bulb.y - 0.49;
      const bulb = fixture.bulb;
      const lamp = {
        id: tile.z * state.size + tile.x,
        bulb,
        powered: tile.powered && tile.fire === 0,
      };
      lamps.push(lamp);
      place(bases, index, mastX, mastGround + 0.039, lampZ, 0.058, 0.028, 0.058);
      place(poles, index, mastX, mastGround + 0.279, lampZ, 0.028, 0.468, 0.028);
      const armStart = new THREE.Vector3(mastX, mastGround + 0.505, lampZ);
      const armEnd = new THREE.Vector3(bulbX + 0.023, bulbGround + 0.505, lampZ);
      delta.copy(armEnd).sub(armStart);
      transform.position.copy(armStart).add(armEnd).multiplyScalar(0.5);
      transform.quaternion.setFromUnitVectors(xAxis, delta.clone().normalize());
      transform.scale.set(delta.length(), 0.024, 0.031);
      transform.updateMatrix();
      arms.setMatrixAt(index, transform.matrix);
      place(housings, index, bulbX, bulbGround + 0.509, lampZ, 0.095, 0.017, 0.07);
      const lensBatch = lamp.powered ? onLenses : offLenses,
        lensIndex = lamp.powered ? onCount++ : offCount++;
      place(lensBatch, lensIndex, bulbX, bulb.y, lampZ, 0.073, 0.024, 0.052);
      if (!lamp.powered) continue;
      const poolIndex = poweredLamps.length * 2;
      poweredLamps.push(lamp);
      const west = x - POOL_HALF,
        east = x + POOL_HALF,
        north = z - POOL_HALF,
        south = z + POOL_HALF;
    ...
    offLenses.count = offCount;
    onLenses.count = onCount;
    pools.count = poweredLamps.length * 2;
    for (const batch of batches) {
      batch.instanceMatrix.needsUpdate = true;
      batch.computeBoundingBox();
      batch.computeBoundingSphere();
    }
    field.update(poweredLamps);
    applyLighting();
  }
  ...
  return {
    ...
    animate(_dt: number, _target: THREE.Vector3) {
      /* Light assignment is world-fixed. */
    },
    setQuality(_value: Quality) {
      /* All presets retain every supplied lamp. */
    },
    getDebug() {
      return {
        count: lamps.length,
        litCount: enabled ? poweredLamps.length : 0,
        pointLights: 0,
        activePointLights: 0,
        field: field.getDebug(),
        groundPools: !disposed && pools.visible ? pools.count / 2 : 0,
        enabled,
        nightBlend: blend,
        positions: lamps.slice(0, 128).map((lamp) => ({
          id: lamp.id,
          x: lamp.bulb.x,
          y: lamp.bulb.y,
          z: lamp.bulb.z,
          powered: lamp.powered,
        })),
        points: [],
      };
    },
