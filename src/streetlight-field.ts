...
export interface StreetlightSource { id:number; bulb:THREE.Vector3; }
const RANGE=1.85;
...
const bindings=new WeakMap<StandardMaterial,{uniforms:FieldUniforms;original:StandardMaterial['onBeforeCompile'];cacheKey:string}>();
...
export function createStreetlightField(size:number) {
  const uniforms:FieldUniforms={
    ...
    cityLightSize:{value:size},cityLightCount:{value:1},cityLightStrength:{value:0},
    cityLightColor:{value:new THREE.Color(0xffdfa2)},
  };
  let sourceCount=0,revision=0;
  function update(sources:readonly StreetlightSource[]) {
    sourceCount=sources.length;revision++;
    const width=Math.max(1,sources.length),data=new Float32Array(width*4);
    const cells=Array.from({length:size*size},()=>[] as {index:number;distance:number}[]);
    const half=size/2;
    sources.forEach((source,index)=>{
      const p=source.bulb;data.set([p.x,p.y-.022,p.z,.55],index*4);
      const gx=p.x+half,gz=p.z+half;
      for(let z=Math.max(0,Math.floor(gz-RANGE));z<=Math.min(size-1,Math.floor(gz+RANGE));z++){
        for(let x=Math.max(0,Math.floor(gx-RANGE));x<=Math.min(size-1,Math.floor(gx+RANGE));x++){
          const candidates=cells[z*size+x];
          candidates.push({index:index+1,distance:(x+.5-gx)**2+(z+.5-gz)**2});
        }
      }
    });
    ...
    cells.forEach((candidates,cell)=>{
      ...
      candidates.slice(0,LIGHTS_PER_CELL).forEach((candidate,i)=>indices[cell*4+i]=candidate.index);
    });
    uniforms.cityLightIndex.value.image.data!.set(indices);uniforms.cityLightIndex.value.needsUpdate=true;
    if(uniforms.cityLightData.value.image.width!==width){uniforms.cityLightData.value.dispose();uniforms.cityLightData.value=texture(data,width,1);}
    else {uniforms.cityLightData.value.image.data!.set(data);uniforms.cityLightData.value.needsUpdate=true;}
  ...
  function applyTo(root:THREE.Object3D) {
    root.traverse(object=>{
      ...
      for(const material of Array.isArray(object.material)?object.material:[object.material]){
        if(!(material instanceof THREE.MeshStandardMaterial)||material.userData.cityLightFieldExclude)continue;
        const existing=bindings.get(material);
        if(existing?.uniforms===uniforms)continue;
        const binding=existing??{uniforms,original:material.onBeforeCompile,cacheKey:material.customProgramCacheKey()};
        binding.uniforms=uniforms;bindings.set(material,binding);
  ...
  return {
    update,applyTo,
    setStrength(value:number){uniforms.cityLightStrength.value=Number.isFinite(value)?Math.max(0,value):0;},
    getDebug:()=>({sources:sourceCount,candidatesPerCell:LIGHTS_PER_CELL,revision,strength:uniforms.cityLightStrength.value}),
    ...
    sourcesAt(x:number,z:number):number[]{
      ...
      return Array.from(uniforms.cityLightIndex.value.image.data!.slice((gz*size+gx)*4,(gz*size+gx+1)*4)).filter(i=>i>0).map(i=>i-1);
    },
    dispose(){uniforms.cityLightStrength.value=0;uniforms.cityLightIndex.value.dispose();uniforms.cityLightData.value.dispose();},
  };
}
