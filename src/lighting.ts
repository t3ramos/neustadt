...
const mixColor = (a: number, b: number, amount: number) => new THREE.Color(a).lerp(new THREE.Color(b), amount);
...
export function getLightingProfile(inputHour: number): LightingProfile {
  ...
  const sunDirection = new THREE.Vector3(-Math.cos(phase), Math.sin(phase) * .94, .3).normalize();
  const moonDirection = sunDirection.clone().negate();
  const altitude = sunDirection.y;
  const daylight = smooth(altitude, -.14, .24);
  const nightBlend = 1 - daylight;
  const twilight = Math.exp(-Math.pow(altitude / .23, 2)) * smooth(altitude, -.3, -.06);
  const sunIntensity = 3.15 * smooth(altitude, 0, .27);
  const moonIntensity = .46 * smooth(-altitude, 0, .25);
  const lightDirection = (altitude >= 0 ? sunDirection : moonDirection).clone();
  const dayLightColor = mixColor(0xffb66e, 0xffefd9, smooth(altitude, .015, .53));
  const lightColor = altitude >= 0 ? dayLightColor : new THREE.Color(0x9bbbea);
  const background = mixColor(0x172b43, 0xcddedb, daylight).lerp(new THREE.Color(0xeab58a), twilight * .57);
  const zenith = mixColor(0x071122, 0x71acd0, daylight).lerp(new THREE.Color(0x6681a2), twilight * .35);
  const ground = mixColor(0x1c292b, 0x82927b, daylight);
  return {
    hour, sunDirection, moonDirection, lightDirection, daylight, nightBlend,
    night: nightBlend > .58,
    sunIntensity, moonIntensity, lightIntensity: sunIntensity + moonIntensity,
    ambientIntensity: .5 + daylight * .6,
    fillIntensity: .12 + daylight * .3,
    exposure: 1.12 - daylight * .15,
    bloom: .08 - daylight * .045,
    background, zenith, ground, lightColor,
...
export interface CityLighting {
  setTime(hour: number): Pick<LightingProfile, 'night' | 'nightBlend' | 'exposure' | 'bloom' | 'background'>;
...
export function stabilizeShadowAnchor(
  ...
  if (![horizontalStep, verticalStep].every(value => Number.isFinite(value) && value > 0)) return target.clone();
  ...
  const referenceUp = new THREE.Vector3(0, Math.abs(forward.y) > .9999 ? 0 : 1, Math.abs(forward.y) > .9999 ? 1 : 0);
 ...
 */
export function reflectionShadowsReady(renderer:Pick<THREE.WebGLRenderer,'shadowMap'>,scene:THREE.Scene):boolean {
  if(!renderer.shadowMap.enabled)return true;
  const comparison=renderer.shadowMap.type===THREE.PCFShadowMap||renderer.shadowMap.type===THREE.PCFSoftShadowMap;
  let ready=true;
  scene.traverseVisible(object=>{
    ...
    if(!map || comparison&&(!map.depthTexture||map.depthTexture.compareFunction===null))ready=false;
  });
  return ready;
}
...
export function stableShadowChunk(source=THREE.ShaderChunk.shadowmap_pars_fragment):string {
  ...
  const replacement=original.replace(/vec2\s+texelSize\s*=\s*vec2\(\s*1\.0\s*\)\s*\/\s*shadowMapSize;[\s\S]*?\)\s*\*\s*0\.2;/,`
...
export function applyStableShadowFiltering(root:THREE.Object3D):void {
  root.traverse(object=>{
    ...
    for(const material of materials){
      if(!(material instanceof THREE.MeshStandardMaterial)||stabilizedMaterials.has(material))continue;
      ...
      material.onBeforeCompile=function(shader,renderer){
        ...
        shader.fragmentShader=shader.fragmentShader.replace('#include <shadowmap_pars_fragment>',deterministicShadowChunk);
      };
      material.customProgramCacheKey=function(){return (usesDefaultKey?originalDefaultKey:previousKey.call(this))+'|neustadt-pcf-tent-v1';};
 ...
 */
export function createCityLighting({scene, renderer, sunlight, ambient, fill, worldSize}: CityLightingOptions): CityLighting {
  const previousEnvironment = scene.environment;
  const previousEnvironmentIntensity = scene.environmentIntensity;
  const worldRadius = Math.max(180, worldSize * 2.35);
  const lightDistance = Math.max(95, worldSize * .85);
  ...
  const skyWidth = 128, skyHeight = 64;
  const pixels = new Float32Array(skyWidth * skyHeight * 4);
  const skyTexture = new THREE.DataTexture(pixels, skyWidth, skyHeight, THREE.RGBAFormat, THREE.FloatType);
  ...
  function updateFog(): void {
    ...
    if (scene.fog instanceof THREE.Fog) {
      ...
      scene.fog.near = driving ? Math.max(28, worldSize * .42) : Math.max(150, worldSize * 2.3);
  ...
  function updateAnchor(target: THREE.Vector3): void {
    ...
    const shadowAnchor = stabilizeShadowAnchor(
      profile.lightDirection, anchor,
      (shadowCamera.right - shadowCamera.left) / 2, sunlight.shadow.mapSize.x,
      (shadowCamera.top - shadowCamera.bottom) / 2, sunlight.shadow.mapSize.y,
    );
    ...
    fill.position.copy(anchor).addScaledVector(profile.lightDirection, -lightDistance * .7);
    fill.position.y = anchor.y + lightDistance * .45;
  ...
  function setTime(hour: number): ReturnType<CityLighting['setTime']> {
    ...
    if (Math.min(difference, 24 - difference) > .8) {
    ...
    scene.environmentIntensity = .24 - profile.nightBlend * .09;
  ...
  function updateFallbackEnvironment(): void {
    const sample = new THREE.Color(), direction = new THREE.Vector3();
    for (let y = 0; y < skyHeight; y++) {
      // DataTexture row zero maps to the south pole in equirectangular UVs.
      const latitude = ((y + .5) / skyHeight - .5) * Math.PI;
      const height = Math.sin(latitude);
      for (let x = 0; x < skyWidth; x++) {
        const longitude = ((x + .5) / skyWidth - .5) * Math.PI * 2;
        direction.set(Math.cos(longitude) * Math.cos(latitude), height, Math.sin(longitude) * Math.cos(latitude));
        sample.copy(profile.background).lerp(profile.zenith, Math.pow(Math.max(0, height), .48));
        sample.lerp(profile.ground, smooth(-height, .02, .6));
        const sunGlow = Math.pow(Math.max(0, direction.dot(profile.sunDirection)), 80) * profile.daylight * .38;
        sample.r += sunGlow;
        sample.g += sunGlow * .72;
        sample.b += sunGlow * .42;
        const offset = (y * skyWidth + x) * 4;
        pixels[offset] = sample.r; pixels[offset + 1] = sample.g; pixels[offset + 2] = sample.b; pixels[offset + 3] = 1;
      }
    }
    ...
    environmentBucket = Math.floor(profile.hour / 1.5);
    lastEnvironmentUpdate = performance.now();
  }

  function refreshReflections(_target: THREE.Vector3, force = false): void {
    if (disposed) return;
    const bucketChanged = environmentBucket !== Math.floor(profile.hour / 1.5);
    if (fallbackEnvironment && !bucketChanged) return;
    const now = performance.now();
    if (fallbackEnvironment && !force && now - lastEnvironmentUpdate < 1000) return;
  ...
  setTime(12);
  return {
    setTime, updateAnchor, refreshReflections, setQuality,
    setDriving(active) { if (disposed) return; driving = active; updateFog(); },
    invalidateReflections() { /* City edits do not alter the sky environment. */ },
    dispose() {
      if (disposed) return;
      disposed = true;
      if (scene.environment === fallbackEnvironment?.texture) scene.environment = previousEnvironment;
      scene.environmentIntensity = previousEnvironmentIntensity;
      fallbackEnvironment?.dispose();
      skyTexture.dispose();
      sky.removeFromParent();
      sky.geometry.dispose(); skyMaterial.dispose(); pmrem.dispose();
