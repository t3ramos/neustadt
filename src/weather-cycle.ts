import type { CityState,Weather } from './types';

function duration(seed:number,cycle:number,weather:Weather):number {
  const n=Math.sin(seed*.017+cycle*78.233+13.4)*43758.5453;
...
export function initializeWeather(state:CityState):void {
  ...
  s.dynamicWeather??=true;s.weatherCycle??=0;
...
export function chooseWeather(state:CityState,weather:Weather,automatic=false):void {
  ...
  const s=state.settings;s.weather=weather;s.dynamicWeather=automatic;
  s.weatherCycle!++;s.weatherRemaining=duration(state.seed,s.weatherCycle!,weather);
}
...
export function advanceWeather(state:CityState,simulationSeconds:number):boolean {
  initializeWeather(state);
  const s=state.settings;
  if(!s.dynamicWeather||!Number.isFinite(simulationSeconds)||simulationSeconds<=0)return false;
  s.weatherRemaining!-=simulationSeconds;
  let changed=false;
  while(s.weatherRemaining!<=0){
    s.weather=s.weather==='rain'?'clear':'rain';s.weatherCycle!++;
    s.weatherRemaining!+=duration(state.seed,s.weatherCycle!,s.weather);changed=true;
  }
  return changed;
}
